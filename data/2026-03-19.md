...
Motivation: 现有的放射学报告生成方法通常使用单一模型的强化学习，或者对独立训练的模型进行后处理智能体化，这些方法无法联合训练多个智能体并通过强化学习优化整个智能体系统。

Method: 提出MARL-Rad框架，协调区域特定智能体和全局整合智能体，使用临床可验证奖励进行多智能体强化学习优化，联合训练多个智能体并优化整个智能体系统。

Result: 在MIMIC-CXR和IU X-ray数据集上的实验表明，MARL-Rad在RadGraph、CheXbert和GREEN等临床效能指标上持续改进，达到最先进的临床效能表现，同时增强了侧向一致性和生成更准确、细节丰富的报告。

Conclusion: MARL-Rad通过多模态多智能体强化学习框架有效提升了放射学报告生成的临床效能，为医疗报告生成提供了一种新的协同智能体优化方法。
...
TL;DR: IMU手写识别中，子词分词主要解决跨书写者风格差异，而拼接数据增强则有效缓解单个书写者的数据稀疏问题。


<details>
  <summary>Details</summary>
Motivation: IMU手写识别面临字符分布不均和书写者间差异的挑战，需要系统研究如何解决这些问题。

Method: 采用两种策略：子词分词（Bigram分词）和基于拼接的数据增强，在OnHW-Words500数据集上进行实验。

Result: 在书写者独立分割中，Bigram分词将词错误率从15.40%降至12.99%；在书写者依赖分割中，拼接数据增强将字符错误率降低34.5%，词错误率降低25.4%。

Conclusion: 子词分词主要缓解书写者间风格差异，而拼接数据增强有效补偿书写者内分布稀疏性，揭示了方差依赖效应。
...
TL;DR: 提出BHU框架，通过Top-K像素选择减少通信开销，结合MU-MIMO传输和Swin-large编码器进行BEV特征融合，使用扩散模型DRL联合优化，在Air-Co-Pred数据集上实现感知性能提升5%同时通信开销降低85%。


<details>
  <summary>Details</summary>
Motivation: 多无人机协同感知在低空经济应用中前景广阔，但多视角视觉数据带来的通信延迟和资源效率问题亟待解决，需要在保证感知性能的同时降低通信开销。

Method: 1) Top-K选择机制筛选信息量最大的像素，实现稀疏化视觉传输；2) 通过MU-MIMO将稀疏图像传输到地面服务器；3) 使用Swin-large-based MaskDINO编码器提取BEV特征并进行协同特征融合；4) 开发基于扩散模型的DRL算法联合选择协同无人机、稀疏化比率和预编码矩阵。

Result: 在Air-Co-Pred数据集上的仿真结果表明，相比传统CNN-based BEV融合基线，BHU框架将感知性能提升超过5%，同时通信开销减少85%。

Conclusion: BHU框架为资源受限无线环境下的多无人机协同感知提供了有效解决方案，在通信效率和感知效用之间取得了良好平衡。
...
TL;DR: 提出一种结合迁移学习和宽度学习系统（BLS）的面部美感预测方法，通过EfficientNets特征提取和BLS快速建模，提高了预测精度。


<details>
  <summary>Details</summary>
Motivation: 面部美感预测面临数据不足易过拟合、人脸外观多变、人类感知复杂等挑战，需要快速构建鲁棒有效的模型。

Method: 1. 使用基于迁移学习的EfficientNets构建特征提取器（E-BLS）；2. 设计连接层连接特征提取器和BLS（ER-BLS）；3. 将提取的面部特征转移到BLS进行预测。

Result: E-BLS和ER-BLS相比现有BLS和CNN方法提高了面部美感预测的准确率，证明了方法的有效性和优越性。

Conclusion: 提出的迁移学习与宽度学习系统融合方法能有效解决面部美感预测问题，并可广泛应用于模式识别、目标检测和图像分类等领域。
...
TL;DR: 本文提出Script-to-Slide Grounding (S2SG)任务，利用大语言模型自动将脚本句子与幻灯片对象进行关联，为自动化教学视频生成奠定基础。


<details>
  <summary>Details</summary>
Motivation: 基于幻灯片的教学视频制作中，将视觉特效与脚本内容关联到幻灯片对象的过程非常耗时费力，需要自动化解决方案。

Method: 提出S2SG任务定义，并开发Text-S2SG方法，利用大语言模型将脚本句子与幻灯片中的文本对象进行关联。

Result: 实验显示该方法性能优异，F1分数达到0.924，证明了大语言模型在该任务上的有效性。

Conclusion: 将隐式的幻灯片视频编辑过程形式化为可计算任务，为自动化教学视频生成开辟了新途径。
...
TL;DR: AwaRes是一个空间按需框架，通过低分辨率全局视图和工具调用检索高分辨率片段，解决视觉语言模型中精度与效率的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型通常需要在高分辨率下处理图像，这导致了精度和计算效率之间的权衡：高分辨率能捕捉细节但计算成本高，低分辨率效率高但可能丢失关键视觉信息（如小文本）。

Method: 1. 使用低分辨率全局视图，通过工具调用仅检索查询所需的高分辨率片段；2. 自动构建监督数据：通过比较低分辨率和高分辨率答案判断是否需要裁剪，使用oracle grounding模型定位证据；3. 通过冷启动SFT和多回合GRPO训练，结合语义答案正确性和裁剪成本惩罚的复合奖励。

Result: 提出了AwaRes框架，能够根据查询需求动态选择高分辨率区域，在保持计算效率的同时提高对细节敏感任务（如小文本识别）的准确性。
...
TL;DR: 提出V2VK管道自动生成AgriMM基准，包含3000+农业类别和607k+视觉问答，并基于此开发了农业专用多模态大模型AgriChat，在多项农业任务上表现优于现有开源模型。


<details>
  <summary>Details</summary>
Motivation: 当前农业多模态大模型面临两大挑战：缺乏大规模农业数据集用于模型开发和评估，以及现有模型缺乏经过验证的领域专业知识来跨不同分类进行推理。

Method: 提出Vision-to-Verified-Knowledge (V2VK)管道，结合视觉描述和基于网络的科学检索，自动生成AgriMM基准数据集。基于该数据集开发了专门的农业多模态大模型AgriChat。

Result: AgriMM基准包含3000+农业类别和607k+视觉问答，涵盖植物物种识别、病害症状识别、作物计数和成熟度评估等任务。AgriChat在多项农业任务上表现优于其他开源模型。

Conclusion: 保持视觉细节与网络验证知识相结合是构建稳健可信农业AI的可靠途径。AgriChat展示了在农业领域的优越性能，代码和数据集已公开。
...
TL;DR: GenLie是一个用于视频谎言检测的全局增强网络，通过局部特征建模和全局监督来学习稀疏但具有判别性的欺骗行为表示。
...
Method: 提出GenLie网络，在全局监督下进行局部特征建模。局部层面捕捉稀疏微妙的欺骗线索，全局监督和优化通过抑制身份相关噪声来确保鲁棒且具有判别性的表示。
...
<details>
  <summary>Details</summary>
Motivation: 文本引导的人体动画发展迅速，但面部动画因缺乏高质量文本配对的面部数据集而滞后。需要填补这一空白，建立文本与面部运动之间的桥梁。

Method: 1. 设计涵盖情绪和头部运动的提示词套件，使用多个生成器合成约80小时面部视频；2. 提取每帧3D面部参数，构建大规模（提示词-参数）配对数据集；3. 基于该数据集训练语言模型执行双向任务：Motion2Language（从3D面部参数生成文本描述）和Language2Motion（从文本生成量化运动令牌用于动画）。

Result: 实验表明，在此设置下语言模型能够很好地解释和合成面部运动，并具有较强的泛化能力。这是首个将面部参数建模视为语言问题的工作。

Conclusion: 通过将面部参数建模为语言问题，建立了文本条件面部动画和运动理解的统一路径，为面部动画领域提供了新的解决方案。
...
TL;DR: 提出基于分歧的多模态融合方法，通过显式测量视觉、音频和文本模态间的冲突来识别矛盾/犹豫视频，在BAH数据集上取得0.6808的Macro F1分数，显著优于基线0.2827。


<details>
  <summary>Details</summary>
Motivation: 针对第10届ABAW竞赛中的矛盾/犹豫视频识别挑战，现有方法未能有效捕捉多模态间的冲突特征，而矛盾/犹豫状态通常表现为不同模态信息的不一致。

Method: 采用基于分歧的多模态融合方法：视觉特征通过Py-Feat提取动作单元，音频通过Wav2Vec 2.0编码，文本通过BERT编码；各模态经BiLSTM和注意力池化处理，投影到共享嵌入空间；融合模块计算模态嵌入间的绝对差异，直接捕捉表征矛盾/犹豫的模态冲突。

Result: 在BAH数据集验证集上获得0.6808的Macro F1分数，显著优于挑战基线0.2827；统计分析1132个视频证实动作单元的时间变异性是识别矛盾/犹豫的主要视觉判别特征。

Conclusion: 基于分歧的多模态融合方法能有效捕捉矛盾/犹豫状态中的模态冲突，动作单元的时间变异性是关键视觉线索，该方法在矛盾/犹豫视频识别任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 当前传感器设备生成的稀疏骨架数据会丢失细粒度时空细节，且预定义的物理传感器拓扑限制了长程依赖建模，需要克服这些限制来提升动作识别性能。

Method: 1. 运动学驱动高斯泼溅模块：利用瞬时关节速度向量动态构建各向异性协方差矩阵，将稀疏骨架序列渲染成富含时空语义的多视角连续热图；2. 概率拓扑构建方法：通过Bhattacharyya距离量化关节高斯分布间的统计相关性，生成自适应先验邻接矩阵；3. 视觉上下文门控机制：让GCN主干自适应地调制渲染的视觉特征。

Result: 实验结果表明KGS-GCN显著增强了复杂时空动态建模能力，为处理低保真传感器数据提供了鲁棒解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有指令图像编辑基准通过混合评估追求任务广度，但忽视了专业应用中关键的一致性失败模式：模型在不同语义尺度任务上的不一致表现。

Method: 构建高质量人工标注基准Omni IIE Bench，采用创新的双轨诊断设计：1) 单轮一致性（属性修改和实体替换的共享上下文任务对）；2) 多轮协调（跨越语义尺度的连续对话任务）。通过多阶段人工筛选流程构建，包括计算机视觉研究生执行的质量标准和专业设计师的行业相关性审查。

Result: 对8个主流IIE模型进行全面评估，首次量化了普遍存在的性能差距：几乎所有模型从低语义尺度任务过渡到高语义尺度任务时都表现出显著的性能下降。
...
TL;DR: 提出.PcRecord统一存储格式和高性能数据处理流水线，显著提升大规模点云数据的加载和处理效率


<details>
  <summary>Details</summary>
Motivation: 点云数据规模大、格式多样，传统算法处理效率低，数据加载和处理耗时成为瓶颈
...
Conclusion: 提出的.PcRecord格式和并行处理系统有效解决了大规模点云数据处理的效率瓶颈问题
...
TL;DR: EmergeNav：一个零样本视觉语言导航框架，通过结构化执行层次将VLM先验知识转化为稳定的导航行为，无需任务特定训练或显式地图。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型（VLMs）在连续环境零样本视觉语言导航（VLN-CE）中存在挑战，虽然模型编码了有用的语义先验，但其开放推理无法直接转化为稳定的长时程具身执行。主要瓶颈在于缺乏组织指令跟随、感知接地、时间进度和阶段验证的执行结构。

Method: 提出EmergeNav框架，将连续VLN建模为结构化具身推理：1) Plan-Solve-Transition层次结构用于阶段化执行；2) GIPE用于目标条件感知提取；3) 对比双记忆推理用于进度接地；4) 角色分离的双视场感知用于时间对齐的局部控制和边界验证。

Result: 在VLN-CE上，仅使用开源VLM骨干网络（无需任务特定训练、显式地图、图搜索或路径点预测器），Qwen3-VL-8B达到30.00 SR，Qwen3-VL-32B达到37.00 SR的零样本性能。
...
TL;DR: 本文提出PhysQuantAgent框架和VisPhysQuant基准数据集，用于通过视觉语言模型进行真实世界物体质量估计，并引入三种视觉提示方法提升估计精度。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在机器人感知和操作中应用广泛，但缺乏可靠的物理属性推理能力，特别是质量估计能力。现有基准数据集大多未在真实传感条件下评估物理量估计，而准确的质量估计对于确定合适的抓握力和确保安全交互至关重要。
...
Conclusion: 通过视觉提示增强视觉语言模型的输入，可以有效提升其对物理属性（特别是质量）的推理能力，为机器人安全操作提供了重要技术支持。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统（ADS）的批准需要评估其在真实交通场景中的行为。现有场景提取方法依赖异构定义，阻碍场景可比性；而场景聚类方法中，基于机器学习的方法缺乏可解释性且可能不符合领域知识。

Method: 基于"场景即规范"概念建立标准化场景提取框架，并开发领域知识引导的场景聚类流程。在highD数据集上进行实验验证。

Result: 实验表明，场景可以被可靠地提取，领域知识可以有效地集成到聚类过程中。该方法支持从高速公路数据记录中推导场景类别的更标准化流程。

Conclusion: 提出的方法学支持从高速公路数据记录中推导场景类别的更标准化过程，从而实现自动驾驶车辆更高效的验证过程。
...
TL;DR: CineSRD：面向影视等开放世界场景的统一多模态说话人日志系统，通过视觉锚点聚类和音频语言模型实现说话人注册与日志，在视觉媒体领域表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统说话人日志系统主要针对会议、访谈等受限场景，无法处理影视等开放世界中的复杂挑战，包括长视频理解、大量说话人、跨模态异步和真实世界变异性。

Method: 提出CineSRD统一多模态框架，利用视频、语音和字幕的视觉、声学和语言线索。首先进行视觉锚点聚类注册初始说话人，然后集成音频语言模型进行说话人轮次检测，精炼标注并补充未注册的屏幕外说话人。

Result: 在构建的中英文视觉媒体说话人日志基准测试中表现优异，同时在传统数据集上也取得有竞争力的结果，验证了其在开放世界视觉媒体场景中的鲁棒性和泛化能力。

Conclusion: CineSRD成功将说话人日志任务扩展到影视等开放世界视觉媒体领域，通过多模态融合有效解决了该领域的独特挑战，为复杂视听节目的说话人分析提供了有效解决方案。
...
TL;DR: MSRAMIE是一个无需训练的代理框架，基于多模态大语言模型，通过结构化多模态推理处理复杂多指令图像编辑任务，显著提升复杂指令遵循能力。
...
Method: 提出MSRAMIE框架，将现有编辑模型作为插件组件，通过结构化多模态推理处理多指令任务。采用MLLM-based Instructor和图像编辑Actor的迭代交互，引入新颖的推理拓扑结构：Tree-of-States和Graph-of-References。在推理过程中，复杂指令被分解为多个编辑步骤，实现状态转换、跨步骤信息聚合和原始输入召回。

Result: 随着指令复杂度增加，MSRAMIE可将指令遵循能力提升超过15%，将单次运行完成所有修改的概率提高超过100%，同时保持感知质量和视觉一致性。

Conclusion: MSRAMIE通过无需训练的方式有效解决了复杂多指令图像编辑的挑战，提供可解释和可控的决策路径，显著提升了现有编辑模型在复杂场景下的性能。
...
TL;DR: MAND是一个用于多模态第一人称开放世界持续学习的模态感知框架，通过自适应模态评分和模态表示稳定训练，显著提升了新颖活动检测和已知类别分类的性能。
...
<details>
  <summary>Details</summary>
Motivation: 多模态知识图谱依赖视觉信息，但大规模图像收集困难，且常排除模糊但相关的视觉内容（如标志、符号、抽象场景），需要更好的视觉信息利用方法。

Method: 三阶段管道：1) 大规模检索实体相关图像；2) 将所有视觉输入转换为文本描述，确保模糊图像贡献语义而非噪声；3) 使用LLM融合多源描述生成简洁的实体对齐摘要，用于增强标准MMKG模型。

Result: 在三个公共MMKG数据集和多个基线模型上获得一致提升（整体Hits@1最高提升7%）；在具有视觉模糊标志和符号的实体子集上，图像转文本带来显著改进（MRR提升201.35%，Hits@1提升333.33%）。

Conclusion: 扩大图像覆盖范围并将模糊视觉内容转换为文本是增强MMKG补全性能的实用路径，同时发布了文本-图像一致性检查接口用于可选的质量审核。
...
TL;DR: 论文分析了紧凑型视觉语言模型推理效率低下的原因，提出了优化方法大幅降低延迟，并引入ArgusVLM模型家族，在保持紧凑高效的同时实现强性能。


<details>
  <summary>Details</summary>
Motivation: 在资源受限环境中部署视觉语言模型需要低延迟和高吞吐量，但现有紧凑型VLM的实际推理速度提升远低于其较小参数量所暗示的潜力，需要解释这种差异并找到优化方案。

Method: 1) 进行端到端效率分析，系统剖析推理过程识别主要瓶颈；2) 开发针对紧凑型VLM的优化方案，大幅降低延迟同时保持准确性；3) 扩展紧凑型VLM以支持结构化感知输出，引入ArgusVLM模型家族。

Result: 优化方法使InternVL3-2B的首token时间降低53%，SmolVLM-256M降低93%；ArgusVLM在多种基准测试中表现优异，同时保持紧凑高效的设计。

Conclusion: 通过系统性瓶颈分析和针对性优化，紧凑型视觉语言模型可以实现显著的推理速度提升；ArgusVLM展示了在保持高效性的同时扩展功能的可能性，为构建高效VLM系统提供实用指导。
...
TL;DR: HopChain框架通过合成多跳视觉语言推理数据来增强VLMs的细粒度推理能力，在24个基准测试中20个有提升


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言数据缺乏复杂的推理链，无法暴露VLMs在长链推理中的感知、推理、知识和幻觉错误

Method: 提出HopChain框架，合成多跳视觉语言推理数据，形成逻辑依赖的实例接地推理链，适合可验证奖励的强化学习

Result: 在24个基准测试中，20个有提升；多跳数据比单跳变体平均准确率高5.3-7.0分；在超长推理链中提升超50分

Conclusion: HopChain是有效、可扩展的多跳数据合成框架，能提升VLMs的通用视觉语言推理能力
...
TL;DR: OpenQlaw是一个用于分析二维量子材料的智能编排系统，通过将核心LLM代理与专业MLLM节点解耦，实现视觉识别、推理和图像渲染的分离，为实验室提供自然交互和持久记忆功能。


<details>
  <summary>Details</summary>
Motivation: 现有领域特定的多模态大语言模型虽然能基于物理知识进行推理，但其输出过于冗长且认知透明度高，导致认知过载，缺乏对研究人员实际交互的即时实用性，需要更有效的系统来支持二维量子材料的实际器件制造。

Method: 基于NanoBot轻量级代理框架和QuPAINT物理感知多模态平台构建OpenQlaw系统，核心LLM代理编排领域专家MLLM节点，解耦视觉识别与推理，支持空间数据解析、尺度感知物理计算、视觉标注生成，并具备持久记忆存储物理尺度比例和样品制备方法。

Result: 系统能够动态处理用户查询，以自然方式回答，支持尺度感知物理计算和隔离视觉标注，通过持久记忆保存物理尺度比例和样品制备方法，将孤立推理转变为上下文感知助手，加速高通量器件制造。

Conclusion: OpenQlaw通过代理架构和核心代理作为领域专家编排器的扩展，解决了现有MLLM在二维量子材料分析中的认知过载问题，实现了从视觉识别到实际器件制造的动态推理，为实验室提供了可访问的智能助手。
...
Method: 1. 基于负感知微调的前向过程RL公式：通过对比推理端点的正负样本建立隐式策略改进方向，无需反向过程展开；2. 流式训练方案：通过滚动KV缓存逐步生成序列，仅在局部剪辑窗口应用RL更新，同时基于先前上下文确保长程一致性；3. 多奖励目标：通过不确定性感知选择性正则化和动态参考更新稳定化，缓解奖励黑客问题。

Result: 大量实验表明，该方法在多个蒸馏自回归视频模型上持续提升生成质量，成为一个稳健且可扩展的对齐解决方案。

Conclusion: Astrolabe为蒸馏自回归视频模型提供了一个高效、可扩展的在线强化学习框架，解决了现有方法的计算瓶颈和对齐问题，显著提升了生成质量。
...
TL;DR: PaAgent是一种具有自我进化画像库的图像修复智能体，通过检索增强生成技术选择最佳修复工具，并采用主客观强化学习策略提升复杂场景下的退化感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有图像修复智能体缺乏对过往交互的洞察总结机制，导致需要穷举搜索最优修复工具，效率低下。为了解决这一问题，需要开发能够总结工具特性并智能选择修复工具的系统。

Method: 1) 构建自我进化的画像库，通过总结修复图像、选择的修复工具和退化图像的特征来丰富库内容；2) 采用检索增强生成技术从画像库中检索相关洞察以选择最优修复工具；3) 提出主客观强化学习策略，结合图像质量分数和语义洞察生成奖励，提升复杂场景下的退化感知能力。

Result: 在8个图像修复基准测试中进行了广泛实验，涵盖6种单一退化和8种混合退化场景，验证了PaAgent在处理复杂图像修复任务方面的优越性。

Conclusion: PaAgent通过自我进化的画像库和检索增强生成技术，有效解决了现有图像修复智能体缺乏洞察总结机制的问题，在复杂退化场景下表现出色，为自动化图像修复任务提供了更高效的解决方案。
...
TL;DR: DesertFormer：基于SegFormer B2的沙漠地形语义分割模型，针对低对比度、极端光照的沙漠环境，实现10类地形分类，mIoU达64.4%，比基线提升24.2%


<details>
  <summary>Details</summary>
Motivation: 沙漠地形感知对自动驾驶在非结构化环境中的导航至关重要，但沙漠环境存在低色度对比、极端光照变化和稀疏植被等独特挑战，传统道路场景分割模型难以应对

Method: 基于SegFormer B2架构，采用分层Mix Transformer（MiT-B2）骨干网络，构建沙漠地形语义分割管道，使用4,176张512x512标注图像训练，包含10个生态类别

Result: 在自建数据集上达到64.4% mIoU和86.1%像素精度，比DeepLabV3 MobileNetV2基线（41.0% mIoU）提升24.2%；识别主要混淆模式：地面杂物到景观、干草到景观

Conclusion: DesertFormer能有效分割沙漠地形，支持安全路径规划；通过类别加权训练和复制粘贴增强改善稀有类别识别；开源代码、检查点和交互式推理仪表板
...
TL;DR: 提出TrackDeform3D框架，使用RGB-D相机低成本自主收集可变形物体3D数据集，包含关键点识别与轨迹跟踪，并发布大规模高质量数据集


<details>
  <summary>Details</summary>
Motivation: 当前可变形物体的结构化3D表示（如关键点、网格）提取困难，现有感知方法难以处理复杂变形，且大规模3D数据收集成本高、依赖简化假设，导致相关数据集稀缺

Method: 提出基于RGB-D相机的低成本自主数据收集框架，识别3D关键点并鲁棒跟踪其轨迹，引入运动一致性约束以生成时间平滑、几何一致的数据

Result: TrackDeform3D在多种物体类别上优于现有跟踪方法，在几何和跟踪精度上均有提升，并创建了包含6个可变形物体、总计110分钟轨迹数据的大规模高质量数据集

Conclusion: 该框架解决了可变形物体3D数据收集的瓶颈问题，为下游任务（如动力学建模和运动规划）提供了高质量的数据基础
...
TL;DR: 提出一种用于湿滑浴室环境中老年人跌倒检测的双流神经网络架构，结合毫米波雷达和地板振动传感器，通过交叉条件融合机制提高检测精度并降低边缘设备延迟和能耗。


<details>
  <summary>Details</summary>
Motivation: 现有基于毫米波雷达或振动传感器的跌倒检测方案通常将运动和冲击视为松散耦合的流，依赖粗略的时间对齐和幅度阈值，未能明确编码雷达观测的摔倒与地板冲击之间的因果关系，也无法有效处理时间漂移、物体掉落干扰以及低功耗边缘设备的延迟和能耗限制。

Method: 提出双流架构：1）Motion-Mamba分支处理毫米波雷达信号，捕捉长距离运动模式；2）Impact-Griffin分支处理地板振动，强调冲击瞬态和跨轴耦合。采用低秩双线性交互和Switch-MoE头进行交叉条件融合，对齐运动和冲击标记并抑制物体掉落干扰。模型设计适合在Raspberry Pi 4B网关上实时执行。

Result: 在构建的浴室跌倒检测基准数据集上（包含超过3小时的同步毫米波雷达和三轴振动记录），模型在测试集上达到96.1%准确率、94.8%精确率、88.0%召回率、91.1%宏F1分数和0.968 AUC。相比最强基线，准确率提高2.0个百分点，跌倒召回率提高1.3个百分点，延迟从35.9ms降至15.8ms，每2.56秒窗口能耗从14200mJ降至10750mJ。

Conclusion: 所提出的双流架构通过明确编码运动与冲击之间的因果关系，有效解决了现有方案在湿滑浴室环境中的局限性，在保持高检测性能的同时显著降低了边缘设备的延迟和能耗，为老年人独立生活提供了更可靠的隐私保护式跌倒检测方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学视觉语言模型存在专业化与泛化性的两难困境：专用模型在单领域数据上训练能捕捉领域细节但泛化能力差，通用模型在多领域数据上训练保留广泛语义但稀释了细粒度诊断线索。需要解决这种专业化-泛化性权衡问题。

Method: 提出ACE-LoRA框架：1）在冻结的图像-文本编码器中集成低秩适配（LoRA）模块；2）引入注意力上下文增强超图神经网络（ACE-HGNN）模块，捕捉超越成对相似性的高阶上下文交互，用局部诊断线索丰富全局表示；3）设计标签引导的InfoNCE损失，有效抑制语义相关图像-文本对之间的假阴性。

Result: 仅增加0.95M可训练参数，ACE-LoRA在跨多个领域的零样本分类、分割和检测基准上，持续优于最先进的医学视觉语言模型和参数高效微调基线方法。

Conclusion: ACE-LoRA成功解决了医学视觉语言模型中专业化与泛化性的权衡问题，通过参数高效的适配框架在保持强大零样本泛化能力的同时，显著提升了细粒度诊断线索的捕捉能力。
...
TL;DR: 提出Gaussian-Hermite采样方法，通过高斯-埃尔米特多项式实现平移一致的采样，使CNN层在训练前就能保持对任意空间平移的不变性，显著提升平移一致性至100%。


<details>
  <summary>Details</summary>
Motivation: 传统CNN中的下采样操作会破坏平移不变性，但下采样对于计算效率和扩大感受野获取上下文信息又很重要。因此需要一种既能保持平移不变性又能进行有效下采样的方法。

Method: 提出Gaussian-Hermite采样方法，利用高斯-埃尔米特多项式进行平移一致的采样。该方法可以直接嵌入标准CNN架构中，无需修改网络结构或增加额外训练过程。

Result: 在CIFAR-10、CIFAR-100和MNIST-rot数据集上的实验表明，GHS方法显著提升了平移一致性，在空间平移下实现了100%的分类一致性，同时相比基线CNN模型还提高了分类准确率。

Conclusion: Gaussian-Hermite采样是一种有效的下采样策略，能够在保持CNN计算效率和感受野优势的同时，实现准确的平移不变性，为构建更鲁棒的CNN模型提供了新思路。
...
TL;DR: FloodLlama：基于TikTok数据微调的开源视觉语言模型，用于从单张街景图像实现厘米级洪水深度估计，为电动汽车和自动驾驶车辆提供实时洪水感知解决方案。


<details>
  <summary>Details</summary>
Motivation: 城市洪水对交通网络连续性构成日益严重的威胁，但目前没有运营系统能够提供厘米级分辨率的实时街级洪水深度信息，这对于动态路径规划、电动汽车安全和自动驾驶操作至关重要。

Method: 使用约19万张合成图像数据集，涵盖7种车辆类型、4种天气条件和41个深度级别（0-40厘米，1厘米分辨率）。通过渐进式课程训练实现从粗到细的学习，使用QLoRA微调LLaMA 3.2-11B Vision模型。建立TikTok多模态感知管道进行实时数据收集。

Result: FloodLlama在34,797次试验中表现优异：平均绝对误差低于0.97厘米，深度洪水时Acc@5cm超过93.7%，浅水时超过96.8%。五阶段机制可解释性框架识别出L23层为关键深度编码转换层，选择性微调减少76-80%可训练参数。Tier 3配置在真实数据上达到98.62%准确率，对视觉遮挡具有强鲁棒性。TikTok管道在底特律676个标注洪水帧上验证了可行性。
...
<details>
  <summary>Details</summary>
Motivation: 图像分割需要大量标注数据，成本高昂且耗时。AI生成的银标准标签易获取但有偏差风险。自监督学习仅需图像，但现有结合对比学习与反事实生成的方法难以扩展到像素级任务。

Method: 提出结合反事实生成与密集对比学习的流水线：双视图(DVD-CL)和多视图(MVD-CL)方法，以及利用银标准标注的监督变体。同时引入新的可视化算法CHRO-map。

Result: 无标注的DVD-CL优于其他密集对比学习方法；使用银标准标签的监督变体优于直接在银标准标注数据上训练，在挑战性数据上达到约94% DSC。

Conclusion: 像素级对比学习通过反事实生成和银标准标注增强，能提高对采集和病理变化的鲁棒性，为图像分割提供有效的无标注/弱标注解决方案。
...
TL;DR: 研究发现视觉语言模型（VLM）集成时存在家族相关错误，提出三种家族感知方法提升集成性能


<details>
  <summary>Details</summary>
Motivation: 现有VLM集成方法忽略同一架构家族模型间的相关错误，导致集成效果受限，特别是在误导性问题上准确率可能降至0%

Method: 提出三种家族感知方法：1）分层家族投票（HFV）；2）基于校准、家族质量和逆家族大小的无训练方法（QualRCCV）；3）学习候选答案评分（LCS），使用支持广度、家族多样性和模型质量特征

Result: HFV在误导性问题上恢复18-26个百分点准确率；QualRCCV首次在所有三个基准上超越校准投票；LCS获得最大提升：VQAv2 +0.68%、TextVQA +0.61%、GQA +2.45%，且从不降低任何基准性能

Conclusion: VLM集成必须考虑家族相关性，提出的家族感知方法能显著提升集成性能，LCS方法在VQAv2测试集上达到87.83%准确率，验证了泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 视频扩散模型正从生成短片段发展为世界模拟器，需要保持相机运动、重访和干预下的一致性。当前方法存在瓶颈：显式3D结构能改善重投影一致性但难以描绘运动物体，而隐式记忆即使有正确姿态也常产生不准确的相机运动。

Method: 提出MosaicMem混合空间记忆系统，将图像块提升到3D空间实现可靠定位和针对性检索，通过patch-and-compose接口在查询视图中组合空间对齐的图像块，保留应持久的内容同时让模型填充应演化的内容。结合PRoPE相机条件和两种新的记忆对齐方法。
...
TL;DR: SMAL-pets是一个从单张图像生成高质量、可编辑3D狗化身的框架，结合3D高斯泼溅和SMAL参数模型，支持通过文本提示进行外观编辑和动画控制。


<details>
  <summary>Details</summary>
Motivation: 创建高保真、可动画的3D狗化身面临多重挑战：缺乏大规模标注数据集、物种形态多样性大、现有方法难以捕捉真实毛发纹理，以及需要大量人工操作进行网格编辑和绑定。

Method: 采用混合架构，将3D高斯泼溅与SMAL参数模型集成，提供既视觉高保真又解剖学基础的表征。引入多模态编辑套件，允许用户通过文本提示精炼化身外观和执行复杂动画。

Result: 开发出SMAL-pets框架，能够从单张输入图像生成高质量、可编辑的动物化身，实现通过自然语言控制模型的美学和行为方面。

Conclusion: SMAL-pets为动画和虚拟现实提供了一个灵活、强大的工具，弥合了重建和生成建模之间的差距，实现了通过自然语言进行外观编辑和动画控制。
...
<details>
  <summary>Details</summary>
Motivation: 现有场景无关的定位方法可能无法充分利用场景特定特征，需要一种能够发现并利用场景特定模式作为地标的定位方法，以提高在各种环境中的定位鲁棒性。

Method: 提出BEV-SLD方法：1) 使用鸟瞰图(BEV)图像发现场景特定模式；2) 在预定空间密度下将这些模式作为地标；3) 通过一致性损失对齐可学习的全局地标坐标与每帧热图；4) 自监督学习确保跨场景的地标检测一致性。

Result: 在校园、工业和森林等多种环境中，BEV-SLD实现了鲁棒的定位性能，与最先进方法相比表现出色。

Conclusion: BEV-SLD通过自监督学习场景特定地标，提供了一种有效的LiDAR全局定位解决方案，在各种复杂环境中都表现出良好的鲁棒性和性能。
...
TL;DR: GazeOnce360：首个从单台向上鱼眼相机实现360°多人视线估计的端到端模型，结合MPSGaze360合成数据集和双分辨率架构


<details>
  <summary>Details</summary>
Motivation: 传统视线估计方法依赖前向相机且视角受限，无法处理360°场景中的多人视线估计。向上鱼眼相机能覆盖更广视野，但存在严重畸变和视角变化问题，需要专门解决方案。

Method: 1) 提出GazeOnce360端到端模型；2) 引入MPSGaze360大规模合成数据集，使用Unreal Engine渲染，包含多样多人配置和精确3D视线/眼标点标注；3) 采用旋转卷积处理鱼眼畸变；4) 眼标点监督；5) 双分辨率架构融合全局低分辨率上下文和高分辨率局部眼部特征。

Result: 实验证明模型各组件有效，展示了鱼眼相机在360°多人视线估计场景中的可行性和潜力。

Conclusion: 该工作证明了基于鱼眼相机的360°多人视线估计在实际场景中的可行性，为多人在场交互应用提供了新解决方案。
...
TL;DR: MLLMs结合专家知识可在隐私约束下实现高性能虹膜防伪检测，超越专业CNN基线和人工检查


<details>
  <summary>Details</summary>
Motivation: 虹膜防伪检测面临三大挑战：1)无法收集未来未知攻击数据；2)收集足够多样本成本高昂；3)生物识别数据共享存在隐私问题。需要能在隐私约束下适应新攻击的解决方案。

Method: 使用通用多模态大语言模型进行虹膜防伪检测：1)分析MLLM视觉编码器嵌入，发现预训练视觉变换器能自然聚类多种攻击类型；2)当聚类重叠时，使用包含人类显著性（受试者描述攻击指标）的结构化提示解决歧义；3)在224张虹膜图像数据集上测试，使用大学批准服务或本地部署模型。

Result: 1) Gemini 2.5 Pro结合专家提示优于专业CNN基线和人工检查员；2)本地可部署的Llama 3.2-Vision达到接近人工的性能水平；3)预训练视觉变换器能自然聚类多种攻击类型，无需专门训练。

Conclusion: 在机构隐私约束下可部署的MLLMs为虹膜防伪检测提供了可行路径，能够结合专家知识解决未知攻击检测问题，同时保护生物识别数据隐私。
...
TL;DR: Patient4D：一种利用静止先验的4D人体网格重建方法，专门针对手术AR场景中患者被遮盖且相机移动的情况，通过姿态锁定和刚性回退机制提升重建鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在手术增强现实（AR）场景中，麻醉患者被手术遮盖物遮挡，同时外科医生的头戴式相机不断改变视角，现有的人体网格恢复（HMR）方法在这种条件下性能下降，因为它们通常在直立、移动的主体和相对稳定的相机视角下训练。

Method: 提出Patient4D重建流程，结合图像级基础模型感知和轻量级几何机制来强制执行跨帧的时间一致性。关键组件包括：1）姿态锁定：使用稳定关键帧锚定姿态参数；2）刚性回退：通过轮廓引导的刚性对齐在严重遮挡下恢复网格。

Result: 在4,680个合成手术序列和三个公共HMR视频基准上评估。在手术遮盖物遮挡下，Patient4D达到0.75的平均IoU，将失败帧从30.5%减少到1.3%（相比最佳基线）。

Conclusion: 利用静止先验可以显著改善临床AR场景中的单目重建，Patient4D的机制稳定预测同时保持与现成HMR模型的兼容性。
...
TL;DR: 该论文提出了一个针对工业应用中实例级图像检索的视觉嵌入模型基准测试，评估了开源基础模型、专有多模态系统和领域特定模型在统一检索协议下的表现。


<details>
  <summary>Details</summary>
Motivation: 工业应用中可靠的产品识别至关重要，错误的匹配会导致昂贵的下游故障。需要评估现代视觉嵌入模型在实际工业场景中的表现，为从业者和研究者提供指导。

Method: 使用统一图像到图像检索协议，评估开源基础嵌入模型、专有多模态嵌入系统和领域特定视觉模型。基准测试包括从制造、汽车、DIY和零售等生产部署中提取的工业数据集以及公共基准，评估时不进行后处理以隔离检索能力。

Result: 基准测试结果揭示了当代基础模型和统一嵌入模型在细粒度实例检索任务中的迁移能力，以及与专门为工业应用训练的模型的比较表现。

Conclusion: 该基准测试通过强调实际约束、异构图像条件和精确实例匹配要求，为从业者和研究者提供了当前视觉嵌入方法在生产级产品识别系统中的优势和局限性信息。
...
TL;DR: 提出SA-CycleGAN-2.5D框架，通过2.5D三平面流形注入、全局自注意力生成器和谱归一化判别器，解决多站点神经影像分析中的扫描仪引起的协变量偏移问题，显著降低域间差异并保持肿瘤病理生理学特征。
...
Method: 提出SA-CycleGAN-2.5D域适应框架：1) 2.5D三平面流形注入，以O(HW)复杂度保持平面间梯度；2) U-ResNet生成器结合密集体素到体素自注意力，超越CNN的O(√L)感受野限制；3) 谱归一化判别器约束Lipschitz常数(K_D≤1)实现稳定对抗优化。

Result: 在654名胶质瘤患者（BraTS和UPenn-GBM两个机构域）上评估，最大均值差异(MMD)降低99.1%(1.729→0.015)，域分类器准确率降至接近随机水平(59.7%)。消融实验证实全局注意力对异质到同质转换方向具有统计显著性(Cohen's d=1.32, p<0.001)。

Conclusion: 通过结合2D效率和3D一致性，该框架生成保持肿瘤病理生理学的体素级协调图像，支持可重复的多中心影像组学分析，有效解决扫描仪引起的域偏移问题。
...
Method: 提出EGS（高效高斯流式处理），一个基于强化学习的插件式锚点采样器，联合选择锚点预算和信息丰富的锚点子集，利用高斯表示的空间特征平衡重建质量和运行时间。

Result: 在动态多视角数据集上，EGS在质量-效率权衡方面优于FPS采样。在快速渲染模式下（256锚点，比8192少32倍），PSNR提升0.52-0.61dB，运行速度比IGS@8192快1.29-1.35倍。

Conclusion: EGS通过强化学习策略实现了预算感知的锚点选择，在显著减少锚点预算的同时保持重建质量，为动态高斯溅射重建提供了更高效的计算资源分配方案。
...
TL;DR: 研究通过层间线性探测和注意力干预分析，揭示了多模态大语言模型在分割任务中的内在空间理解机制，发现适配器导致表示退化，但LLM层通过注意力机制逐步恢复。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型越来越多地应用于像素级视觉任务，但其内在的空间理解能力仍然未被充分理解。研究者希望探究MLLM在分割任务中的工作机制。

Method: 采用层间线性探测评估整个MLLM流程（视觉编码器、适配器、LLM），进行基于干预的注意力敲除分析测试跨令牌注意力是否逐步细化视觉表示，并评估图像令牌间双向注意力对空间一致性的影响。

Result: 分析发现：适配器引入分割表示退化，但LLM层通过注意力介导的细化逐步恢复，其中正确分类的令牌引导错误分类的邻居走向正确标签。在早期图像令牌位置，这种恢复受因果注意力限制，而图像令牌间的双向注意力可以缓解此问题。

Conclusion: 这些发现提供了MLLM如何处理视觉信息进行分割的机制性解释，为未来设计具有分割能力的模型提供了信息。
...
TL;DR: GigaWorld-Policy是一种基于世界-动作模型的机器人策略学习方法，通过解耦动作预测和视频生成，实现了9倍加速和7%成功率提升


<details>
  <summary>Details</summary>
Motivation: 现有世界-动作模型存在两个关键瓶颈：1) 联合推理视觉动态和动作导致推理开销大；2) 联合建模使视觉和运动表示纠缠，动作预测准确性过度依赖视频生成质量

Method: 提出动作中心的WAM，将策略训练分为两个耦合组件：基于当前观测预测未来动作序列，同时基于预测动作和相同观测生成未来视频。采用因果设计防止未来视频令牌影响动作令牌，使视频生成在推理时可选

Result: 在真实机器人平台上，GigaWorld-Policy比领先的WAM基线Motus运行速度快9倍，任务成功率提高7%；相比pi-0.5，在RoboTwin 2.0上性能提升95%

Conclusion: GigaWorld-Policy通过解耦动作预测和视频生成，实现了高效的动作中心世界-动作模型，在保持性能的同时显著提升推理速度
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型和多模态模型在文档布局分析方面有所改进，但区域合并、分割和遗漏等结构错误仍然存在。传统的基于重叠的指标（如IoU、mAP）无法捕捉这些逻辑不一致性。
...
Method: 提出实用的多阶段过滤流程，整合两阶段模型辅助筛选、研究人员整理和专家验证，构建ConfusionBench基准，包含平衡的困惑识别数据集和视频定位数据集。
...
TL;DR: DANCE：面向3D CNN的细粒度、输入感知动态剪枝框架，通过激活变异性放大和自适应激活剪枝实现显著的计算和内存节省，硬件验证显示1.37-2.22倍加速和1.47倍能效提升。


<details>
  <summary>Details</summary>
Motivation: 现代CNN在处理视频和图像时无法根据输入样本的计算复杂度进行动态调整以最小化能耗，需要一种能够保持性能的同时最大化能效的动态剪枝方法。

Method: 提出两阶段方法：1) 激活变异性放大(AVA)：重新训练3D CNN模型以增加神经元激活幅度的方差；2) 自适应激活剪枝(AAP)：训练轻量级激活控制器网络，基于第一层输出统计动态剪枝帧、通道和特征。

Result: 在NVIDIA Jetson Nano GPU和Qualcomm Snapdragon 8 Gen 1平台上分别实现1.37倍和2.22倍加速，相比现有技术达到最高1.47倍的能效提升，同时保持性能几乎不受影响。

Conclusion: DANCE框架通过细粒度、输入感知的动态剪枝，在保持3D CNN性能的同时显著提高了计算效率和能效，为边缘设备上的视频处理提供了有效的节能解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有故事可视化方法在角色身份一致性和视觉风格一致性方面存在不足，特别是在复杂交互和长叙事中容易出现身份漂移问题。需要一种能保持严格一致性的解决方案。

Method: 提出两阶段框架：1）Group-Shared Attention机制，在注意力层实现无损跨样本信息流，结构化编码跨帧身份对应关系；2）Direct Preference Optimization，通过偏好数据同时提升视觉保真度和身份保持，避免冲突的辅助损失函数。

Result: 在ViStoryBench基准测试中取得新的SOTA效果，显著超越基线方法：角色身份一致性（CIDS）提升+10.0，风格一致性（CSD）提升+18.7，同时保持高保真生成质量。

Conclusion: 提出的两阶段框架通过GSA机制和DPO优化，有效解决了故事可视化中的一致性问题，为生成语义对齐且严格一致的故事图像提供了有效方案。
...
TL;DR: 提出了一种用于阿尔茨海默病分类的多模态3D卷积神经网络，使用原始OASIS 1 MRI体积数据，结合T1结构信息和组织概率图，在受试者级别交叉验证中达到72.34%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有研究多分析从MRI体积中提取的单个2D切片，而临床神经影像实践通常依赖完整的三维脑结构。体积分析可能更好地捕捉与疾病进展相关的脑区空间关系。

Method: 提出多模态3D卷积神经网络，结合原始T1 MRI体积和通过FSL FAST分割获得的灰质、白质、脑脊液概率图，使用5折受试者级别交叉验证进行评估。

Result: 在OASIS 1队列上获得72.34%±4.66%的平均准确率和0.7781±0.0365的ROC AUC。GradCAM可视化显示模型关注内侧颞叶和脑室等与阿尔茨海默病相关的解剖区域。

Conclusion: 提出的多模态3D框架建立了可重复的受试者级别基准，突出了体积MRI分析在阿尔茨海默病分类中的潜在优势，为理解数据表示和评估策略对性能的影响提供了背景。
...
<details>
  <summary>Details</summary>
Motivation: 现有的MLLM智能体在长视频理解任务中面临挑战，这些任务具有高信息密度和长时间跨度特点。简单的任务分解和协作机制不足，而基于嵌入的检索方法可能会丢失复杂问题的关键信息。

Method: Symphony采用多智能体系统，模拟人类认知模式：1）将长视频理解分解为细粒度子任务；2）引入基于反思增强的深度推理协作机制；3）提供基于VLM的定位方法分析任务并评估视频片段相关性。

Result: 在LVBench、LongVideoBench、VideoMME和MLVU等基准测试中达到最先进性能，在LVBench上比先前最优方法提升5.0%。

Conclusion: Symphony通过模拟人类认知的多智能体协作方法，有效解决了长视频理解中的信息密度高和时间跨度大的挑战，显著提升了推理能力和复杂问题定位能力。
...
Method: 设计循环推理框架，迭代处理局部视频片段，通过演化思维链维护全局上下文，记录任务分解、关键步骤及其完成状态，从而推理复杂时间依赖关系。

Result: 在ALFRED和Ego4D自动生成的大规模数据集上训练，在进度估计及相关下游应用（进度增强策略学习、强化学习奖励建模、主动辅助）中表现优异，达到长时程任务进度估计的新SOTA。

Conclusion: R²VLM通过循环推理和思维链机制，在避免长视频处理高成本的同时保留关键推理能力，实现了高效且泛化性强的长时程任务进度估计。
...
TL;DR: 提出QGN模型解决GMNER任务中预训练通用目标检测器与文本实体不匹配的问题，通过文本引导和跨模态交互实现统一的多模态推理和解码。


<details>
  <summary>Details</summary>
Motivation: 现有GMNER方法通常分为两步：先用预训练通用目标检测器检测物体，再将命名实体与检测到的物体匹配。这种方法存在主要限制：通用目标检测器独立于文本实体运行，倾向于检测常见物体而忽略命名实体所需的特定细粒度区域，导致不精确并损害系统性能。

Method: 提出proposal-free Query-Guided Network (QGN)，通过文本引导和跨模态交互统一多模态推理和解码。QGN无需物体检测建议框，直接通过查询引导实现准确的实体定位。

Result: 在广泛使用的基准测试上，QGN在比较的GMNER模型中取得了最佳性能，在开放域场景中实现了准确的定位和鲁棒的性能。

Conclusion: QGN通过统一的多模态推理框架解决了传统两阶段方法中检测器与文本实体不匹配的问题，在GMNER任务上表现出色，为开放域场景提供了有效的解决方案。
...
TL;DR: MedSAD-CLIP：一种基于CLIP的医学异常检测与分割方法，通过Token-Patch交叉注意力和轻量适配器，在有限标注数据下实现精细病灶定位，在多个医学数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP方法在医学异常检测中主要依赖全局表示和弱监督，导致定位粗糙、分割质量有限。本研究探索在现实临床场景下（有限但有意义的标注异常数据可用），如何通过监督方式有效适配CLIP以提升医学异常检测性能。

Method: 1. 提出Token-Patch交叉注意力机制，利用细粒度文本-视觉线索改善病灶定位；2. 使用轻量图像适配器和可学习提示词，在保持CLIP丰富语义对齐的同时适应医学领域；3. 设计基于边界的图像-文本对比损失，增强正常与异常全局特征区分度。

Result: 在脑部、视网膜、肺部和乳腺四个不同医学数据集上的实验表明，MedSAD-CLIP在像素级分割和图像级分类任务上均优于现有最先进方法，展示了监督CLIP适配作为统一可扩展医学异常理解范式的潜力。

Conclusion: MedSAD-CLIP通过细粒度文本-视觉交互和高效领域适配，显著提升了医学异常检测与分割性能，证明了监督CLIP适配在医学图像分析中的有效性和实用性。
...
TL;DR: FineViT是一种新型视觉编码器，通过渐进式训练策略解决MLLMs中视觉编码器的性能瓶颈问题，使用密集重标注数据和局部感知增强来提升细粒度视觉感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs中的CLIP-based视觉编码器存在性能瓶颈，主要问题包括：低分辨率预训练导致视觉细节丢失，以及依赖噪声大、粒度粗的网络爬取图像-文本对。这些限制影响了密集空间任务的性能。

Method: 提出FineViT视觉编码器，采用渐进式训练策略：1）在高原生分辨率下，使用数十亿个全局重标注图像-文本对从头训练编码器，建立细节丰富的语义基础；2）通过LLM对齐，利用FineCap-450M数据集（包含4.5亿高质量局部标注）进一步增强局部感知能力。

Result: FineViT在零样本识别和检索任务中达到最先进性能，尤其在长上下文检索方面表现突出。当集成到MLLMs中时，持续优于SigLIP2和Qwen-ViT等多模态视觉编码器。

Conclusion: FineViT通过渐进式训练策略有效解决了视觉编码器的细粒度感知瓶颈，有望成为细粒度视觉感知的强大新基准，为MLLMs提供更优质的视觉编码能力。
...
TL;DR: EvoGuard是一个用于AI生成图像检测的智能代理框架，通过动态编排多种现成检测器，利用自主规划和反思能力实现多轮推理，无需细粒度标注即可达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: AI生成图像的快速扩散带来了严重的信息误传风险，现有检测方法要么依赖低层特征泛化能力有限，要么使用MLLM但面临扩展性差和标注成本高的问题，需要适应复杂动态真实环境的解决方案。

Method: 提出EvoGuard代理框架：1) 封装多种SOTA MLLM和非MLLM检测器作为可调用工具；2) 通过能力感知的动态编排机制协调这些工具；3) 利用代理的自主规划和反思能力智能选择工具、反思中间结果并决定下一步行动；4) 使用基于GRPO的代理强化学习算法优化，仅需低成本二元标签。

Result: 实验表明EvoGuard达到SOTA准确率，同时缓解了正负样本间的偏差。更重要的是，它支持即插即用集成新检测器，无需训练即可提升整体性能，为持续演变的AIGI威胁提供了高度实用的长期解决方案。

Conclusion: EvoGuard通过智能代理框架有效利用异构检测器的互补优势，超越了单一模型的限制，消除了对细粒度标注的依赖，为AI生成图像检测提供了一个可扩展、实用且长期有效的解决方案。
...
TL;DR: OnlineHMR是一个完全在线的3D人体网格恢复框架，支持实时处理单目视频，满足因果性、忠实性、时间一致性和效率四个在线处理标准。
...
Method: 采用双分支架构，通过因果键值缓存设计和滑动窗口学习策略实现流式推理；结合以人为中心的增量SLAM，在物理合理的轨迹校正下提供在线世界坐标系对齐。

Result: 在标准EMDB基准测试和高动态自定义视频上，性能与现有的基于分块的方法相当，同时独特地支持在线处理。

Conclusion: OnlineHMR首次实现了完全在线的人体网格恢复，满足了实时应用的需求，为AR/VR和远程呈现等交互场景提供了可行的解决方案。
...
TL;DR: 提出新的资产检查3D重建数据集，包含合成场景的深度图、相机位姿和网格模型，评估现有方法在密集采集轨迹和复杂表面条件下的表现


<details>
  <summary>Details</summary>
Motivation: 资产检查需要高保真3D模型，但现有重建数据集缺乏密集采集轨迹、毫米级细节和非朗伯表面等实际检查条件，难以评估方法性能

Method: 创建包含三个合成场景的新数据集，具有模拟检查轨迹、地面真实深度图、相机位姿和网格模型，包含不同表面条件和非朗伯内容

Result: 评估显示当前最先进方法在密集采集轨迹和复杂表面条件下表现显著不佳，暴露了可扩展性差距

Conclusion: 现有3D重建方法难以满足资产检查的实际需求，需要新的研究方向来实现可部署的资产检查3D重建
...
<details>
  <summary>Details</summary>
Motivation: 现有组合图像检索方法难以从参考图像中提取与用户意图最相关的语义线索，容易受到无关视觉噪声的干扰

Method: 利用多模态大语言模型进行思维链推理，生成保留、移除和目标推断文本；设计两个参考视觉注意力选择模块，提取补丁级和实例级语义；采用加权分层组合模块融合多粒度视觉线索与修改文本

Result: 在CIRR和FashionIQ两个基准测试中均优于现有方法，达到新的最先进性能

Conclusion: MCoT-MVS通过多模态思维链推理指导的多级视觉选择，有效提升了组合图像检索的准确性和鲁棒性
...
TL;DR: 提出Material Magic Wand工具，通过材质感知嵌入自动识别3D网格中共享相同材质的重复部件，简化材质分配工作流程。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的3D形状（如松果鳞片、建筑窗户）常包含几何变化但材质相同的重复结构，手动逐个选择这些部件进行材质分配既繁琐又耗时。

Method: 提出材质感知部件编码器，为每个3D部件生成同时考虑局部几何和全局上下文的嵌入表示，使用监督对比损失训练模型，使相同材质的部件嵌入更接近，不同材质的嵌入更分离。

Result: 构建了包含100个形状和241个部件级查询的基准数据集，通过大量实验验证了方法的有效性，并在交互式材质分配应用中展示了实用价值。

Conclusion: Material Magic Wand工具能够有效识别3D网格中共享材质的部件组，显著简化了材质分配工作流程，为3D艺术家提供了实用的自动化工具。
...
TL;DR: 该研究发现视觉语言模型（VLMs）的安全对齐在视觉模态整合后减弱，图像能显著提高越狱成功率。研究识别出越狱方向，并提出通过移除越狱相关偏移（JRS-Rem）的防御方法。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在整合视觉模态后安全对齐能力减弱，即使文本提示包含明确有害意图，添加图像也能大幅提高越狱成功率。需要理解这种现象的内在机制并开发有效的防御方法。

Method: 研究发现VLMs在表示空间中能清楚区分良性输入和有害输入，且越狱样本形成与拒绝样本可分离的独特内部状态。研究识别出越狱方向，定义越狱相关偏移为图像诱导表示偏移沿该方向的分量。提出JRS-Rem防御方法，在推理时移除越狱相关偏移。

Result: 越狱相关偏移能可靠地表征越狱行为，为不同越狱场景提供统一解释。JRS-Rem防御方法在多个场景中提供强大防御，同时在良性任务上保持性能。

Conclusion: 视觉模态将表示向特定越狱状态偏移，导致未能触发拒绝机制。通过移除越狱相关偏移的防御方法能有效增强VLM安全性，同时保持正常功能。
...
TL;DR: InfoShot：一种面向长视频理解的任务无关、镜头感知帧采样方法，通过信息论目标选择互补关键帧，平衡视频覆盖与关键事件捕捉
...
Method: 1) 将视频分割成语义一致的镜头；2) 从每个镜头中选择两个互补关键帧：一个代表主要内容，一个捕捉镜头内的异常变化；3) 基于信息论目标优化采样集

Result: 在帧数限制下，InfoShot提高了异常检测命中率和下游视频问答准确性，同时在标准视频理解基准测试中匹配或优于强基线方法

Conclusion: InfoShot通过镜头感知的帧采样策略，有效平衡了长视频的整体上下文和决策关键时刻，无需重新训练即可提升视频理解性能
...
TL;DR: StereoWorld是一个相机条件化的立体世界模型，能够端到端生成立体视频，同时学习外观和双目几何，无需深度估计或修复，相比单目转换方法在立体一致性、视差精度和相机运动保真度方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么基于单目RGB或RGBD，要么需要将单目生成结果转换为立体格式，这会导致立体不一致、视差不准确和相机运动保真度低的问题。需要一种能够直接在RGB模态中学习双目几何的端到端立体视频生成方法。

Method: 1. 统一的相机帧RoPE：通过相机感知的旋转位置编码增强潜在标记，实现相对、视图和时间一致的相机条件化，同时通过稳定的注意力初始化保留预训练视频先验。2. 立体感知注意力分解：将完整的4D注意力分解为3D视图内注意力加水平行注意力，利用极线先验以更低的计算成本捕捉视差对齐的对应关系。

Result: 在基准测试中，StereoWorld相比强大的单目转换流程，在立体一致性、视差精度和相机运动保真度方面都有提升，生成速度提高3倍以上，视角一致性额外提升5%。

Conclusion: StereoWorld实现了无需深度估计或修复的端到端双目VR渲染，通过度量尺度深度接地增强了具身策略学习，并且兼容长视频蒸馏以实现扩展的交互式立体合成。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶新视角合成存在监督鸿沟：模型需要在推理时合成未见过的视角，但训练时缺乏这些偏移姿态的真实图像作为监督信号。

Method: 1. 提出虚拟位移策略：使用单目深度代理模拟遮挡模式并映射到原始视图，将视图合成重构为自监督修复任务；2. 伪3D接缝合成策略：整合相邻相机视觉数据，显式建模真实世界光度差异和标定误差。

Result: VisionNVS相比依赖激光雷达的基线方法，在几何保真度和视觉质量上表现更优，为可扩展的驾驶模拟提供了鲁棒解决方案。

Conclusion: 通过将视图合成重构为自监督修复任务，VisionNVS消除了传统方法中的领域鸿沟，仅使用相机数据就能实现高质量的新视角合成，为自动驾驶模拟提供了更可扩展的解决方案。
...
### [64] [UniSAFE: A Comprehensive Benchmark for Safety Evaluation of Unified Multimodal Models](https://arxiv.org/abs/2603.17476)
...
TL;DR: UniSAFE是首个针对统一多模态模型的系统级安全评估基准，涵盖7种I/O模态组合，揭示当前UMMs在安全对齐方面的关键漏洞


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型虽然具有强大的跨模态能力，但引入了单任务模型未观察到的安全风险。现有安全基准在任务和模态上碎片化，无法全面评估复杂的系统级漏洞

Method: 构建UniSAFE基准，采用共享目标设计，将常见风险场景映射到任务特定的I/O配置中，包含6,802个精心策划的实例，涵盖7种I/O模态组合，包括传统任务和新型多模态上下文图像生成设置

Result: 评估15个最先进的UMMs（包括专有和开源模型），发现关键漏洞：多图像组合和多轮设置中安全违规率升高，图像输出任务比文本输出任务更易受攻击

Conclusion: 当前UMMs存在显著的系统级安全漏洞，需要更强的安全对齐措施。UniSAFE为全面评估多模态模型安全提供了重要工具
...
TL;DR: 研究探索语音属性条件对3D手语动作生成的影响，建立了基于扩散模型的基线系统，发现将符号化ASL-LEX标注转换为自然语言对CLIP编码器至关重要，最佳模型在所有指标上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 生成自然、正确且视觉流畅的基于文本输入的3D手语动作仍然非常具有挑战性。本研究旨在探索语音属性条件在手语动作生成中的作用，使用ASL-LEX 2.0标注（如手形、手部位置和运动）来改进生成质量。

Method: 首先建立基于Human Motion MDM风格的扩散模型基线，使用SMPL-X表示。然后系统研究不同文本编码器（CLIP vs. T5）、条件模式（仅gloss vs. gloss+语音属性）和属性表示格式（符号化 vs. 自然语言）对生成效果的影响。

Result: 研究发现将符号化ASL-LEX标注转换为自然语言是CLIP编码器有效条件化的必要条件，而T5编码器基本不受这种转换影响。最佳模型（CLIP配合映射属性）在所有指标上都超越了现有的SignAvatar方法。

Conclusion: 输入表示是文本编码器属性条件化的关键因素，这促使采用结构化条件方法，其中gloss和语音属性通过独立路径编码，为改进3D手语动作生成提供了重要见解。
...
TL;DR: 提出一个利用基础模型解决材料分类数据稀缺问题的新框架，通过图像生成自动标注和先验知识融合提升分类性能


<details>
  <summary>Details</summary>
Motivation: 材料分类在计算机视觉和图形学中很重要，但面临标注数据稀缺的挑战，现有基于视觉语言基础模型的方法在材料识别任务中效果仍不理想

Method: 包含两个关键创新：(1) 鲁棒的图像生成和自动标注流程，创建多样化的材料中心图像数据集，通过融合物体语义和材料属性的文本提示自动分配标签；(2) 先验知识融合策略，从视觉语言模型中提取信息，结合联合微调方法优化预训练视觉基础模型和VLM先验

Result: 在多个数据集上取得显著改进，合成数据集能有效捕捉真实世界材料的特性，视觉语言模型先验的整合显著提升了最终性能

Conclusion: 提出的框架能有效利用基础模型克服数据限制并提高分类准确性，代码和数据集将开源
...
<details>
  <summary>Details</summary>
Motivation: 从单目RGB图像估计3D手部姿态对于AR/VR、人机交互和手语理解应用至关重要。现有方法通常忽略手势语义信息，而本文认为离散的手势标签可以作为强大的归纳偏置来提升姿态估计精度。

Method: 提出两阶段框架：1) 手势感知预训练，利用InterHand2.6M的粗粒度与细粒度手势标签学习信息丰富的嵌入空间；2) 基于手势嵌入引导的逐关节token Transformer，作为中间表示来回归MANO手部参数。训练采用分层目标函数，涵盖参数、关节和结构约束。

Result: 在InterHand2.6M数据集上的实验表明，手势感知预训练持续提升了单手姿态估计精度，超越了最先进的EANet基线。该方法的优势能够跨架构迁移，无需任何修改。

Conclusion: 手势语义可以作为有效的归纳偏置来提升3D手部姿态估计性能。提出的两阶段框架通过手势感知预训练和Transformer引导的回归，在保持架构通用性的同时实现了精度提升。
...
TL;DR: 提出一种基于运动自适应的时序注意力机制，用于参数高效的视频生成，在冻结的Stable Diffusion模型基础上仅添加少量参数即可实现竞争性结果。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法通常对所有视频内容采用统一的时序处理方式，未能根据运动内容动态调整注意力机制。高运动和低运动序列需要不同的时序关注策略来平衡细节保持和场景一致性。

Method: 1) 运动自适应时序注意力机制：根据估计的运动内容动态调整时序注意力感受野，高运动序列局部跨帧关注以保留快速变化细节，低运动序列全局关注以增强场景一致性；2) 级联注入策略：在UNet所有transformer块中注入轻量级时序注意力模块，下采样和中间块使用全局注意力进行语义稳定，上采样块使用运动自适应注意力进行细粒度优化；3) 时序相关噪声初始化和运动感知门控。

Result: 仅添加25.8M可训练参数（占基础UNet的2.9%），在100K视频训练后，在WebVid验证集上取得竞争性结果。标准去噪目标本身提供足够的隐式时序正则化，优于添加显式时序一致性损失的方法。消融研究揭示了噪声相关性与运动幅度之间的权衡关系。

Conclusion: 运动自适应时序注意力机制能够有效平衡视频生成中的细节保持和场景一致性，仅需少量额外参数即可实现高质量视频生成，为推理时控制提供了实用的噪声相关性-运动幅度权衡机制。
...
TL;DR: ASSR-EIC：基于任意尺度超分辨率的可变码率极端图像压缩框架，通过单一模型支持多种码率，在极低码率下实现高质量重建


<details>
  <summary>Details</summary>
Motivation: 现有扩散基极端图像压缩方法需要为每个目标码率训练单独模型，计算开销大且部署困难。同时，现有联合超分辨率方法在极低码率下效果不佳，且固定超分辨率尺度无法适应不同码率需求

Method: 提出ASSR-EIC框架：编码器端引入任意尺度下采样模块实现可控码率降低；解码器端采用基于扩散的联合退化感知任意尺度超分辨率解码器，支持单一模型内的码率自适应重建。利用压缩和缩放感知扩散先验指导重建，设计全局压缩-缩放适配器和局部压缩-缩放调制器，并引入双重语义增强设计

Result: 实验表明ASSR-EIC在极端图像压缩中达到最先进性能，同时支持灵活的码率控制和自适应码率相关重建

Conclusion: ASSR-EIC成功解决了现有极端图像压缩方法的局限性，通过任意尺度超分辨率实现了可变码率压缩，在单一模型中支持多种码率设置，为极端图像压缩提供了高效实用的解决方案
...
TL;DR: Loc3R-VLM是一个为2D视觉语言模型增加3D理解能力的框架，通过单目视频输入实现空间认知，在语言定位和3D问答任务上达到SOTA


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在空间理解和视角感知推理方面仍有不足，现有方法主要通过在输入中添加几何线索而非显式教授3D推理能力

Method: 受人类空间认知启发，采用两个联合目标：全局布局重建构建场景结构整体表示，显式情境建模锚定自我中心视角；利用预训练3D基础模型的轻量相机姿态先验确保几何一致性和度量尺度对齐

Result: 在语言定位任务上达到最先进性能，在情境和通用3D问答基准上优于现有2D和视频方法

Conclusion: 空间监督框架能够实现强大的3D理解能力，为2D视觉语言模型赋能高级3D认知
...
### [71] [Mutually Causal Semantic Distillation Network for Zero-Shot Learning](https://arxiv.org/abs/2603.17412)
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本学习方法通常使用弱监督的单向注意力机制，学习到的潜在语义表示是虚假且有限的，无法有效发现视觉和属性特征之间的内在语义知识（如属性语义）。

Method: 提出相互因果语义蒸馏网络（MSDN++），包含两个子网络：属性→视觉因果注意力子网络学习基于属性的视觉特征，视觉→属性因果注意力子网络学习基于视觉的属性特征。通过因果注意力机制学习因果视觉-属性关联，并在语义蒸馏损失的指导下相互协作学习。

Result: 在CUB、SUN、AWA2和FLO等广泛使用的基准数据集上进行大量实验，MSDN++相比强基线方法有显著改进，达到了新的最先进性能。
...
### [72] [Towards Motion-aware Referring Image Segmentation](https://arxiv.org/abs/2603.17413)
*Chaeyun Kim,Seunghoon Yi,Yejin Kim,Yohan Jo,Joonseok Lee*

Main category: cs.CV

TL;DR: 提出针对Referring Image Segmentation中运动相关查询性能不足的问题，通过数据增强和多模态对比学习提升模型对运动描述的理解能力


<details>
  <summary>Details</summary>
Motivation: 现有Referring Image Segmentation方法在处理运动相关查询时性能显著低于外观描述查询，需要解决模型对运动表达理解不足的问题

Method: 1. 高效数据增强方案：从原始标注中提取运动中心短语，无需额外标注即可让模型接触更多运动表达；2. 多模态径向对比学习：在融合的图像-文本嵌入上执行对比学习，而非单模态表示

Result: 提出的方法显著提升了多个RIS模型在运动中心查询上的性能，同时在外观描述上保持竞争力；创建了新的测试划分M-Bench用于全面评估

Conclusion: 通过数据增强和多模态对比学习的结合，有效解决了RIS中运动相关查询性能不足的问题，为处理上下文相关的对象描述提供了有效方案
...
### [73] [SHIFT: Motion Alignment in Video Diffusion Models with Adversarial Hybrid Fine-Tuning](https://arxiv.org/abs/2603.17426)
...
<details>
  <summary>Details</summary>
Motivation: 图像条件视频扩散模型在视觉真实性方面表现出色，但在微调后经常出现运动保真度下降的问题，如运动动态减弱或长期时间一致性退化

Method: 提出基于像素通量动态的像素运动奖励，捕捉瞬时和长期运动一致性；设计SHIFT框架，将监督微调和优势加权微调融合，利用新颖的对抗优势提高收敛速度并减轻奖励攻击

Result: 实验表明该方法能有效解决现代视频扩散模型监督微调中的动态度崩溃问题

Conclusion: SHIFT框架通过像素运动奖励和混合微调策略，成功提升了视频扩散模型的运动保真度，解决了微调后的运动退化问题
...
### [74] [ECHO: Towards Emotionally Appropriate and Contextually Aware Interactive Head Generation](https://arxiv.org/abs/2603.17427)
*Xiangyu Kong,Xiaoyu Jin,Yihan Pan,Haoqin Sun,Hengde Zhu,Xiaoming Xu,Xiaoming Wei,Lu Liu,Siyang Song*

Main category: cs.CV

TL;DR: 论文提出ECHO框架，通过长程上下文理解和空间感知解耦交叉注意力模块，解决交互式头部生成中上下文适当性和唇部同步的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有交互式头部生成方法依赖短时行为线索，缺乏长程上下文建模，导致面部行为缺乏上下文适当性；同时双轨信号融合方式存在交叉干扰，可能损害唇部同步质量。

Method: 提出ECHO框架：1) 长程上下文理解组件，建模行为动态和语言驱动的情感语义；2) 块级空间感知解耦交叉注意力模块，保持音频驱动的唇部运动，同时自适应整合用户行为线索到非唇部区域；3) 两阶段训练范式。

Result: 大量实验证明所提组件的有效性，ECHO在交互式头部生成任务上表现出优越性能。

Conclusion: ECHO框架通过长程上下文建模和空间解耦的信号融合，显著提升了生成头像的面部行为上下文适当性、情感合理性和唇部同步质量。
...
### [75] [AdaZoom-GUI: Adaptive Zoom-based GUI Grounding with Instruction Refinement](https://arxiv.org/abs/2603.17441)
...
<details>
  <summary>Details</summary>
Motivation: GUI定位是视觉语言模型的关键能力，但当前面临高分辨率图像、小UI元素和模糊用户指令的挑战，需要更精确的定位和指令理解方法
...
Conclusion: AdaZoom-GUI框架通过自适应缩放方法显著提升了GUI定位的准确性和效率，为GUI自动化交互提供了实用解决方案
...
### [76] [FACE-net: Factual Calibration and Emotion Augmentation for Retrieval-enhanced Emotional Video Captioning](https://arxiv.org/abs/2603.17455)
*Weidong Chen,Cheng Ye,Zhendong Mao,Peipei Song,Xinyan Liu,Lei Zhang,Xiaojun Chang,Yongdong Zhang*

Main category: cs.CV

TL;DR: 提出FACE-net框架，通过检索增强、事实校准和情感增强解决情感视频描述中的事实-情感偏差问题


<details>
  <summary>Details</summary>
Motivation: 现有情感视频描述方法在挖掘和协调事实与情感线索方面不足，难以处理不同样本中事实与情感需求不同的偏差问题

Method: 提出检索增强框架FACE-net，包含外部知识库检索、基于不确定性估计的事实校准、渐进式视觉情感增强和动态偏差调整路由模块

Result: 通过统一架构协同挖掘事实-情感语义，为生成提供自适应准确指导，突破所有样本学习中事实-情感描述的妥协倾向

Conclusion: FACE-net框架有效解决了情感视频描述中的事实-情感偏差问题，通过检索增强、事实校准和情感增强机制提升了描述质量
...
### [77] [AR-CoPO: Align Autoregressive Video Generation with Contrastive Policy Optimization](https://arxiv.org/abs/2603.17461)
...
<details>
  <summary>Details</summary>
Motivation: 现有的SDE-based GRPO方法在流式自回归视频生成器上难以有效对齐，因为少步ODE和一致性模型采样器偏离标准流匹配ODE，且短轨迹对初始化噪声高度敏感，导致中间SDE探索失效

Method: AR-CoPO采用分块级对齐机制：1）在随机选择的分块处通过分叉机制构建邻域候选；2）分配序列级奖励；3）执行局部GRPO更新。同时提出半在线策略训练策略，结合在线探索和参考轨迹回放缓冲区的利用

Result: 在Self-Forcing上的实验表明，AR-CoPO在域外泛化和域内人类偏好对齐方面均优于基线，提供了真正对齐而非奖励黑客的证据

Conclusion: AR-CoPO成功地将对比策略优化框架适配到流式自回归生成场景，解决了现有强化学习方法在该设置下的对齐挑战，实现了更有效的视频生成器对齐
...
### [78] [VirPro: Visual-referred Probabilistic Prompt Learning for Weakly-Supervised Monocular 3D Detection](https://arxiv.org/abs/2603.17470)
*Chupeng Liu,Jiyong Rao,Shangquan Sun,Runkai Zhao,Weidong Cai*

Main category: cs.CV

TL;DR: 提出VirPro方法，通过自适应多模态预训练增强弱监督单目3D检测，利用视觉引导的概率提示学习来捕捉场景视觉多样性，在KITTI基准上实现4.8%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 单目3D检测通常依赖伪标签技术，现有方法使用确定性语言提示作为弱监督信号，但手工制作的文本描述难以捕捉不同场景中个体的视觉多样性，限制了模型学习场景感知表示的能力。

Method: 提出视觉引导的概率提示学习(VirPro)：1) 构建自适应提示库(APB)存储可学习的实例条件提示；2) 引入多高斯提示建模(MGPM)，将场景视觉特征融入文本嵌入以表达视觉不确定性；3) 从融合的视觉语言嵌入解码提示目标高斯分布，获得统一的对象级提示嵌入；4) 使用RoI级对比匹配增强模态对齐和语义连贯性。

Result: 在KITTI基准上的大量实验表明，集成该预训练范式能带来显著的性能提升，相比基线方法平均精度提升高达4.8%。

Conclusion: VirPro通过自适应多模态预训练有效解决了弱监督单目3D检测中手工文本提示难以捕捉视觉多样性的问题，提出的概率提示学习方法能够增强场景感知表示，提升检测性能。
...
### [79] [Revisiting Cross-Attention Mechanisms: Leveraging Beneficial Noise for Domain-Adaptive Learning](https://arxiv.org/abs/2603.17474)
...
Result: 在VisDA-2017、Office-Home和DomainNet上达到SOTA性能，VisDA-2017上比CDTrans提升2.3%，在"卡车"类上提升5.9%

Conclusion: 结合域翻译、有益噪声增强的注意力和尺度感知对齐，能有效实现鲁棒的跨域表示学习
...
TL;DR: 提出UAV-CB数据集和LFBNet网络，解决低空复杂背景和伪装条件下的无人机检测难题


<details>
  <summary>Details</summary>
Motivation: 现实场景中无人机常与建筑物、植被、电线等背景融合，导致低对比度、弱边界和纹理混淆，现有数据集未能充分捕捉这些伪装和复杂背景挑战

Method: 构建UAV-CB RGB-T数据集，并提出局部频率桥接网络(LFBNet)，在局部频率空间建模特征，同时解决频率-空间融合差距和跨模态差异差距

Result: 在UAV-CB和公共基准测试中，LFBNet实现了最先进的检测性能，在伪装和杂乱条件下表现出强大的鲁棒性

Conclusion: LFBNet为现实应用中多模态无人机感知提供了频率感知的新视角，有效解决了复杂背景和伪装条件下的检测挑战
...
<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型在将复杂结构化数字图形转换为可执行代码方面面临重大挑战，这需要高保真视觉感知和精确代码生成的协同作用，而传统描述性任务无法充分评估这种能力。

Method: 创建包含1080个样本的综合基准，涵盖科学可视化到复杂符号表示等多种数字内容，每个样本都配有可执行的参考代码。评估框架将性能解耦为感知保真度和符号精度两个维度。

Result: 评估显示领先的大型多模态模型之间存在显著性能差距，即使是当前最先进的模型在复杂场景中也难以保持结构完整性，表明多模态代码生成仍然是一个严峻挑战。

Conclusion: Omni-I2C基准揭示了当前大型多模态模型在视觉到代码转换任务中的局限性，强调了需要更强大的视觉感知和代码生成协同能力，为未来模型改进提供了重要参考。
...
TL;DR: 提出EI框架和MoR方法解决多模态医学影像疾病识别的两大挑战：早期干预融合和参数高效微调


<details>
  <summary>Details</summary>
Motivation: 当前多模态医学影像疾病识别面临两个主要问题：1) 主流"单模态嵌入后融合"范式无法充分利用多模态数据的互补和相关信息；2) 标注多模态医学图像稀缺且与自然图像存在显著领域偏移，阻碍了前沿视觉基础模型的应用

Method: 提出早期干预(EI)框架：将一种模态作为目标，其余作为参考，利用参考模态的高层语义标记作为干预标记，在早期阶段引导目标模态的嵌入过程。同时提出MoR方法：一种参数高效的微调方法，使用一组不同秩的低秩适配器和权重松弛路由器来适应视觉基础模型

Result: 在视网膜疾病、皮肤病变和膝关节异常分类的三个公共数据集上进行广泛实验，验证了所提方法相对于多个竞争基线的有效性

Conclusion: 提出的EI框架和MoR方法有效解决了多模态医学影像疾病识别的挑战，实现了更好的特征融合和参数高效微调
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在稀疏视图监督下预测过多高斯基元导致几何不稳定和深度质量差，且仅依赖2D分割器特征进行语义提升，导致3D语义不完整和泛化能力有限

Method: 1. 误差感知高斯丢弃(EGD)：利用渲染误差线索抑制冗余倾向的高斯基元，实现误差引导的容量控制；2. 混合训练课程(MTC)：逐步融合2D分割器提升的语义与模型自身涌现的3D语义先验，通过对象级原型对齐增强语义一致性和完整性

Result: 在ScanNet和Replica数据集上，UniSem在深度预测和开放词汇3D分割方面表现优异。使用16个输入视图时，深度相对误差降低15.2%，开放词汇分割平均准确率提升3.7%

Conclusion: UniSem通过联合优化深度精度和语义泛化，解决了稀疏无位姿图像3D重建中的几何不稳定和语义不完整问题，为3D高斯溅射提供了统一的改进框架
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在开放词汇语义和部件分割任务中，通过串行结构提取图像-文本对齐线索，导致类别级语义和空间上下文之间存在知识干扰，限制了模型获取更丰富的视觉语言对齐信息。

Method: 提出并行成本聚合范式PCA-Seg：1）专家驱动感知学习模块，包含多专家解析器从多视角提取互补特征，系数映射器自适应学习像素特定权重；2）特征正交化解耦策略，减少语义流和上下文流之间的冗余，从正交化特征中学习多样化知识。

Result: 在8个基准测试上进行了广泛实验，PCA-Seg的每个并行块仅增加0.35M参数，但实现了最先进的开放词汇语义和部件分割性能。

Conclusion: 提出的PCA-Seg范式通过并行成本聚合有效缓解了类别语义与空间上下文之间的知识干扰问题，能够从成本体积中捕获更丰富的视觉语言对齐信息，为开放词汇分割任务提供了一种简单而有效的解决方案。
...
TL;DR: MM-OVSeg：一种多模态光学-SAR融合框架，用于恶劣天气条件下的开放词汇分割，通过跨模态统一和双编码器融合实现鲁棒性


<details>
  <summary>Details</summary>
Motivation: 开放词汇分割在遥感领域具有巨大潜力，但现有方法主要局限于晴空光学数据，在云层或雾霾污染条件下表现不佳。需要一种能够利用光学和SAR数据互补优势的解决方案。

Method: 提出MM-OVSeg多模态光学-SAR融合框架，包含两个关键设计：1）跨模态统一过程用于多传感器表示对齐；2）双编码器融合模块，集成多个视觉基础模型的分层特征，实现文本对齐的多模态分割。

Result: 大量实验表明，MM-OVSeg在不同云层条件下实现了优越的鲁棒性和泛化能力。源代码和数据集已公开。

Conclusion: MM-OVSeg通过有效融合光学和SAR数据的互补优势，解决了恶劣天气条件下开放词汇分割的挑战，为遥感领域提供了更稳健的多模态分割解决方案。
...
TL;DR: AdapTS：用于多类别和持续学习的统一教师-学生框架，通过共享冻结主干和轻量适配器实现边缘部署，大幅减少内存开销


<details>
  <summary>Details</summary>
Motivation: 现有视觉异常检测方法主要针对单类别场景，无法满足实际工业环境中多类别和持续学习的需求。教师-学生架构虽然高效，但在持续学习场景中尚未被探索。

Method: 提出AdapTS框架：使用单一共享冻结主干网络，在学生路径中注入轻量可训练适配器；采用分割引导目标和合成Perlin噪声增强训练；基于原型的任务识别机制在推理时动态选择适配器。

Result: 在MVTec AD和VisA数据集上，AdapTS在多类别和持续学习场景中性能与现有TS方法相当，同时大幅减少内存开销。最轻版本AdapTS-S仅需8MB额外内存，比STFPM少13倍，比RD4AD少48倍，比DeSTSeg少149倍。

Conclusion: AdapTS为复杂工业环境中的边缘部署提供了高度可扩展的解决方案，通过统一框架解决了多类别和持续学习的视觉异常检测问题，同时保持高效性能和极低内存需求。
...
TL;DR: 提出Rel-Zero关系零水印框架，利用图像块间关系距离在AI编辑中的不变性，实现无需修改原图的内容认证。


<details>
  <summary>Details</summary>
Motivation: 扩散模型图像编辑技术威胁数字内容真实性。传统嵌入水印方法为保持鲁棒性会引入可见扰动，损害视觉质量；现有零水印方法依赖全局特征，难以抵抗复杂编辑操作。

Method: 基于关键观察：AI编辑中单个图像块会发生显著变化，但块对之间的相对距离关系保持相对不变。提出关系零水印框架，从这些编辑不变的关系中提取唯一零水印，无需修改原图。

Result: 大量实验表明，Rel-Zero相比现有零水印方法，在各种编辑模型和操作下实现了显著提升的鲁棒性。

Conclusion: 通过基于内在结构一致性而非绝对外观的水印机制，Rel-Zero提供了非侵入式且具有弹性的内容认证解决方案。
...
Method: 提出Equivariant Coordinate-based Kernel Convolution (ECKConv)，通过在双陪集空间中定义核域实现SE(3)等变性，使用基于坐标的网络进行显式核设计，增强学习能力和内存效率。

Result: 在点云分类、姿态配准、部件分割和大规模语义分割等任务中验证了ECKConv的刚性等变性、内存可扩展性，并优于最先进的等变方法。

Conclusion: ECKConv通过交织算子框架解决了对称性与可扩展性的权衡问题，在保持严格SE(3)等变性的同时实现了大规模点云任务的高效处理。
...
TL;DR: Video-SFT提升MLLMs的视频理解能力，但常导致静态图像性能下降，形成图像-视频权衡；增加帧数改善视频性能但不保证图像性能；提出混合帧策略部分缓解此问题


<details>
  <summary>Details</summary>
Motivation: 研究Video-SFT对MLLMs视觉能力细粒度演变的影响，特别是空间与时间理解的平衡问题，目前对此了解不足

Method: 系统研究Video-SFT如何重塑MLLMs视觉能力，分析不同架构、参数规模和帧采样设置下的模式；提出指令感知的混合帧策略，自适应分配帧数

Result: Video-SFT一致提升视频性能，但对静态图像基准测试增益有限甚至导致性能下降；增加采样帧数一般改善视频性能，但不稳定改善图像性能；混合帧策略部分缓解图像-视频权衡

Conclusion: Video-SFT对MLLMs并非免费午餐，保持空间理解能力在联合图像-视频训练中仍是核心挑战
...
TL;DR: ProGVC是一个渐进式生成视频压缩框架，通过多尺度残差令牌映射和Transformer自回归上下文模型，实现了低比特率下的感知压缩，同时支持渐进传输和灵活码率适应。


<details>
  <summary>Details</summary>
Motivation: 现有感知视频编解码器缺乏对可变比特率和渐进传输的原生支持，且生成模块与熵编码弱耦合，限制了比特率降低。需要一种统一渐进传输、高效熵编码和细节合成的编解码框架。

Method: 提出ProGVC框架：1) 将视频编码为层次化多尺度残差令牌映射，支持渐进传输；2) 使用基于Transformer的多尺度自回归上下文模型估计令牌概率，用于熵编码和预测截断的细尺度令牌；3) 在解码器端恢复感知细节。

Result: 大量实验表明，ProGVC作为一种新的编码范式，在低比特率下提供了有前景的感知压缩性能，同时具备实际可扩展性。

Conclusion: ProGVC成功统一了渐进传输、高效熵编码和细节合成，为低比特率感知视频压缩提供了新的解决方案，具有实用可扩展性。
...
TL;DR: PF-RPN是一种无需外部提示的通用区域提议网络，通过稀疏图像感知适配器、级联自提示模块和中心度引导查询选择模块，在有限数据下识别潜在对象，适用于多种检测领域。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖示例图像、预定义类别或文本描述来定位潜在对象，这种对外部提示的依赖限制了灵活性，难以适应现实场景。需要一种无需外部提示的通用对象识别方法。

Method: 提出PF-RPN框架：1）SIA模块使用可学习查询嵌入进行初始定位；2）CSP模块通过级联自提示机制识别剩余对象；3）CG-QS模块利用中心度评分网络选择高质量查询嵌入。该方法仅需少量数据训练，无需微调即可应用于不同领域。

Result: 在19个数据集上的实验验证了方法的有效性。仅使用MS COCO 5%的数据训练，即可直接应用于水下物体检测、工业缺陷检测、遥感图像检测等多个领域，无需微调。

Conclusion: PF-RPN是一种无需外部提示的通用区域提议网络，通过创新的自提示机制和查询选择策略，在有限数据下实现了跨领域的潜在对象识别，为计算机视觉应用提供了更灵活、适应性更强的解决方案。
...
TL;DR: FrescoDiffusion：一种无需训练的方法，通过预计算的潜在先验增强分块去噪，实现4K超高分辨率图像到视频生成，解决全局布局一致性问题


<details>
  <summary>Details</summary>
Motivation: 现有扩散式图像到视频模型难以处理超高分辨率输入（如4K），在壁画动画等复杂场景中，分块去噪会破坏全局布局一致性，而原生分辨率生成会丢失细节

Method: 首先生成低分辨率视频并上采样其潜在轨迹作为全局参考，然后在每个扩散时间步计算分块噪声预测，通过加权最小二乘目标函数将分块预测与全局参考融合，保持全局一致性同时保留细节

Result: 在VBench-I2V数据集和自建的壁画I2V数据集上，相比分块基线方法，FrescoDiffusion在全局一致性和保真度方面表现更好，计算效率高，并能通过正则化控制创意与一致性之间的权衡

Conclusion: FrescoDiffusion通过预计算潜在先验和融合策略，有效解决了超高分辨率图像到视频生成中的全局一致性难题，为壁画动画等复杂场景提供了实用的解决方案
...
TL;DR: 提出一种特征保留的人脸匿名化框架，通过密集面部关键点保留表情，并通过轻量后处理模块确保光照和肤色一致性，在保持隐私的同时提升下游任务可用性。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸匿名化方法主要关注身份移除和图像真实感，但忽略了面部表情以及光照、肤色等光度一致性特征，而这些特征对于重光照、色彩恒常性、医疗或情感分析等下游任务至关重要。

Method: 在DeepPrivacy基础上扩展，引入密集面部关键点以更好地保留表情，并添加轻量级后处理模块来确保光照方向和肤色的一致性。同时建立了专门评估表情保真度、光照一致性和色彩保持的指标。

Result: 在CelebA-HQ数据集上的实验表明，该方法生成的匿名化人脸在真实感方面有所提升，并且在表情、光照和肤色保真度方面显著优于现有先进基线方法。

Conclusion: 特征感知的匿名化是实现更有用、更公平、更可信的隐私保护面部数据的重要一步，强调了在隐私保护的同时保留关键面部特征的重要性。
...
TL;DR: PanoVGGT：一个用于全景图像的置换等变Transformer框架，可联合预测相机位姿、深度图和3D点云，解决了全景图像中的非针孔畸变问题。


<details>
  <summary>Details</summary>
Motivation: 全景图像提供360°视野但在消费设备中越来越常见，然而它们引入了非针孔畸变，给联合位姿估计和3D重建带来挑战。现有的前馈模型是为透视相机设计的，在这种设置下泛化能力差。

Method: 提出PanoVGGT，一个置换等变Transformer框架，通过单次前向传播从一张或多张全景图联合预测相机位姿、深度图和3D点云。模型包含球面感知位置嵌入和全景特定的三轴SO(3)旋转增强，支持球面域的有效几何推理。为解决固有的全局框架模糊性，在训练中引入了随机锚定策略。

Result: 在PanoCity和标准基准测试上的大量实验表明，PanoVGGT实现了有竞争力的精度、强大的鲁棒性和改进的跨域泛化能力。同时贡献了PanoCity，一个具有密集深度和6-DoF位姿标注的大规模户外全景数据集。

Conclusion: PanoVGGT通过球面感知设计和随机锚定策略，有效解决了全景图像的位姿估计和3D重建问题，在精度、鲁棒性和泛化能力方面表现出色，代码和数据集将公开发布。
...
TL;DR: LoGSAM：一种参数高效的检测驱动框架，将放射科医生口述转换为文本提示，用于基于基础模型的脑肿瘤定位和分割，仅需更新5%参数即可实现最先进的分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有脑肿瘤MRI定位和分割方法大多依赖任务特定的监督模型，受限于标注数据的稀缺性。需要一种能够利用放射科医生自然语言描述、减少对大量标注数据依赖的解决方案。
...
Result: 在BRISC 2025数据集上达到80.32%的Dice分数（state-of-the-art）。使用德国放射科医生口述对12个未见MRI扫描进行评估，达到91.7%的病例级准确率。

Conclusion: 通过智能利用预训练基础模型并进行最小参数更新，可以构建模块化的语音到分割管道，减少对大量标注数据的依赖，同时保持高性能。
...
### [96] [Edit-As-Act: Goal-Regressive Planning for Open-Vocabulary 3D Indoor Scene Editing](https://arxiv.org/abs/2603.17583)
*Seongrae Noh,SeungWon Seo,Gyeong-Moon Park,HyeongYeop Kang*

Main category: cs.CV

TL;DR: Edit-As-Act：一个通过目标回归规划在3D空间中进行开放词汇场景编辑的框架，将编辑视为最小化动作序列而非生成任务，确保物理一致性和指令忠实度。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇系统将编辑视为生成任务，导致全局变化或物理不一致。作者认为编辑应是最小化动作序列，使期望的世界状态为真，同时保持其他一切不变。

Method: 设计EditLang（PDDL启发的动作语言），预测符号化目标谓词，通过语言驱动规划器提出动作，验证器强制执行目标导向性、单调性和物理可行性。

Result: 在E2A-Bench（63个编辑任务，9个室内环境）上，Edit-As-Act在所有编辑类型和场景类别上显著优于现有方法。

Conclusion: 通过将推理与低级生成分离，Edit-As-Act实现了指令忠实度、语义一致性和物理合理性这三个现有范式无法同时满足的标准。
...
### [97] [Trust the Unreliability: Inward Backward Dynamic Unreliability Driven Coreset Selection for Medical Image Classification](https://arxiv.org/abs/2603.17603)
*Yan Liang,Ziyuan Yang,Zhuxin Lei,Mengyu Sun,Yingyu Chen,Yi Zhang*

Main category: cs.CV

TL;DR: 提出DUCS策略，通过动态不可靠性驱动的核心集选择，专注于训练中置信度波动大且频繁被遗忘的边界样本，以提升医学图像数据的高压缩率下模型性能。


<details>
  <summary>Details</summary>
Motivation: 大规模医学影像数据在有限资源下的高效管理面临挑战。传统核心集选择方法在医学数据上效果有限，因为医学数据存在类内差异大、类间相似性高的复杂性。研究发现神经网络在训练中对靠近类中心的样本记忆更好，但专注于这些样本不利于决策边界的建模。

Method: 提出动态不可靠性驱动的核心集选择(DUCS)策略，包含两个评估视角：1) 内向自我意识：通过分析训练过程中置信度的演变来量化每个样本的不确定性；2) 后向记忆追踪：通过追踪样本被遗忘的频率来评估模型对每个样本的保留能力。选择置信度波动大且频繁被遗忘的不可靠样本，这些样本通常靠近决策边界。

Result: 在公共医学数据集上的大量实验表明，该方法在压缩率较高时相比现有SOTA方法具有优越性能。

Conclusion: 不可靠样本（置信度波动大且频繁被遗忘）比稳定样本更具信息价值，能有效帮助模型精化决策边界。DUCS策略在医学图像数据的高压缩率场景下表现出色。
...
### [98] [ReLaGS: Relational Language Gaussian Splatting](https://arxiv.org/abs/2603.17605)
...
Method: 1) 构建层次化语言蒸馏高斯场景；2) 高斯剪枝机制优化场景几何；3) 多视图语言对齐策略聚合2D特征为准确3D对象嵌入；4) 构建开放词汇3D场景图，结合视觉语言标注和图神经网络关系推理。

Result: 在开放词汇分割、场景图生成和关系引导检索等任务上验证了方法的有效性，实现了高效可扩展的开放词汇3D推理。

Conclusion: ReLaGS框架通过联合建模层次语义和对象间/内关系，实现了无需场景特定训练的统一3D感知与推理，为开放词汇3D理解提供了新方案。
...
### [99] [S-VGGT: Structure-Aware Subscene Decomposition for Scalable 3D Foundation Models](https://arxiv.org/abs/2603.17625)
*Xinze Li,Pengxu Chen,Yiyuan Wang,Weifeng Su,Wentao Cheng*

Main category: cs.CV

TL;DR: S-VGGT通过结构帧级别的冗余处理解决3D基础模型中全局注意力二次计算成本问题，使用场景图进行结构分区，实现并行高效处理。
...
Method: 首先利用初始特征构建密集场景图来表征结构冗余并指导场景分区，然后将帧软分配到少量平衡的子场景中，设计共享参考帧的并行几何桥接，实现独立高效处理。

Result: S-VGGT通过结构重组从源头上减少全局注意力成本，提供内在加速，且与令牌级加速方法正交，可组合使用获得叠加加速效果而不损失重建保真度。

Conclusion: S-VGGT通过结构帧级别的冗余处理有效解决了3D基础模型的计算瓶颈，为大规模3D场景处理提供了可扩展的解决方案。
...
### [100] [A Multi-Agent System for Building-Age Cohort Mapping to Support Urban Energy Planning](https://arxiv.org/abs/2603.17626)
*Kundan Thota,Thorsten Schlachter,Veit Hagenmeyer*

Main category: cs.CV

TL;DR: 提出多智能体LLM系统融合异构数据源，结合BuildingAgeCNN卫星图像分类器，用于城市建筑年代分布预测，支持可持续热力规划
...
Method: 1) 多智能体LLM系统（Zensus、OSM、Monument智能体）融合异构数据源；2) 数据协调器进行地理编码和去重；3) BuildingAgeCNN分类器基于ConvNeXt骨干网络，集成FPN、CoordConv空间通道和SE模块；4) 包含校准置信度估计和低置信度人工审查的预测流程

Result: BuildingAgeCNN在空间交叉验证中达到90.69%总体准确率，但宏观F1分数为67.25%，反映了类别不平衡和历史时期相邻队列间的持续混淆；系统为能源需求规划提供支持

Conclusion: 多智能体LLM系统不仅帮助收集结构化数据，还能协助能源需求规划者优化区域供热网络，实现低碳可持续能源系统目标
...
### [101] [Part-Aware Open-Vocabulary 3D Affordance Grounding via Prototypical Semantic and Geometric Alignment](https://arxiv.org/abs/2603.17647)
*Dongqiang Gou,Xuming He*

Main category: cs.CV

TL;DR: 提出两阶段跨模态框架，通过语言模型生成部件感知指令和几何一致性建模，解决开放词汇3D功能区域定位问题


<details>
  <summary>Details</summary>
Motivation: 现有方法在开放词汇泛化、细粒度几何对齐和部件级语义一致性方面存在挑战，需要提升3D功能区域定位的语义和几何表示能力
...
Conclusion: 提出的方法通过增强语义和几何表示，有效解决了开放词汇3D功能区域定位的挑战，在多个基准上验证了其有效性
...
### [102] [Anchoring and Rescaling Attention for Semantically Coherent Inbetweening](https://arxiv.org/abs/2603.17651)
*Tae Eun Choi,Sumin Shim,Junhyeok Kim,Seong Jae Hwang*

Main category: cs.CV

TL;DR: 提出TGI-Bench基准测试和Keyframe-anchored Attention Bias方法，解决生成式中间帧合成中帧不一致、语义错位和节奏不稳定的问题，在无需额外训练的情况下实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有生成式中间帧合成模型在处理稀疏序列和大运动时，会出现帧不一致、节奏不稳定和语义错位的问题。由于中间帧合成涉及固定端点且存在多种合理路径，需要从关键帧和文本中获取额外指导来指定预期路径。

Method: 提出Keyframe-anchored Attention Bias方法，从关键帧和文本中提取语义和时间指导到每个中间帧；使用Rescaled Temporal RoPE增强帧一致性，使自注意力机制能更忠实地关注关键帧；建立TGI-Bench基准测试，专门用于文本条件中间帧合成的评估。

Result: 在无需额外训练的情况下，该方法在短序列和长序列的各种挑战中都实现了最先进的帧一致性、语义保真度和节奏稳定性。

Conclusion: 提出的方法通过关键帧锚定的注意力偏置和重新缩放的时间RoPE，有效解决了生成式中间帧合成中的一致性问题，并通过TGI-Bench基准测试提供了针对性的评估框架。
...
### [103] [Interpretable Cross-Domain Few-Shot Learning with Rectified Target-Domain Local Alignment](https://arxiv.org/abs/2603.17655)
*Yaze Zhao,Yixiong Zou,Yuhua Li,Ruixuan Li*

Main category: cs.CV

TL;DR: 提出CC-CDFSL方法解决CLIP在跨域少样本学习中局部特征对齐问题，通过循环一致性约束和语义锚机制提升局部视觉-语言对齐，实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的跨域少样本学习方法在医学诊断等需要细粒度视觉线索的下游任务中存在局部对齐问题，域差距和稀缺训练数据加剧了局部细微模式捕捉的困难

Method: 提出CC-CDFSL方法：1) 基于翻译任务的循环一致性约束，将局部视觉特征翻译为文本特征再翻译回视觉特征；2) 语义锚机制，通过视觉特征增强提供更大语料库，再收缩图像特征过滤无关映射

Result: 在多个基准测试、骨干网络和微调方法上验证：1) 有效改善局部视觉-语言对齐；2) 通过可视化补丁增强学习模式和模型决策的可解释性；3) 实现最先进的性能

Conclusion: 提出的CC-CDFSL方法成功解决了CLIP在跨域少样本学习中的局部对齐问题，通过自监督的循环一致性和语义锚机制，显著提升了模型在细粒度下游任务中的表现和可解释性
...
### [104] [FINER: MLLMs Hallucinate under Fine-grained Negative Queries](https://arxiv.org/abs/2603.17662)
*Rui Xiao,Sanghwan Kim,Yongqin Xian,Zeynep Akata,Stephan Alaniz*

Main category: cs.CV

TL;DR: 提出了FINER基准来评估多模态大语言模型在细粒度查询中的幻觉问题，并开发了FINER-Tuning方法显著减少幻觉


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注粗粒度图像相关问题，而多模态大语言模型在细粒度查询中容易出现幻觉问题，这一问题在现有基准中代表性不足

Method: 引入FINER（细粒度否定查询）基准，包含FINER-CompreCap和FINER-DOCCI两个子集；提出FINER-Tuning方法，利用直接偏好优化在FINER启发数据上进行微调

Result: FINER-Tuning在四个前沿MLLMs上实现高达24.2%的幻觉减少（InternVL3.5-14B），同时提升八个现有幻觉基准的性能，并增强六个通用多模态基准的能力

Conclusion: FINER基准有效揭示了MLLMs在细粒度查询中的幻觉问题，FINER-Tuning方法能显著减少幻觉同时提升模型整体性能，为多模态幻觉研究提供了新工具
...
Method: 提出实例感知的离散化框架，基于输入相关先验学习自适应时间步分配，将基于梯度的离散化搜索扩展到条件生成设置

Result: 在合成数据、像素空间扩散、潜在空间图像和视频流匹配模型等多种设置中，该方法一致提高了生成质量，相比训练只有边际调整成本，推理开销可忽略

Conclusion: 实例感知的离散化框架能够有效提升扩散和流匹配模型的生成性能，通过适应实例特定复杂性克服了全局时间步调度的局限性
...
### [106] [WeatherReasonSeg: A Benchmark for Weather-Aware Reasoning Segmentation in Visual Language Models](https://arxiv.org/abs/2603.17680)
*Wanjun Du,Zifeng Yuan,Tingting Chen,Fucai Ke,Beibei Lin,Shunli Zhang*

Main category: cs.CV

TL;DR: WeatherReasonSeg：首个评估视觉语言模型在恶劣天气条件下推理分割能力的基准，包含可控合成数据集和真实世界数据集，揭示VLM性能随天气严重程度单调下降且不同天气类型产生不同脆弱模式


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在理想条件下的推理分割表现优秀，但在雨雪雾等恶劣天气导致视觉线索严重退化时，其可靠性未知。需要评估VLM在真实世界恶劣天气条件下的鲁棒性。

Method: 1. 构建可控推理数据集：对现有分割数据集应用不同严重程度的合成天气，实现细粒度鲁棒性分析；2. 构建真实世界数据集：通过掩码引导的LLM提示生成语义一致的查询，捕获真实世界复杂性；3. 扩展评估范围到五个推理维度：功能、应用场景、结构属性、交互和需求匹配。

Result: 对多种VLM的广泛实验发现：1. VLM性能随天气严重程度单调下降；2. 不同天气类型引发不同的脆弱模式。WeatherReasonSeg为推进鲁棒的天气感知推理提供了基础。

Conclusion: WeatherReasonSeg是首个专门评估VLM在恶劣天气条件下推理分割能力的基准，揭示了VLM在恶劣天气下的性能退化模式和脆弱性差异，为开发更鲁棒的天气感知视觉语言模型提供了重要基础。
...
TL;DR: DeepCORO-CLIP是一个基于视频-文本对比学习的多视图基础模型，用于自动化冠状动脉造影解读，在狭窄检测、心血管事件预测和疾病进展评估等多个任务上表现出色。
...
Method: 采用视频-文本对比学习训练多视图基础模型，整合多个投影并使用基于注意力的池化进行研究级评估。在28,117名患者的203,808个血管造影视频上训练，并在外部数据集上验证。

Result: 显著狭窄检测AUROC内部0.888，外部0.89；与核心实验室QCA相比MAE为13.6%（临床报告为19.0%）。在慢性完全闭塞、血栓和钙化检测上表现良好。迁移学习预测一年MACE的AUROC为0.79，LVEF估计MAE为7.3%。平均推理时间4.2秒。

Conclusion: DeepCORO-CLIP为自动化冠状动脉造影解读提供了基础，支持点护理应用，代码、数据和模型权重已公开。
...
TL;DR: 该论文提出使用成对闪光-非闪光接触式指纹采集作为轻量级主动感知机制，用于接触式指纹的欺骗检测，通过光照诱导的差异特征提高鲁棒性和可解释性。
...
Method: 采用成对闪光-非闪光接触式指纹采集作为主动感知机制，通过初步实证分析显示闪光照明能增强材料和结构相关特性。使用可解释指标分析光照诱导差异，包括通道间相关性、镜面反射特性、纹理真实性和差分成像。

Result: 互补特征有助于区分真实指纹与打印、数字和模制呈现攻击。研究还分析了成对采集的局限性，包括对成像设置的敏感性、数据集规模和新兴高保真欺骗攻击。

Conclusion: 研究证明了光照感知分析在提高接触式指纹呈现攻击检测鲁棒性和可解释性方面的潜力，激励未来在成对采集和物理信息特征设计方面的工作。
...
### [109] [Eye image segmentation using visual and concept prompts with Segment Anything Model 3 (SAM3)](https://arxiv.org/abs/2603.17715)
...
Method: 使用多样化的眼图像数据集进行评估，包括实验室环境的高分辨率高质量视频和野外采集的具有挑战性的TEyeD数据集。比较SAM3的视觉提示和概念提示模式与SAM2的性能差异。

Result: 在大多数情况下，无论是实验室数据集还是野外数据集，SAM3（无论是视觉提示还是概念提示）的表现都不如SAM2。SAM2不仅性能更好，而且处理速度更快。

Conclusion: SAM2仍然是眼图像分割的最佳选择。研究团队提供了适配的SAM3代码库，可以处理任意时长的视频。
...
TL;DR: 提出AFSS采样策略，动态决定每轮训练中哪些图像需要处理、哪些可以跳过，从而加速YOLO系列检测器的训练过程，同时提升精度。


<details>
  <summary>Details</summary>
Motivation: YOLO检测器虽然推理速度快，但训练过程耗时，因为每个epoch都要处理所有训练图像，即使很多图像已经充分学习。这与"你只看一次"的效率理念不符，因此探索YOLO是否真的需要在每个epoch都看到所有训练图像。

Method: 提出抗遗忘采样策略(AFSS)，通过计算每个训练图像的检测召回率和精度的最小值来衡量学习充分性，将图像动态分为简单、中等、困难三个等级。简单图像稀疏重采样，中等图像部分选择，困难图像每轮全采样。学习充分性定期更新，使检测器能自适应地关注信息丰富的图像。

Result: 在MS COCO 2017、PASCAL VOC 2007、DOTA-v1.0和DIOR-R等数据集上，AFSS为YOLO系列检测器实现了超过1.43倍的训练加速，同时提高了准确率。

Conclusion: AFSS策略有效解决了YOLO训练效率低下的问题，通过动态采样机制在加速训练的同时提升性能，验证了YOLO不需要在每个epoch都处理所有训练图像。
...
### [111] [SARE: Sample-wise Adaptive Reasoning for Training-free Fine-grained Visual Recognition](https://arxiv.org/abs/2603.17729)
*Jingxiao Yang,DaLin He,Miao Pan,Ge Su,Wenqi Zhang,Yifeng Hu,Tangwei Li,Yuke Li,Xuhong Zhang*

Main category: cs.CV

TL;DR: SARE是一个样本自适应推理框架，通过级联检索与推理设计，结合自反经验机制，在无需训练的情况下实现细粒度视觉识别，达到SOTA性能并显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有方法在利用大视觉语言模型进行细粒度视觉识别时存在两个主要问题：1）对所有样本采用相同的推理流程，不考虑识别难度差异，导致精度和效率不优；2）缺乏整合和重用错误特定经验的机制，导致在类似挑战性案例上重复失败。

Method: SARE采用级联设计，结合快速候选检索与细粒度推理，仅在必要时调用后者。在推理过程中，引入自反经验机制，利用过去的失败经验在推理时提供可迁移的判别性指导，无需参数更新。

Result: 在14个数据集上的广泛实验证实，SARE实现了最先进的性能，同时显著减少了计算开销。

Conclusion: SARE通过样本自适应推理和自反经验机制，有效解决了现有方法在细粒度视觉识别中的局限性，实现了精度与效率的平衡。
...
TL;DR: SynRL：通过合成视频数据教授视觉语言模型时间原语（方向、速度、状态跟踪），在15个基准测试中显著提升视频理解能力，仅用7.7K合成样本就超越165K真实世界样本的性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在视频理解中存在两个关键局限：1）现有数据集缺乏时间中心性，答案可以从孤立关键帧推断而不需要整体时间整合；2）专有模型生成的训练数据在基本时间感知（如运动方向、速度判断）上存在系统性错误。需要一种能有效教授时间原语的方法。

Method: 提出SynRL后训练框架，通过程序生成的合成视频教授时间原语（方向、速度、状态跟踪）。将时间理解分解为短期感知原语（速度、方向）和长期认知原语，通过代码生成视频构建7.7K CoT和7K RL样本，提供真实帧级标注。

Result: 在15个基准测试（时间定位、复杂推理、通用视频理解）上取得显著改进。仅用7.7K合成CoT样本就超越Video-R1的165K真实世界样本。证明从抽象合成模式学到的跟踪帧间变化、比较速度等基础时间技能能有效迁移到复杂真实场景。

Conclusion: 建立了视频后训练的新范式：通过精心设计的合成数据进行视频时间学习，为视频理解提供了更经济高效的扩展路径。合成数据在教授基础时间技能方面比大规模真实数据更有效。
...
TL;DR: MoBaNet：一种参数高效、模态平衡的对称融合框架，用于多模态遥感语义分割，通过跨模态提示注入适配器和差异引导门控融合模块实现，显著减少可训练参数并解决模态不平衡问题。
...
Method: 1. 基于冻结VFM骨干的对称双流架构；2. 跨模态提示注入适配器（CPIA）通过生成共享提示注入瓶颈适配器实现深度语义交互；3. 差异引导门控融合模块（DGFM）利用跨模态差异指导特征选择；4. 模态条件随机掩码（MCRM）策略在训练时掩码单一模态并施加硬像素辅助监督。

Result: 在ISPRS Vaihingen和Potsdam基准测试中达到最先进性能，相比完全微调显著减少可训练参数，验证了其鲁棒和平衡的多模态融合有效性。
...
TL;DR: DiffVP通过提取CT扫描与参考图像之间的语义差异作为视觉提示，引导LLM生成更准确的CT报告，避免冗余解剖背景干扰。


<details>
  <summary>Details</summary>
Motivation: 现有CT报告生成方法通常整体编码3D体积，无法区分信息性线索与冗余解剖背景，需要更有效的视觉条件机制。

Method: 提出差分视觉提示(DiffVP)，使用分层差异提取器捕获全局和局部语义差异，通过差异到提示生成器转换为可学习的视觉前缀标记，作为LLM的结构化条件信号。

Result: 在两个大规模基准测试中，DiffVP显著优于现有方法，BLEU-1-4平均提升+10.98和+4.36，在RadGenome-ChestCT上临床效能F1分数达到0.421。

Conclusion: DiffVP通过显式建模扫描与参考图像之间的语义差异，有效抑制不变解剖结构同时放大诊断相关视觉证据，无需显式病灶定位即可实现准确报告生成。
...
### [115] [Fine-Grained Post-Training Quantization for Large Vision Language Models with Quantization-Aware Integrated Gradients](https://arxiv.org/abs/2603.17809)
*Ziwei Xiang,Fanhu Zeng,Hongjian Fang,Rui-Qi Wang,Renxing Chen,Yanan Zhu,Yi Chen,Peipei Yang,Xu-Yao Zhang*

Main category: cs.CV

TL;DR: 本文提出了一种基于积分梯度的细粒度量化方法QIG，用于大型视觉语言模型的量化加速，将量化粒度从模态级别提升到token级别，显著提升了量化后的模型精度。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然在下游任务中表现出色，但存在巨大的计算和内存开销，阻碍了实际部署。现有的量化方法通常在模态级别衡量token敏感性，无法捕捉复杂的跨token交互，也无法在token级别定量衡量量化误差。

Method: 受机制可解释性中公理化归因的启发，提出了基于量化感知积分梯度的细粒度量化策略QIG。该方法利用积分梯度定量评估token敏感性，将量化粒度从模态级别提升到token级别，同时反映模态间和模态内的动态特性。

Result: 在多个LVLM模型上，在W4A8和W3A16设置下进行了广泛实验，结果表明该方法提高了模型在多个基准测试上的准确性，且延迟开销可忽略。在3位仅权重量化下，LLaVA-onevision-7B的平均准确率提高了1.60%，与全精度版本的差距缩小到仅1.33%。

Conclusion: 提出的QIG方法通过细粒度的token级别量化策略，有效解决了现有量化方法在捕捉跨token交互方面的不足，显著提升了量化后模型的性能，为大型视觉语言模型的实际部署提供了有效的量化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 为无纹理3D模型自动生成逼真且自洽的外观是数字内容创作的关键挑战。现有通用视频生成模型难以保持严格的几何一致性和外观稳定性，不适合高质量的3D重建。

Method: 将3D外观生成任务重构为几何条件视频扩散问题：给定3D网格，首先渲染和编码多模态几何特征，以像素级精度约束视频生成过程。还设计了用于下游重建任务的多阶段流水线，包含3D感知修复技术。

Result: 实验结果表明，该方法在视频一致性和最终重建质量方面优于现有方法。生成的视频不仅是高质量的动态预览，还可作为可靠的3D感知中间表示，用于UV纹理投影或监督神经渲染方法。

Conclusion: TAPESTRY框架能够从无纹理网格自动创建生产就绪的完整3D资产，通过几何条件视频扩散生成高质量的360度旋转视频，解决了3D外观生成的一致性问题。
...
### [117] [ResNet-50 with Class Reweighting and Anatomy-Guided Temporal Decoding for Gastrointestinal Video Analysis](https://arxiv.org/abs/2603.17784)
*Romil Imtiaz,Dimitris K. Iakovidis*

Main category: cs.CV

TL;DR: 基于ResNet-50帧分类器和解剖引导时序事件解码的多标签胃肠道视频分析系统，通过剪裁类权重和时序优化策略，在挑战测试集上将时序mAP从0.3801提升至0.4303。
...
Method: 1. 使用ResNet-50作为帧分类器处理336x336尺寸的帧；2. 采用剪裁类权重训练损失解决类别不平衡；3. 结合GT风格帧级事件组合、解剖投票平滑、基于解剖的病理门控和保守滞后解码器进行时序事件解码。

Result: 在挑战测试集上，时序mAP从0.3801提升至0.4303，表明剪裁类权重和优化的时序解码策略有效提升了系统性能。

Conclusion: 提出的多标签胃肠道视频分析管道通过剪裁类权重解决类别不平衡，结合解剖引导的时序解码策略，显著提升了罕见病理检测的准确性和整体系统性能。
...
TL;DR: C2P是一个无需提示的通用医学图像分割框架，通过解耦几何和语义表示，利用MLLM提取语义token，引入几何token强制执行物理约束，实现跨模态的精确分割。


<details>
  <summary>Details</summary>
Motivation: 现有通用医学图像分割方法过度依赖手动视觉提示或参考图像，限制了自动化和鲁棒性；同时跨模态联合训练难以处理大的领域偏移。

Method: 提出C2P框架：1) 将解剖知识解耦为几何和语义表示；2) 利用MLLM提取可学习的语义token；3) 引入显式监督的几何token强制执行物理约束；4) 解耦的token与图像特征深度交互生成动态核进行掩码预测；5) 引入几何感知推理共识机制评估预测可靠性。

Result: 在包含7种模态、8个数据集的统一基准测试中，C2P显著优于通用或单模型方法；在零样本任务和跨模态迁移中表现出强大的泛化能力。

Conclusion: C2P通过解耦几何和语义表示，结合MLLM的知识提取和几何约束强制执行，实现了无需提示、鲁棒的跨模态医学图像分割，在泛化性和性能上均有显著提升。
...
### [119] [ChopGrad: Pixel-Wise Losses for Latent Video Diffusion via Truncated Backpropagation](https://arxiv.org/abs/2603.17812)
...
<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型通过循环帧处理实现高质量生成，但在像素域训练时，由于激活值在整个视频序列中累积，导致内存成本过高。这一根本限制使得使用像素级损失对长视频或高分辨率视频进行微调在计算上不可行。

Method: 提出ChopGrad截断反向传播方案，将梯度计算限制在局部帧窗口内，同时保持全局一致性。该方法通过理论分析验证了这种近似的有效性，使其能够使用帧级损失进行高效微调。

Result: ChopGrad将训练内存从与视频帧数线性缩放（完整反向传播）降低到恒定内存。在多种条件视频生成任务中表现优异，包括视频超分辨率、视频修复、神经渲染场景的视频增强以及受控驾驶视频生成。

Conclusion: ChopGrad解决了视频扩散模型训练中的内存瓶颈问题，使得使用像素级损失对长视频和高分辨率视频进行高效微调成为可能，同时在多个视频生成任务中保持了与现有最先进模型相当的性能。
...
### [120] [PC-CrossDiff: Point-Cluster Dual-Level Cross-Modal Differential Attention for Unified 3D Referring and Segmentation](https://arxiv.org/abs/2603.17753)
*Wenbin Tan,Jiawen Lin,Fangyong Wang,Yuan Xie,Yong Xie,Yachao Zhang,Yanyun Qu*

Main category: cs.CV

TL;DR: PC-CrossDiff：用于3D视觉定位的双任务框架，通过点级和聚类级差分注意力机制解决复杂多物体场景中的隐式定位线索解析和空间干扰抑制问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉定位方法在简单单物体场景中表现良好，但在现实世界中常见的复杂多物体场景中性能严重下降，主要面临两个挑战：1）难以解析区分视觉相似物体的隐式定位线索；2）无法有效抑制共现物体的动态空间干扰。

Method: 提出PC-CrossDiff统一双任务框架，包含：1）点级差分注意力模块，通过文本与点云的双向差分注意力自适应提取隐式定位线索；2）聚类级差分注意力模块，建立分层注意力机制，增强定位相关空间关系同时抑制模糊或无关空间关系。

Result: 在ScanRefer、NR3D和SR3D基准测试中达到最先进性能。特别是在ScanRefer的隐式子集上，3DREC任务的Overall@0.50分数提升了+10.16%，显示出强大的隐式空间线索解析能力。

Conclusion: PC-CrossDiff通过创新的双级差分注意力架构，有效解决了复杂多物体场景中的3D视觉定位挑战，显著提升了隐式定位线索的解析能力和空间干扰抑制效果，推动了3D视觉定位在实际应用中的部署。
...
### [121] [Evidence Packing for Cross-Domain Image Deepfake Detection with LVLMs](https://arxiv.org/abs/2603.17761)
*Yuxin Liu,Fei Wang,Kun Li,Yiqi Nie,Junjie Chen,Zhangling Duan,Zhaohong Jia*

Main category: cs.CV

TL;DR: 提出SCEP框架，无需微调大型视觉语言模型，通过证据驱动的推理进行图像深度伪造检测，优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 虽然大型视觉语言模型具有强大的图像理解能力，但将其应用于图像深度伪造检测通常需要昂贵的微调，且对多样化、不断演变的伪造操作泛化能力差。

Method: 提出语义一致证据包（SCEP）框架：1）使用视觉编码器的CLS标记作为全局参考；2）将补丁特征聚类成连贯组；3）使用融合度量（CLS引导的语义不匹配+频率和噪声异常）对补丁评分；4）从每个聚类中采样高置信度补丁并应用基于网格的NMS，生成证据包；5）用证据包条件化冻结的LVLM进行预测。

Result: 在多样化基准测试中，SCEP无需LVLM微调即优于强基线方法。

Conclusion: SCEP提供了一种无需微调的训练免费LVLM框架，通过证据驱动的推理有效检测图像深度伪造，具有更好的泛化能力。
...
### [122] [CrowdGaussian: Reconstructing High-Fidelity 3D Gaussians for Human Crowd from a Single Image](https://arxiv.org/abs/2603.17779)
*Yizheng Song,Yiyu Zhuang,Qipeng Xu,Haixiang Wang,Jiahe Zhu,Jing Tian,Siyu Zhu,Hao Zhu*

Main category: cs.CV

TL;DR: CrowdGaussian：从单张图像直接重建多人3D高斯泼溅表示的框架，通过自监督适应处理遮挡，使用自校准学习提升渲染质量


<details>
  <summary>Details</summary>
Motivation: 现有单视图3D人体重建研究主要关注清晰、近距离的单人图像，在更常见的多人场景中表现不佳。多人重建面临三大挑战：1) 大量遮挡 2) 低清晰度 3) 多样外观

Method: 提出CrowdGaussian统一框架：1) 自监督适应管道，使预训练大模型能从严重遮挡输入重建完整3D人体 2) 自校准学习策略，通过混合身份保持样本与干净/损坏图像对，让单步扩散模型自适应优化粗糙渲染到最佳质量，输出可蒸馏回多人3DGS表示

Result: 大量实验表明，CrowdGaussian能够生成照片级真实感、几何连贯的多人场景重建

Conclusion: CrowdGaussian成功解决了多人3D重建中的遮挡、低清晰度和外观多样性挑战，实现了从单张图像直接重建高质量多人3D高斯泼溅表示
...
### [123] [LoST: Level of Semantics Tokenization for 3D Shapes](https://arxiv.org/abs/2603.17995)
*Niladri Shekhar Dutt,Zifan Shi,Paul Guerrero,Chun-Hao Paul Huang,Duygu Ceylan,Niloy J. Mitra,Xuelin Chen*

Main category: cs.CV

TL;DR: LoST提出了一种基于语义显著性的3D形状tokenization方法，通过语义对齐损失实现高效的自回归3D生成，仅需0.1%-10%的token就能达到SOTA效果。


<details>
  <summary>Details</summary>
Motivation: 当前3D生成中的tokenization主要依赖为渲染和压缩设计的几何层次细节(LoD)方法，这些方法token效率低且缺乏语义连贯性，不适合自回归建模。
...
Result: LoST在几何和语义重建指标上大幅超越之前的LoD-based方法，实现SOTA重建；仅需0.1%-10%的token就能实现高效高质量的AR 3D生成，并支持语义检索等下游任务。

Conclusion: LoST通过语义驱动的tokenization解决了3D形状tokenization的开放性问题，为自回归3D生成提供了更高效、语义更连贯的表示方法。
...
TL;DR: 本研究系统比较了三种方法在畜牧行为分类中的表现：从头训练、冻结特征提取和参数高效微调（PEFT）。结果表明PEFT（特别是QLoRA）在仅使用2.72%参数的情况下达到83.16%的测试准确率，优于其他方法。


<details>
  <summary>Details</summary>
Motivation: 精准畜牧业中的自动化行为分类面临高计算成本和有限标注数据的挑战。需要找到在保持高性能的同时降低计算需求的方法。

Method: 使用2,160张训练图像，系统比较三种方法：1）从头训练（ResNet-18、ViT-Small）；2）冻结DINOv3特征提取；3）参数高效微调（PEFT），包括QLoRA和DoRA，测试不同秩（8、16、64）和目标模块（q_proj vs 所有线性层）。在211,800个测试样本上评估泛化能力。

Result: PEFT显著优于其他方法：最佳QLoRA配置（所有线性层，秩=64）达到83.16%测试准确率，仅使用2.72%参数（183.0M），训练时间5.8小时。相比之下，ResNet-18为72.87%（16.8小时），ViT-Small为61.91%（18.7小时），冻结DINOv3为76.56%（17.5小时）。DoRA达到类似准确率（83.14%）但训练时间更长（11.0小时）。增加适配器容量持续改善泛化而不导致过拟合。

Conclusion: 在农业图像中适配基础模型时，欠拟合而非过拟合是主要挑战。PEFT（特别是QLoRA）为在农业畜牧业应用中部署数十亿参数视觉模型提供了有效指导，能够在保持高性能的同时显著降低计算成本。
...
### [125] [Unified Spatio-Temporal Token Scoring for Efficient Video VLMs](https://arxiv.org/abs/2603.18004)
*Jianrui Zhang,Yue Yang,Rohun Tripathi,Winson Han,Ranjay Krishna,Christopher Clark,Yong Jae Lee,Sangho Lee*

Main category: cs.CV

TL;DR: STTS是一种简单轻量的时空令牌评分模块，可在ViT和LLM中修剪视觉令牌，无需文本条件或令牌合并，通过端到端训练实现50%令牌修剪，效率提升62%，性能仅下降0.7%。


<details>
  <summary>Details</summary>
Motivation: 现有令牌修剪方法要么只在ViT中修剪（仅适用于单模态感知任务），要么只在LLM中修剪（需要复杂的文本条件机制），缺乏统一、无需文本条件的跨架构视觉令牌修剪方法。

Method: 提出STTS模块，通过学习时间维度的辅助损失和空间维度的LLM下游梯度来评分令牌，配合高效的打包算法，在ViT和LLM中同时修剪视觉令牌，完全兼容端到端训练。

Result: 在13个长短视频QA任务上，修剪50%视觉令牌，训练和推理效率提升62%，平均性能仅下降0.7%。效率增益随视频采样帧数增加而增加，长视频QA测试时缩放可额外获得0.5-1%性能提升。

Conclusion: STTS是一种新颖、简单而有效的统一跨架构视觉令牌修剪技术，显著提升计算效率同时保持性能，特别适合视频任务中的时空冗余处理。
...
### [126] [Differential Attention-Augmented BiomedCLIP with Asymmetric Focal Optimization for Imbalanced Multi-Label Video Capsule Endoscopy Classification](https://arxiv.org/abs/2603.17879)
*Podakanti Satyajith Chary,Nagarajan Ganapathy*

Main category: cs.CV

TL;DR: 提出用于视频胶囊内窥镜的多标签分类框架，通过差分注意力机制和多种类别不平衡处理策略，在Galar数据集上取得显著性能提升


<details>
  <summary>Details</summary>
Motivation: 视频胶囊内窥镜（VCE）数据存在极端类别不平衡问题，病理发现仅占所有标注帧的不到0.1%，需要专门的方法来处理这种不平衡并提高分类性能

Method: 1. 改进BiomedCLIP模型，用差分注意力机制替代标准多头自注意力，通过计算两个softmax注意力图的差异来抑制注意力噪声
2. 采用sqrt-frequency加权采样器、非对称焦点损失、mixup正则化和每类阈值优化来处理类别不平衡
3. 通过中值滤波平滑和间隙合并来增强时间一致性，生成事件级JSON输出

Result: 在RARE-VISION测试集（3个NaviCam检查，161,025帧）上，整体时间mAP@0.5达到0.2456，mAP@0.95达到0.2353，单GPU上总推理时间约8.6分钟

Conclusion: 提出的多标签分类框架通过结合架构改进和优化策略，有效解决了VCE数据中的极端类别不平衡问题，在保持高效推理的同时取得了有竞争力的性能
...
TL;DR: 提出Mask-to-Point (M2P)学习方法，利用视频对象分割标注改进视觉基础模型，用于密集点跟踪任务，在TAP-Vid-DAVIS基准上显著超越DINOv2/3模型。


<details>
  <summary>Details</summary>
Motivation: 当前基于静态图像预训练的视觉基础模型（如DINOv2）在捕捉视频密集时间对应关系方面存在固有不足，需要改进以适应点跟踪任务。

Method: 提出M2P学习方法，包含三个基于掩码的弱监督约束：局部结构一致性损失（使用Procrustes分析）、掩码标签一致性损失（强制前景点匹配前景区域）、掩码边界约束（显式监督边界点）。

Result: 仅使用3.6K个VOS训练视频，M2P在TAP-Vid-DAVIS基准上分别比DINOv2-B/14和DINOv3-B/16提升12.8%和14.6%性能，可作为点跟踪任务的通用预训练模型。

Conclusion: M2P通过利用视频对象分割标注进行弱监督学习，有效改进了视觉基础模型在密集点跟踪任务上的性能，展示了作为点跟踪通用预训练模型的潜力。
...
TL;DR: 本文提出STAS方法，通过引导视频扩散变换器中罕见的高幅度激活信号来提升视频生成质量，无需额外训练且计算开销极小。
...
Method: 提出Structured Activation Steering（STAS）方法，这是一种无需训练的自引导式方法。基于观察到的MA模式：首帧token幅度最大，潜在帧边界token（潜在空间时间块的首尾部分）幅度次之，内部token幅度适中。STAS引导首帧和边界token的MA值朝向缩放后的全局最大参考幅度。

Result: STAS在不同文本到视频模型中实现了视频质量和时间一致性的持续改进，同时引入的计算开销可忽略不计。

Conclusion: 视频扩散变换器中的Massive Activations具有结构化模式，反映了模型对潜在空间时间分块的隐式优先级。利用这一模式设计的STAS方法能有效提升视频生成质量，为视频生成模型优化提供了新思路。
...
### [129] [VideoAtlas: Navigating Long-Form Video in Logarithmic Compute](https://arxiv.org/abs/2603.17948)
*Mohamed Eltahir,Ali Habibullah,Yazan Alshoibi,Lama Ayash,Tanveer Hussain,Naeemullah Khan*

Main category: cs.CV

TL;DR: VideoAtlas：一种任务无关的视频层次化网格表示方法，结合递归语言模型实现对数级计算增长的长视频理解


<details>
  <summary>Details</summary>
Motivation: 现有视频语言模型面临两大挑战：表示方法依赖有损近似，长上下文处理通过文本转换丢失视觉保真度。需要一种无损、可导航、可扩展且无需预处理的方法来处理长视频。

Method: 提出VideoAtlas作为任务无关的环境，将视频表示为层次化网格，支持无损递归缩放。结合递归语言模型，采用Master-Worker并行架构：Master协调全局探索，Worker并发深入特定区域收集视觉证据。

Result: 1) 计算复杂度随视频时长对数增长，网格结构复用带来30-60%多模态缓存命中率；2) 通过限制最大探索深度实现计算-精度权衡；3) 计算分配随问题粒度自适应扩展。在1小时到10小时视频上保持最稳健的性能。

Conclusion: 结构化环境导航是视频理解的可扩展范式，VideoAtlas通过层次化网格表示和递归语言模型实现了无损、对数级计算增长的长视频处理能力。
...
TL;DR: 本文提出TINA（Text-free INversion Attack），一种无需文本引导的DDIM反演攻击方法，能够绕过当前基于文本的概念擦除防御，证明现有擦除方法只是掩盖概念而非真正消除视觉知识。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像扩散模型的概念擦除技术主要关注切断文本到图像的映射关系，但忽略了底层视觉知识仍然存在。作者认为这种文本中心的擦除范式存在根本缺陷，需要从视觉角度验证擦除效果。

Method: 提出TINA方法：1) 使用DDIM反演在擦除后的模型中寻找视觉生成路径；2) 采用零文本条件避免触发文本中心防御；3) 引入优化过程解决无文本引导时累积的近似误差问题。

Result: 实验表明TINA能够从经过最先进遗忘学习处理的模型中重新生成被擦除的概念，证明当前擦除方法只是掩盖概念而非真正消除内部视觉知识。

Conclusion: 现有概念擦除方法存在根本缺陷，仅关注文本映射而忽略视觉知识保留。需要开发直接操作内部视觉知识的新范式，TINA为此提供了验证工具和方向指引。
...
TL;DR: 这篇综述论文系统梳理了视频理解领域，从三个互补视角组织文献：低层视频几何理解、高层语义理解、统一视频理解模型，并总结了从孤立任务特定方法向统一建模范式的转变趋势。


<details>
  <summary>Details</summary>
Motivation: 视频理解需要建模时间动态和演化视觉上下文，对时空推理要求更高，是计算机视觉的基础问题。现有文献缺乏系统性的结构化概述，需要整合不同视角来提供连贯的研究地图。

Method: 采用结构化综述方法，将视频理解文献组织为三个互补视角：1）低层视频几何理解（几何结构、运动分析等）；2）高层语义理解（动作识别、事件检测等）；3）统一视频理解模型（通用视频基础模型）。

Result: 提供了视频理解领域的连贯研究地图，总结了关键建模趋势和设计原则，识别了从孤立任务特定流水线向统一建模范式的广泛转变，这种范式能够适应多样下游目标。

Conclusion: 通过整合三个互补视角，该综述为视频理解领域提供了系统性概览，总结了向统一视频基础模型发展的趋势，并指出了构建鲁棒、可扩展统一视频基础模型的开放挑战。
...
TL;DR: Omni-3DEdit是一个统一的、基于学习的3D编辑模型，能够隐式地泛化各种3D编辑任务，无需耗时的在线优化，将推理时间从数十分钟缩短到约两分钟。


<details>
  <summary>Details</summary>
Motivation: 现有基于指令驱动的3D编辑方法依赖2D模型指导3D表示的显式和迭代优化，存在两个主要问题：1）缺乏不同3D编辑任务的通用设计，因为3D几何的显式操作需要任务依赖的规则；2）迭代优化过程非常耗时，通常需要数千次2D/3D更新调用。

Method: 1）构建数据流水线，合成相对丰富的高质量配对多视图编辑样本；2）采用预训练生成模型SEVA作为骨干，在序列空间中连接源视图潜变量和条件标记；3）提出双流LoRA模块来解耦不同视图线索，大幅增强模型的表示学习能力。

Result: 作为基于学习的模型，Omni-3DEdit无需耗时的在线优化，可以在一次前向传播中完成各种3D编辑任务，将推理时间从数十分钟减少到约两分钟。大量实验证明了该方法的有效性和效率。

Conclusion: Omni-3DEdit通过统一的、基于学习的方法解决了传统3D编辑方法的局限性，实现了高效、通用的3D编辑，为3D内容创作提供了更实用的解决方案。
...
TL;DR: 该论文全面评估了细胞分割基础模型（CellPoseSAM、CellSAM、μSAM）和通用分割模型（SAM、SAM2、SAM3）在多种显微镜数据集上的表现，并提出了自动提示生成（APG）策略来改进μSAM的分割效果。


<details>
  <summary>Details</summary>
Motivation: 细胞分割是显微镜图像分析的基础任务，虽然已有多个基于SAM的细胞分割基础模型，以及更新的SAM2和SAM3模型，但缺乏对这些模型在显微镜数据上的全面评估。需要开发更好的策略来改进这些模型在显微镜图像分割中的应用。

Method: 1. 在多样化的（光学）显微镜数据集上评估细胞分割基础模型（CellPoseSAM、CellSAM、μSAM）和通用分割模型（SAM、SAM2、SAM3）；2. 提出自动提示生成（APG）策略，用于改进基于SAM的显微镜基础模型；3. 将APG应用于μSAM作为基础模型。

Result: APG策略能持续改进μSAM的分割结果，使其与最先进的CellPoseSAM模型竞争。研究为SAM类模型在显微镜领域的适应策略提供了重要经验，并为创建更强大的显微镜基础模型提供了策略。

Conclusion: 该研究全面评估了现有分割基础模型在显微镜图像分析中的表现，提出的APG策略能有效改进μSAM的分割性能，为开发更强大的显微镜基础模型提供了实用指导和方法。
...
TL;DR: 研究比较了不同人类显著性先验（手绘标注、眼动热图、分割掩码、DINOv2嵌入）在开放集虹膜呈现攻击检测中的效果，发现去噪眼动热图在AUROC和APCER指标上表现最佳。
...
Method: 通过一系列实验比较手绘标注、眼动热图、分割掩码和DINOv2嵌入与最先进的深度学习基线方法，采用留一攻击类型出范式进行开放集PAD评估。

Result: 在开放集PAD任务中，去噪眼动热图在AUROC和APCER@BPCER=1%指标上表现出最佳泛化改进，优于交叉熵基线和其他显著性方法。

Conclusion: 去噪眼动热图是开放集虹膜呈现攻击检测中最有效的人类显著性形式，为后续研究提供了模型、代码和显著性图资源。
...
TL;DR: 论文提出EditSpilloverProbe框架，将图像编辑模型中的"编辑溢出"现象重新定义为探测世界知识的自然探针，揭示了编辑控制与世界理解之间的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 图像编辑模型在实践中存在普遍的"编辑溢出"现象——模型会修改编辑区域外语义相关但未指定的内容。这引发了一个根本问题：溢出是反映了真正的隐式世界理解，还是仅仅是注意力泄漏？

Method: 提出了EditSpilloverProbe系统框架：1) 定义溢出分类法（空间、语义、混合、随机）；2) 开发自动化检测和分类流程；3) 构建基于真实世界中文文本编辑任务的基准数据集EditSpilloverBench；4) 系统评估5个代表性编辑模型。

Result: 1) 不同架构的溢出率差异显著（3.49%到11.46%，3.3倍比率）；2) 语义溢出数量反映世界理解能力——nano_banana产生最多语义溢出（每图27.8），而qwen_2511编辑控制最精确但语义溢出较少（每图16.3）；3) 空间衰减分析显示语义相关溢出比例保持恒定（40%-58%），证明语义溢出反映真实世界理解而非空间扩散。

Conclusion: 编辑溢出可作为评估图像编辑模型世界理解能力的有效探针，揭示了编辑精度与世界理解之间的权衡关系，语义溢出反映了模型真正的世界知识而非简单的注意力泄漏。
...
TL;DR: 提出一个统一的身份感知音视频生成框架，通过数据自动提取和身份注入机制，实现多身份的面部外观和声音音色的细粒度控制。


<details>
  <summary>Details</summary>
Motivation: 现有技术虽然能将真实个体合成到生成视频中，但缺乏开放可访问的框架来实现跨多个身份的面部外观和声音音色的细粒度控制，这限制了身份感知内容创作的进一步发展。

Method: 1) 设计数据自动提取流水线，从音视频模态中提取身份信息并配对标注；2) 提出灵活可扩展的身份注入机制，将面部外观和声音音色作为身份控制信号；3) 采用多阶段训练策略加速收敛并增强跨模态一致性。

Result: 实验证明了该框架的优越性，能够实现高保真和一致的身份个性化，支持从单主体到多主体交互的多样化场景。

Conclusion: 该工作提出了一个统一且可扩展的身份感知音视频生成框架，解决了现有技术中缺乏细粒度身份控制的问题，为身份感知内容创作提供了有效的解决方案。
...
TL;DR: CAT框架通过创造性分词器将模糊提示转化为可重用的创造性语义令牌，无需重复推理即可增强文本到图像生成的创造力，实现3.7倍加速和4.8倍计算成本降低。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型依赖离散自然语言提示，对模糊创意提示理解有限，而基于推理或代理的方法需要重复计算，成本高昂且不可重用。

Method: 提出CAT框架，包含创造性分词器，通过创造性语义解耦训练，利用部分重叠概念对的关系捕获代理的潜在创造性表示，生成可重用的令牌模板。

Result: 在建筑设计、家具设计和自然混合任务上，CAT相比最先进方法实现3.7倍加速和4.8倍计算成本降低，生成图像在人类偏好和图文对齐方面表现更优。

Conclusion: CAT为增强文本到图像生成的创造力提供了可扩展且有效的范式，通过可重用创造性令牌避免了重复推理，显著提升了效率和效果。
...
TL;DR: SpiderCam是一个基于FPGA的实时稀疏深度相机，能在32.5FPS下生成480x400深度图，总功耗仅624mW，工作范围52厘米。


<details>
  <summary>Details</summary>
Motivation: 开发一种低功耗的实时3D感知系统，用于移动设备和嵌入式应用，解决现有3D相机功耗过高的问题。

Method: 使用定制相机同时捕捉两个不同焦距的图像，通过SystemVerilog在FPGA上实现改进的差分离焦深度计算算法，采用内存本地化设计以适应小尺寸设备。

Result: 实现了32.5FPS的实时深度计算，总功耗仅624mW，工作范围52厘米，成为文献中首个亚瓦特功耗的被动FPGA 3D相机。

Conclusion: SpiderCam展示了通过算法改进和硬件优化，可以在极低功耗下实现实时3D感知，为移动和嵌入式应用提供了可行的解决方案。
...
TL;DR: 提出一个基于变分自编码器的半灰盒、噪声感知异常检测框架，用于检测边缘AI协同推理中的恶意数据注入攻击


<details>
  <summary>Details</summary>
Motivation: 边缘AI协同推理中，资源受限的终端设备将部分处理数据卸载到边缘服务器完成端到端处理，但这种边缘卸载容易受到恶意数据注入攻击，导致隐蔽的错误分类，特别是在存在环境噪声的情况下难以检测

Method: 提出基于变分自编码器(VAE)的半灰盒、噪声感知异常检测框架，包含鲁棒的噪声感知特征，能够捕捉环境噪声的特征行为

Result: 在流行的目标分类DNN上评估，在真实噪声条件下实现了高达90%的AUROC检测性能，同时揭示了特征相似性和高噪声水平带来的限制

Conclusion: 该框架能够有效检测边缘AI协同推理中的对抗性操纵，提高检测准确率并降低误报率，但需要注意特征相似性和高噪声水平的挑战
...
TL;DR: 提出几何驱动的2D-3D-2D范式，通过多视角冗余自动传播标注，构建大规模无人机RGB和RGB-T语义分割数据集SegFly


<details>
  <summary>Details</summary>
Motivation: 现有无人机RGB和RGB-T数据集规模小、多样性不足、标注成本高，且商用无人机难以实现精确的RGB-T对齐

Method: 利用高重叠航拍图像的多视角冗余，将少量人工标注的RGB图像提升为语义3D点云，再重投影到所有视角，自动生成RGB和热成像的伪真值标注；同时使用3D几何作为中间对齐空间实现跨模态图像配准

Result: 构建了SegFly数据集，包含2万多张高分辨率RGB图像和1.5万多对几何对齐的RGB-T图像对；自动生成97%的RGB标注和100%的热成像标注，准确率达91%和88%；RGB-T配准准确率达87%

Conclusion: 几何驱动的2D-3D-2D流水线能够实现可扩展的多模态场景理解，SegFly数据集显著提升了传统架构和视觉基础模型的性能
...
TL;DR: 提出一个用于锁骨CT扫描法律年龄估计的可解释多阶段管道，结合自动锁骨检测、集成梯度引导切片选择和保形预测，在法医数据集上实现最先进性能（MAE 1.55年）。


<details>
  <summary>Details</summary>
Motivation: 法律年龄估计在法医和医学法律环境中至关重要，需要准确、稳健且可重复的方法。虽然现有AI方法主要关注手部X光或牙科影像，但锁骨CT扫描尽管在年龄估计中有效，却尚未得到充分探索。

Method: 提出三阶段管道：1) 基于特征的连通分量方法自动检测锁骨，需要最少手动标注；2) 集成梯度引导切片选择策略构建多切片卷积神经网络的输入数据；3) 使用保形预测区间提供不确定性量化。

Result: 在1,158个全身死后CT扫描数据集上评估，最终模型在测试集上达到MAE 1.55±0.16年，优于人类专家（约1.90年）和先前方法（同一数据集上MAE超过1.75年）。保形预测支持可配置的覆盖率水平。

Conclusion: 该方法实现了锁骨CT扫描法律年龄估计的最先进性能，具有可解释性和不确定性量化，已集成到Skeleton-ID软件中，旨在作为多因素法医工作流程的决策支持组件。
...
TL;DR: 提出C-TRAIL多模态法律数据集和两阶段框架，将行车记录仪视频转化为法律责任判定，填补视频证据与法律分析之间的空白


<details>
  <summary>Details</summary>
Motivation: 行车记录仪普及使交通事故视频证据丰富，但将视频内容转化为法律责任判定仍依赖人工专家。现有研究要么关注视频感知理解，要么基于文本案例的法律分析，缺乏结合视频证据的法律责任判定方法

Method: 1) 提出C-TRAIL数据集：在中国交通法规体系下对齐行车记录仪视频、文本描述、责任模式及对应法规；2) 两阶段框架：交通事故理解模块生成视频文本描述 + 法律多智能体框架输出责任模式、法规集和完整判决报告

Result: 在C-TRAIL和MM-AU数据集上实验表明，该方法优于通用和法律大语言模型及现有智能体方法，同时提供透明可解释的法律推理过程

Conclusion: 成功填补了视频证据与法律分析之间的空白，提出的多模态法律数据集和两阶段框架能够有效将行车记录仪视频转化为法律责任判定，具有实际应用价值
...
TL;DR: TransText：首个基于Alpha-as-RGB范式的方法，无需修改预训练生成模型即可实现图层感知的文字动画生成，解决了透明通道与RGB语义先验的兼容性问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法将透明通道作为额外潜在维度附加到RGB空间，需要重建VAE，但高质量透明文字数据稀缺，重新训练VAE计算成本高且可能破坏从海量RGB数据学习的语义先验，导致潜在模式混合。

Method: 提出TransText框架，采用Alpha-as-RGB范式，通过潜在空间拼接将alpha通道嵌入为RGB兼容的视觉信号，明确确保跨模态（RGB和Alpha）一致性，同时防止特征纠缠，无需修改预训练生成流形。

Result: 实验表明TransText显著优于基线方法，能够生成连贯、高保真的透明动画，具有多样化和精细的效果。

Conclusion: TransText通过创新的Alpha-as-RGB范式成功解决了图层感知文字动画生成的挑战，在保持预训练模型语义先验的同时实现了高质量的透明动画生成。
...
TL;DR: LaDe是一个分层媒体设计生成框架，通过潜在扩散模型生成可编辑的RGBA图层，支持文本到图层、图像到图层和设计分解三种任务，在文本对齐方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么限制图层数量固定，要么要求每个图层只包含空间连续区域，导致图层数量随设计复杂度线性增长，无法生成语义上有意义的灵活图层结构。

Method: LaDe包含三个组件：1) LLM提示扩展器将简短用户意图转换为结构化图层描述；2) 带4D RoPE位置编码的潜在扩散Transformer联合生成完整设计和RGBA图层；3) RGBA VAE解码每个图层并支持完整alpha通道。

Result: 在Crello测试集上，LaDe在文本到图层和图像到图层任务中优于Qwen-Image-Layered，通过GPT-4o mini和Qwen3-VL评估器验证，在文本到图层对齐方面表现更好。

Conclusion: LaDe框架能够生成灵活数量的语义有意义图层，支持多种媒体设计任务，在图层生成质量和对齐方面优于现有方法，为可编辑媒体设计生成提供了有效解决方案。
...
TL;DR: 本文提出Robust-ComBat方法，通过MLP补偿异常值，解决传统ComBat在包含病理病例时产生的失真问题，在多站点扩散MRI数据中实现更可靠的协调。


<details>
  <summary>Details</summary>
Motivation: 传统ComBat协调方法假设数据呈高斯分布，但在临床实践中，神经疾病患者常呈现偏离正常值的扩散指标，形成病理异常值，这会扭曲站点效应估计。由于大多数患者在接受脑成像时尚未确诊，难以将其排除在协调队列之外。

Method: 提出Robust-ComBat方法，使用简单的多层感知器（MLP）提供稳健的异常值补偿。该方法在包含病理病例的情况下仍能实现可靠的协调，同时保留疾病相关信号。研究评估了10种异常值拒绝方法和4种ComBat变体。

Result: 在7种神经疾病条件下，Robust-ComBat在所有ComBat变体中均优于传统统计基线，具有更低的协调误差。在包含高达80%神经疾病患者的真实多站点队列中表现一致优异。

Conclusion: Robust-ComBat通过MLP异常值补偿，解决了传统ComBat在包含病理病例时的失真问题，为临床多站点扩散MRI数据协调提供了更可靠的解决方案。
...
TL;DR: AHOY：一种从被严重遮挡的单目视频中重建完整、可动画的3D高斯化身的方法，通过身份微调的扩散模型生成未观察身体区域的密集监督。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设输入无遮挡（完全可见的主体，通常处于规范姿势），排除了绝大多数真实世界视频（人物经常被家具、物体或其他人遮挡）。从这类视频重建面临基本挑战：大部分身体区域可能从未被观察到，且每个姿势缺乏多视角监督。

Method: 提出四个关键贡献：(1) 幻觉作为监督的流程，使用身份微调的扩散模型为先前未观察的身体区域生成密集监督；(2) 两阶段规范到姿势依赖的架构，从稀疏观察引导到完整的姿势依赖高斯图；(3) 地图姿势/LBS姿势解耦，吸收生成数据中的多视角不一致性；(4) 头/身体分离监督策略，保留面部身份。

Result: 在YouTube视频和具有显著遮挡的多视角捕捉数据上评估，展示了最先进的重建质量。生成的化身足够稳健，可以用新姿势进行动画，并合成到使用手机视频捕获的3DGS场景中。

Conclusion: AHOY方法能够从被严重遮挡的单目视频中成功重建完整、可动画的3D高斯化身，解决了现有方法无法处理遮挡视频的限制，为真实世界视频中的人物重建提供了有效解决方案。
...
TL;DR: 提出雷达数据自适应压缩方案，通过梯度下降动态调整压缩率，结合DCT变换和量化技术，实现100倍特征尺寸缩减且性能损失仅约1%
...
Method: 1) 基于检测置信度代理梯度进行梯度下降动态调整压缩率；2) 使用零阶梯度近似避免传输梯度张量；3) 对雷达数据立方体应用DCT变换并选择性剪枝；4) 通过缩放量化保持雷达补丁的动态范围

Result: 在RADIal、CARRADA和Radatron数据集上验证，实现超过100倍特征尺寸缩减，性能下降仅约1个百分点

Conclusion: 提出的自适应反馈雷达数据压缩方案能有效解决高维雷达数据通信瓶颈，在保持检测性能的同时大幅减少数据传输量
...
TL;DR: Motion-MLLM通过引入IMU数据增强多模态大语言模型，利用运动信息进行3D场景理解和空间推理，在保持高精度的同时显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在3D空间推理中通常依赖计算昂贵的3D表示（如点云或BEV地图），或缺乏物理基础来解决尺度和大小模糊性。需要一种更高效且具有物理基础的方法。

Method: 提出Motion-MLLM框架，包含两个关键组件：1）级联运动-视觉关键帧过滤模块，利用IMU数据和视觉特征选择稀疏但有代表性的关键帧；2）非对称跨模态融合模块，运动令牌作为中介将自我运动线索和跨帧视觉上下文注入视觉表示。

Result: Motion-MLLM在3D场景理解和空间推理任务中显著改进，相比基于视频帧和显式3D数据的SOTA方法，在相似或更高精度下显著降低开销（成本效益分别提高1.40倍和1.63倍）。

Conclusion: 通过将视觉内容锚定在物理自我运动轨迹中，Motion-MLLM能够推理绝对尺度和场景空间关系，为高效且物理基础的3D场景理解提供了新方向。
...
TL;DR: DynaEdit是一种无需训练的通用视频编辑方法，基于预训练文本到视频流模型，能够修改动作、插入交互对象和引入全局效果，解决了现有方法在复杂编辑任务中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法存在严重限制：训练模型因缺乏相关训练数据而难以处理复杂编辑，无训练方法只能保持结构和运动不变，无法修改运动或交互行为。

Method: 基于无反转方法（不干预模型内部），提出DynaEdit框架，通过新颖机制解决低频错位和高频抖动问题，实现模型无关的通用视频编辑。

Result: DynaEdit在复杂文本视频编辑任务上达到最先进水平，包括修改动作、插入与场景交互的对象、引入全局效果等，解决了现有方法的局限性。

Conclusion: DynaEdit展示了无训练方法在复杂视频编辑任务中的潜力，为基于预训练模型的通用视频编辑提供了有效解决方案，克服了数据收集和编辑灵活性方面的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 合成可控的6-DOF物体操作轨迹对于机器人交互复杂3D环境至关重要，但现有方法依赖2D或部分3D表示，限制了完整场景几何捕捉和轨迹精度。

Method: 提出GMT多模态Transformer框架，将轨迹表示为连续6-DOF姿态序列，采用定制条件策略融合几何、语义、上下文和目标导向信息。
...
TL;DR: 提出无需训练、基于文本嵌入空间导向的连续可控图像编辑框架，通过语言模型自动构建对比提示对计算导向向量，实现平滑编辑控制。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要额外训练或人工干预，我们希望开发一种无需训练、自动化的连续可控图像编辑方法，仅通过文本嵌入空间的简单导向即可实现平滑编辑。

Method: 使用大语言模型自动构建去偏对比提示对，在生成器的文本编码器空间中计算导向向量；提出弹性范围搜索程序自动确定有效导向强度区间，避免欠导向和过导向；通过缩放同一向量实现连续编辑。

Result: 方法在连续编辑行为上可与基于训练的方法相媲美，优于其他无需训练的方法；引入新评估指标量化导向连续性，证明方法能产生平滑连续的编辑效果。

Conclusion: 仅通过文本嵌入空间的简单导向即可实现连续可控的图像编辑，无需额外训练；方法轻量、通用，可自然扩展到文本条件图像和视频生成。
...
TL;DR: EchoGen是一个统一的布局到图像生成和图像定位框架，通过渐进式训练策略实现两个任务的协同优化，在多个基准测试中达到最先进性能。
...
Method: 提出渐进式训练策略：1) 并行多任务预训练阶段，利用共享token加速训练；2) 双重联合优化阶段，利用任务对偶性顺序整合两个任务；3) 循环强化学习阶段，使用一致性约束作为奖励，通过GRPO策略显著增强模型的统一能力。

Result: 在布局到图像生成和图像定位基准测试中均取得了最先进的结果，并显示出两个任务协同优化的明显增益。

Conclusion: EchoGen框架通过渐进式训练策略成功解决了联合训练中的优化挑战，实现了布局到图像生成和图像定位任务的协同提升，证明了两个任务联合优化的有效性。
...
TL;DR: SkeletonLLM：通过将骨架序列转换为视觉模态，使多模态大语言模型能够理解和处理非视觉的骨架数据，实现通用骨架理解。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型局限于原生视觉-语言模态，无法直接处理非视觉的结构化数据如人体骨架。现有方法要么将骨架动态压缩为有损特征向量，要么量化为离散标记，在不同骨架格式间泛化能力差。

Method: 1. 提出DrAction可微分渲染器，将骨架运动学转换为紧凑图像序列；2. 引入协同训练策略：因果推理蒸馏从教师模型迁移结构化推理能力，判别性微调增强混淆动作的决策边界。

Result: SkeletonLLM在识别、描述、推理和跨格式迁移等多种任务上表现出强大的泛化能力，为将MLLM应用于非原生模态提供了可行路径。

Conclusion: 通过将任意骨架序列转换为MLLM的原生视觉模态，SkeletonLLM实现了通用骨架理解，展示了将MLLM应用于非视觉结构化数据的可行性。
...
TL;DR: 评估大语言模型（ChatGPT-4o、Perplexity AI、GeminiAI）在提供可靠且易懂的孕期健康信息方面的表现，发现Perplexity在语义上与专家最接近，而ChatGPT-4o在清晰度和医学术语使用上更优。


<details>
  <summary>Details</summary>
Motivation: 印度农村地区由于医疗资源和基础设施有限，获取可靠的孕产保健信息面临挑战。随着互联网普及（8.3亿用户，近半数农村女性上网），数字工具为健康教育提供了新机会。

Method: 向三个大语言模型（ChatGPT-4o、Perplexity AI、GeminiAI）提出17个孕期相关问题，并与孕产健康专家的回答进行比较。使用语义相似度、名词重叠度和可读性指标评估内容质量。

Result: Perplexity在语义上与专家回答最接近，而ChatGPT-4o生成的文本更清晰易懂，医学术语使用更好。随着农村互联网普及，大语言模型可作为孕产健康教育的可扩展辅助工具。

Conclusion: 研究表明大语言模型有潜力作为农村地区孕产健康教育的辅助工具，但需要平衡准确性和清晰度。随着互联网普及，AI工具可改善服务不足地区的医疗沟通。
...
<details>
  <summary>Details</summary>
Motivation: 金融分析师面临从冗长的10-K报告（通常超过100页）中提取信息的重大挑战，需要高效的工具来辅助信息检索和分析。

Method: 采用混合检索（全文检索+语义检索）的RAG系统，可选地使用交叉编码器模型进行神经重排序，在FinDER基准数据集（包含1,500个查询）上进行系统评估。

Result: 重排序显著提升答案质量：得分8分以上的正确率从33.5%提升至49.0%（提升15.5个百分点），完全错误答案的错误率从35.3%降至22.5%。

Conclusion: 重排序在金融RAG系统中起关键作用，通过现代语言模型和优化的检索策略，系统性能优于基线方法。
...
TL;DR: 提出基于评分标准的推理框架，通过不确定性校准来提升语音评估模型与人类评分者的一致性


<details>
  <summary>Details</summary>
Motivation: 当前大型语音语言模型在第二语言语音评估中难以与人类评分者的细微差异对齐，需要更可靠和可解释的自动化评估方法

Method: 使用评分标准引导的推理框架，明确编码准确性、流利度和韵律等多方面评估标准；采用Qwen2-Audio-7B-Instruct模型微调，结合多评分者人类判断；开发不确定性校准回归方法，支持置信区间解释

Result: 高斯不确定性建模和符合校准方法在人类评分对齐方面表现最佳，优于回归和分类基线；模型能可靠评估流利度和韵律，但准确性评估仍具挑战性

Conclusion: 评分标准引导、不确定性校准的推理为构建可信赖、可解释的语音语言模型语音评估提供了原则性路径
...
TL;DR: 该研究提出了一个NL2SQL系统的鲁棒性评估基准，包含约10种扰动类型，在传统和智能体两种设置下评估了多个SOTA LLM，发现模型在多数扰动下表现稳定，但在字符级噪声和语义保留的语言变体上性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 现实世界的数据库环境是动态、嘈杂且不断演变的，而传统的NL2SQL基准评估通常假设静态模式和格式良好的用户输入，因此需要建立鲁棒性评估框架来测试系统在实际环境中的表现。

Method: 引入包含约10种扰动类型的鲁棒性评估基准，在传统管道和智能体两种设置下评估多个SOTA LLM（包括Grok-4.1、Gemini-3-Pro、Claude-Opus-4.6和GPT-5.2），分析不同扰动类型对性能的影响。

Result: 模型在多数扰动下表现稳定，但在表面级噪声（如字符级损坏）和语义保留的语言变体（词汇或句法形式改变）上性能显著下降。表面级噪声对传统管道影响更大，而语言变体对智能体设置挑战更大。

Conclusion: NL2SQL系统在鲁棒性方面仍面临挑战，特别是在处理语言变异性方面。研究结果强调了需要开发更强大的系统来处理现实世界中的动态和嘈杂环境。
...
TL;DR: 论文分析现有大语言模型评估方法在定义模糊任务上的局限性，通过复杂指令遵循和自然语言转Mermaid序列图两个案例研究，揭示当前评估存在覆盖不足、指标不一致、LLM评委不稳定等问题，提出需要更稳健、可解释的评估设计。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的许多评估针对的是定义模糊的任务，这些任务输入输出空间不明确，成功标准模糊。现有评估基准和指标未能为这类任务提供可靠或诊断性的模型能力信号，需要分析其失败原因并改进评估方法。

Method: 采用两个案例研究：1) 复杂指令遵循任务，分析其存在的覆盖范围有限、对指令措辞敏感、指标不一致且不可比较、LLM评委引入的不稳定性等问题；2) 自然语言转Mermaid序列图任务，展示多维度评估标准如何提供超越聚合分数的可操作见解。

Result: 研究发现当前评估经常混淆不同的失败模式，导致分数不稳定、无法诊断且难以采取行动。复杂指令遵循评估存在系统性缺陷，而自然语言转Mermaid序列图的多维度评估能提供更深入的洞察。这些发现暴露了现有评估实践的根本局限性。

Conclusion: 现有评估方法在定义模糊任务上存在根本缺陷，需要设计更稳健、可解释的评估方案。多维度评估标准比单一聚合分数更有价值，未来评估应避免混淆不同失败模式，提供更诊断性的信号来指导模型改进。
...
TL;DR: 研究发现现代推理大语言模型中的跨语言知识迁移主要受文字脚本障碍影响，而非语言或语系差异。通过改进推理机制可以显著减少跨脚本知识迁移差距。
...
Method: 1) 在ECLeKTic和MultiLoKo数据集上进行观察性数据分析；2) 回归分析确定脚本匹配是知识迁移失败的主要预测因素；3) 提供源语言关键实体测试模型表现；4) 开发合成生成管道创建SFT样本，训练模型更好处理音译歧义。

Result: 回归分析显示脚本匹配（而非语言或语系）是知识迁移失败的主要预测因素；提供源语言关键实体能显著改善跨脚本问题；通过SFT训练改进推理机制可减少跨脚本知识迁移差距。

Conclusion: 跨语言参数知识迁移的主要障碍是文字脚本差异，通过后训练改进模型的推理机制可以有效提升跨脚本知识转移能力。
...
TL;DR: 提出基于集成学习的自训练框架用于无监督神经机器翻译，通过多模型共享监督提升性能，同时保持单模型推理成本


<details>
  <summary>Details</summary>
Motivation: 无监督机器翻译通常依赖单模型，性能有限。本文旨在通过集成学习提升无监督翻译质量，同时避免增加推理时的计算成本

Method: 1) 训练多个共享主要语言对但使用不同辅助语言的UNMT模型；2) 使用词级集成解码生成伪翻译；3) 将集成输出作为合成平行数据进一步训练每个模型；4) 部署时根据验证性能选择单个模型

Result: 相比单模型UNMT基线获得统计显著改进：英译其他语言平均提升1.7 chrF，其他语言译英平均提升0.67 chrF

Conclusion: 集成驱动的自训练框架能有效提升无监督机器翻译性能，通过多模型协作生成更高质量的伪翻译数据，同时保持部署时的单模型推理效率
...
TL;DR: CoCoEval框架评估LLM模拟对话中的不一致和不合作行为，发现LLM对话比人类对话更少此类行为，提示工程难以可靠控制，微调会导致行为过度产生，凸显模拟人类对话的困难。


<details>
  <summary>Details</summary>
Motivation: 模拟人类对话对建模社会互动很重要，但现有研究对LLM模拟对话中不一致和不合作行为的分析有限，而这些行为是模拟真实人类社交互动的关键。

Method: 提出CoCoEval评估框架，使用LLM-as-a-Judge方法在对话轮次层面检测10种不一致和不合作行为，评估GPT-4.1、GPT-5.1和Claude Opus 4在学术、商业、政府会议和辩论场景中的表现。

Result: 1) 基础提示下LLM对话比人类对话少很多不一致不合作行为；2) 提示工程无法可靠控制这些行为，不同提示会导致行为不足或过度产生；3) 在人类对话上微调会使LLM过度产生特定行为（如重复）。

Conclusion: 模拟人类对话具有挑战性，LLM难以准确再现人类社交互动中的复杂行为，对使用LLM作为人类社交互动代理表示担忧。
...
TL;DR: 利用MoE LLMs中的跨语言不一致性进行知识定位，通过对比成功和失败语言的路由模式来识别负责特定知识的关键专家模块


<details>
  <summary>Details</summary>
Motivation: 现代LLMs在不同语言间存在行为差异，通常被视为需要缓解的问题。本文提出利用这种跨语言不一致性作为MoE LLMs可解释性工具，将问题转化为机会。

Method: 两阶段方法：1) 在多语言中查询困难事实问题，生成"成功"和"失败"激活桶；2) 对MoE路由器logits进行统计对比分析，识别对知识重要的专家。通过停用这些专家验证其必要性。

Result: 仅停用约20个专家（共6000个），模型在超过40%的情况下不再正确回答问题。证明该方法能有效定位负责特定知识的关键专家模块。

Conclusion: 该方法提供了一种现实且可扩展的知识定位方法，能够应对日益复杂的LLMs，为模型可解释性研究提供了新视角。
...
TL;DR: 提出基于英语语法档案（EGP）的框架，用于检测二语学习者的语法结构尝试并分类为成功/不成功，同时用于CEFR水平评估，结合规则和LLM方法。


<details>
  <summary>Details</summary>
Motivation: 评估二语学习者的语法能力对于提供针对性反馈和评估熟练度至关重要，但现有方法可能未能充分利用学习者的成功尝试来提供积极反馈。

Method: 使用英语语法档案（EGP）作为语法结构分类框架，结合规则基和LLM基分类器检测语法结构尝试，并开发混合管道用于熟练度评估，同时评估自动语法纠错的效果。

Result: LLM在语义和语用细微差别的语法结构上优于规则方法，而规则方法在纯形态或句法特征上仍有竞争力；混合方法（规则预过滤+LLM）表现最佳；自动纠错管道接近基于人工修正的半自动系统性能。

Conclusion: 该框架强调学习者的成功尝试，能够提供积极的形成性反馈，并为语法发展提供可操作的见解，结合规则和LLM方法的混合管道在语法结构检测和熟练度评估方面表现最佳。
...
TL;DR: TAP-GPT：基于TableGPT2的表格专用LLM框架，用于阿尔茨海默病少样本分类，在缺失数据处理和可解释性方面表现优异


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病诊断需要处理表格生物标志物数据，但这类数据通常样本小、不完整，传统深度学习方法难以超越经典方法。预训练大语言模型具有少样本泛化、结构化推理和可解释输出等优势，为临床预测提供了新范式。

Method: 提出TAP-GPT框架，基于TableGPT2进行领域适应，使用表格提示而非纯文本进行微调，专门用于阿尔茨海默病的少样本分类。在四个ADNI数据集上评估，包括QT-PAD生物标志物、结构MRI、淀粉样蛋白PET和tau PET等多模态数据。

Result: TAP-GPT在多模态和单模态设置下均优于其骨干模型和传统机器学习基线，在少样本场景表现优异。特征选择缓解了高维输入的性能下降，无需插补即可在模拟和真实缺失数据下保持稳定性能。模型产生结构化、模态感知的推理，与已知AD生物学一致，且在自反思下表现更稳定。

Conclusion: 这是首次系统地将表格专用LLM应用于多模态生物标志物的AD预测，证明预训练模型能有效处理结构化临床预测任务，为基于表格LLM的多智能体临床决策支持系统奠定了基础。
...
TL;DR: CODMAS是一个用于自动化RTL优化的多智能体框架，通过结构化辩证推理、领域感知代码生成和确定性评估，在流水线和时钟门控优化中显著降低关键路径延迟和功耗。


<details>
  <summary>Details</summary>
Motivation: RTL代码优化对提高芯片的功耗、性能和面积(PPA)至关重要，但传统方法依赖人工经验且效率有限。需要自动化工具来提升优化质量和效率。

Method: CODMAS采用辩证多智能体系统：Articulator智能体通过逐步转换计划揭示潜在假设；Hypothesis Partner智能体预测结果并协调偏差；Domain-Specific Coding Agent生成架构感知的Verilog代码；Code Evaluation Agent验证语法、功能和PPA指标。

Result: 在RTLOPT基准测试中，CODMAS在流水线优化中减少约25%的关键路径延迟，在时钟门控中减少约22%的功耗，同时相比基线方法减少了功能和编译失败。
...
TL;DR: SYMDIREC是一个神经符号框架，通过符号规划分解RTL任务，结合微调检索器和LLM推理，在硬件设计自动化的RTL综合和总结任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在RTL任务上面临挑战：硬件描述语言语法严格、监督数据有限、与自然语言对齐弱。现有提示和检索增强生成方法缺乏符号规划，限制了结构精度。

Method: SYMDIREC将RTL任务分解为符号子目标，通过微调检索器检索相关代码，利用LLM推理组装验证输出。支持Verilog和VHDL，无需LLM微调。

Result: 在RTL综合任务上Pass@1率提高约20%，在总结任务上ROUGE-L指标提升15-20%，优于提示和检索增强生成基线方法。

Conclusion: SYMDIREC证明了符号引导在RTL任务中的有效性，神经符号框架能显著提升硬件设计自动化的性能。
...
TL;DR: 提出基于本地LLM的文本匿名化方法，通过类型一致的替代品替换PII，在保护隐私的同时保持语义实用性和可训练性，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在LLM时代，需要在保护敏感信息的同时不损害数据实用性。现有方法要么隐私保护不足，要么过度损害数据价值，需要一种平衡隐私保护与数据实用性的解决方案。

Method: 开发本地LLM驱动的替代管道，在组织边界内执行，用类型一致的现实替代品替换PII。评估采用多指标跨技术方法，包括隐私、语义效用、可训练性评估，并在问答代理前插入匿名化层。

Result: 方法在隐私保护、主题漂移控制、事实效用和可训练性损失方面达到最优，优于微软Presidio、谷歌DLP和ZSTS等基准方法，在隐私-效用-可训练性综合边界上表现最佳。

Conclusion: 本地LLM替代方法能生成既负责任又有操作价值的匿名化语料，既安全用于代理管道，又适合下游微调且性能损失有限，实现了隐私保护与数据实用性的平衡。
...
TL;DR: 对齐模型在预测人类行为方面存在规范偏差：在多轮策略游戏中表现不如基础模型，但在单轮规范游戏中表现更好


<details>
  <summary>Details</summary>
Motivation: 研究后训练对齐是否真正让语言模型能够准确预测人类行为，特别是在复杂的多轮策略互动中

Method: 比较120个基础-对齐模型对在10,000多个真实人类决策上的表现，涵盖议价、说服、谈判和重复矩阵游戏等多轮策略场景

Result: 基础模型在多轮策略游戏中预测人类选择的能力比对齐模型强近10倍；但对齐模型在单轮教科书游戏和非策略彩票选择中表现更好

Conclusion: 对齐会引入规范偏差：在人类行为符合规范预测时改善预测，但在多轮策略互动中损害预测能力，揭示了优化模型供人类使用与作为人类行为代理之间的根本权衡
...
TL;DR: 本文提出Tharu-LLaMA (3B)模型和TharuChat数据集，通过LLM-to-Human引导方法为资源匮乏的塔鲁语创建专门指令跟随模型，解决全球南方土著语言在AI革命中被排除的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的快速发展造成了深刻的数字鸿沟，将全球南方的土著语言排除在AI革命之外。塔鲁语作为印度-雅利安语系方言，约有170万使用者，面临严重的数据稀缺和语言碎片化问题，导致现有多语言模型经常产生"幻觉"或默认使用印地语、尼泊尔语等高资源语言。

Method: 采用LLM-to-Human引导管道构建TharuChat数据集，利用经过提示工程优化的Gemini模型，结合Rana Tharu语法和民间传说合成训练数据。数据集反映了该地区嘈杂、异质的语言现实，主要基于Rana Tharu方言（约70%），同时整合Dangaura和Kochila方言元素。

Result: 实证消融研究表明，尽管数据集存在方言混合和残余Awadhi/印地语影响等缺陷，小规模合成数据非常有效：数据集容量从25%增加到100%时，困惑度从6.42线性降低到2.88。模型可在消费级硬件上实现。

Conclusion: Tharu-LLaMA模型作为概念验证，展示了通过生成式AI保护资源匮乏的喜马拉雅地区语言的可行性，为其他低资源语言提供了可复制的框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前大音频语言模型虽然能生成富有表现力的语音，但可靠的情感控制仍然难以实现：情感转换经常偏离目标情感，并且可能通过拒绝、幻觉或改写等方式降低语言保真度。

Method: 通过成功过滤的激活聚合方法识别情感敏感神经元，该方法同时强制情感实现和内容保持。在推理时对这些神经元进行干预，实现无需训练的情感控制。

Result: 在三个大音频语言模型（Qwen2.5-Omni-7B, MiniCPM-o 4.5, Kimi-Audio）上，ESN干预产生了情感特定的增益，能够泛化到未见过的说话者，并通过自动和人工评估得到支持。

Conclusion: 研究结果为语音生成中的无需训练情感控制建立了机制框架，可控性取决于选择器设计、掩码稀疏度、过滤和干预强度等因素。
...
TL;DR: CulT-Eval是一个用于评估机器翻译系统处理文化表达（如成语、俚语、文化特定项目）能力的基准测试，包含7,959个实例，揭示了当前模型在保留文化含义方面的系统性问题。


<details>
  <summary>Details</summary>
Motivation: 文化表达（成语、俚语、文化特定项目）在自然语言中普遍存在，其含义超越字面形式。现有机器翻译系统难以准确翻译这类表达，且现有评估基准碎片化，缺乏系统性的文化表达翻译评估框架。

Method: 构建CulT-Eval基准测试，包含7,959个精心策划的实例，涵盖多种文化表达类型，并建立全面的错误分类法。通过大规模语言模型评估和详细分析，识别现有自动指标未能捕捉的系统性失败模式，并针对文化诱导的意义偏差提出补充评估指标。

Result: 当前模型在保留文化基础含义和捕捉文化及上下文细微差别方面存在困难，无法准确翻译文化表达。研究识别了现有自动指标未能捕捉的重复性和系统性失败模式。

Conclusion: 需要专门的基准测试和评估指标来评估机器翻译系统处理文化表达的能力。CulT-Eval填补了这一空白，为系统评估文化表达翻译提供了框架，并揭示了当前模型的局限性。
...
TL;DR: 研究发现英语单个音素携带结构化、多维度的语义信号，挑战了语言中音义关系任意性的传统假设。


<details>
  <summary>Details</summary>
Motivation: 传统语言学认为词语的音义关系是任意的，但声音象征现象的证据不断积累。然而，尚无研究系统性地绘制语言中每个音位单位的多维度语义图谱。

Method: 使用涵盖220对字母对比的最小对范式，让三个大型语言模型独立恢复九个感知维度上一致的音素-意义关联。这些关联由发音-语音特征系统预测，发音方式和位置映射到不同的语义维度。通过英语母语者的行为数据验证，并初步考察五种类型学上不同的语言的跨语言证据。

Result: 英语母语者以远高于随机水平（80.8%）的准确率确认了这些模式。初步的跨语言证据表明核心映射关系超越了英语。大型语言模型仅凭文本输入（未接触语音或发音）就能恢复这些系统性的关联。

Conclusion: 声音-意义象似性不是偶尔的奇特现象，而是语音信号中普遍存在的结构化属性，其系统性如此之强，以至于大型语言模型仅凭文本输入就能恢复它。
...
TL;DR: Ruyi2.5是一个基于AI Flow框架的多模态家族模型，扩展了"一次训练，多处部署"范式到多模态领域，构建了共享骨干架构，在不同规模模型间保持语义一致性。基于此开发的Ruyi2.5-Camera是一个隐私保护相机服务系统，采用两阶段识别流程：边缘模型进行特征去标识化，云端模型进行深度行为推理。还提出了BPPO方法加速强化学习微调。


<details>
  <summary>Details</summary>
Motivation: 将"一次训练，多处部署"范式扩展到多模态领域，解决多模态模型在不同部署层级间的语义一致性问题。同时针对隐私保护相机服务场景，需要平衡视觉识别能力与隐私保护需求。

Method: 1) 构建共享骨干架构，在统一流程中协同训练不同规模的模型；2) 开发两阶段识别系统：边缘模型使用信息瓶颈引导的不可逆特征映射进行去标识化，云端模型进行深度行为推理；3) 提出BPPO方法，通过二进制响应选择减少样本冗余，专注于响应前缀的梯度更新。

Result: Ruyi2.5在通用多模态基准测试中与Qwen3-VL相当，而Ruyi2.5-Camera在隐私约束的监控任务中显著优于Qwen3-VL。BPPO方法相比GRPO实现了2-3倍的训练加速。

Conclusion: Ruyi2.5成功将"一次训练，多处部署"范式扩展到多模态领域，实现了跨部署层级的语义一致性。Ruyi2.5-Camera系统有效平衡了视觉识别与隐私保护需求，BPPO方法显著提升了强化学习微调效率。
...
<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型的空间推理能力，特别是通过纯文本方式隔离空间推理与视觉感知，了解模型在3D空间理解方面的真实能力。

Method: 创建GSU文本网格数据集，包含导航、物体定位和结构组合三个任务，测试不同模型（包括纯文本LLM、视觉语言模型VLMs）的表现，并尝试通过微调小模型来提升性能。

Result: 大多数模型能理解基本网格概念，但在相对参考系和从坐标列表识别3D形状方面表现不佳；视觉模态暴露不能为VLMs提供可泛化的3D空间理解；前沿模型能解决基本任务但困难变体仍有挑战，微调小模型可匹配前沿模型性能。

Conclusion: 纯文本空间推理评估揭示了LLM在3D空间理解方面的局限性，微调小模型可达到前沿模型性能，为专用具身智能体开发提供了可行路径。
...
TL;DR: PACE-RAG：针对帕金森病等复杂疾病的个性化药物推荐框架，通过结合患者个体特征与相似病例的处方模式，超越传统LLM和RAG方法，实现可解释的临床决策支持。


<details>
  <summary>Details</summary>
Motivation: 现有LLM虽然具备广泛医学知识，但无法捕捉实际处方模式的细微差别；传统RAG方法中，基于指南的检索过于通用，基于相似患者的检索则容易复制多数模式而忽略个体临床特异性。需要一种能综合患者个体特征与相似病例处方倾向的解决方案。

Method: 提出PACE-RAG框架，通过分析针对特定临床信号的治疗模式，识别最优处方并生成可解释的临床摘要。该方法将患者个体情境与相似病例的处方倾向相结合。

Result: 在帕金森病队列和MIMIC-IV基准测试中，使用Llama-3.1-8B和Qwen3-8B模型，PACE-RAG达到最先进性能，F1分数分别为80.84%和47.22%。

Conclusion: PACE-RAG是一个稳健、临床基础扎实的个性化决策支持解决方案，能够有效结合患者个体特征与相似病例的处方模式，为复杂疾病的药物推荐提供可解释的临床依据。
...
TL;DR: SafeTutors基准揭示LLM作为AI导师时存在教学安全风险：过度披露答案、强化错误概念、放弃支架式教学等，多轮对话中教学失败率从17.7%升至77.8%，且危害因学科而异。


<details>
  <summary>Details</summary>
Motivation: 当前LLM作为AI导师的评估只关注解题准确性和通用安全性，忽略了教学有效性与安全性在师生互动中的联合评估。教学安全风险不同于传统LLM安全风险，主要危害在于通过答案过度披露、错误概念强化和支架式教学放弃等方式悄然侵蚀学习过程。

Method: 引入SafeTutors基准，基于学习科学文献构建包含11个危害维度和48个子风险的理论基础风险分类法，在数学、物理和化学学科中联合评估安全性和教学效果。

Result: 所有模型都表现出广泛危害；模型规模不能可靠改善表现；多轮对话会恶化行为，教学失败率从单轮的17.7%上升到多轮的77.8%；危害因学科而异，单轮"安全/有帮助"的结果可能掩盖系统性导师失败。

Conclusion: 教学安全需要专门评估框架，现有LLM作为AI导师存在系统性风险，需要学科感知的缓解措施，多轮互动评估至关重要，不能依赖单轮评估结果。
...
TL;DR: LLMs通过学习论证重构可以提升批判性思维能力，作者提出了GAAR自动重构引擎、Arguinas数据集，并验证了学习论证重构对下游批判性思维任务的益处。


<details>
  <summary>Details</summary>
Motivation: 人类通过识别、重构和评估论证来培养批判性思维，论证重构尤其重要因为它使论证的底层推理显式化。然而，LLMs是否也能通过学习论证重构来增强批判性思维能力尚不清楚。

Method: 提出了一个包含三个贡献的整体框架：(1) GAAR自动重构任意论证的引擎；(2) 使用GAAR引擎合成新的高质量论证重构数据集Arguinas；(3) 研究学习论证重构是否有利于下游批判性思维任务。

Result: 实验结果显示，在七个批判性思维任务中，学习论证重构的模型表现优于未学习的模型，当在Arguinas数据集上训练时观察到最大的性能提升。

Conclusion: LLMs通过学习论证重构可以增强批判性思维能力，GAAR引擎和Arguinas数据集为此提供了有效工具，论证重构训练对提升模型在批判性思维任务上的表现有显著益处。
...
TL;DR: 提出MSL理论度量来量化推理链的最小必要长度，并基于此开发TRiMS方法，在保持准确性的同时减少80%以上的推理token消耗。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通过长思维链实现复杂推理，但这导致严重的推理膨胀和计算冗余。需要最大化"每个token的智能"来优化推理效率。

Method: 1. 提出MSL理论度量，定义保持答案正确性的最短推理长度；2. 分析主流CoT压缩策略的结构因素；3. 提出TRiMS方法，结合GRPO算法和MSL估计进行训练，通过动态批次聚合和基于批次标准差的优势计算来稳定训练过程。

Result: TRiMS在所有基准测试中实现了超过80%的CoT token减少，同时带来轻微准确率提升。

Conclusion: MSL为推理链压缩提供了首个可测量的下界，TRiMS方法有效实现了推理效率的显著提升，为最大化"每个token的智能"提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索基于Transformer的语言模型如何学习语言类别，通过比较其训练过程中的行为与人类语言习得中的抽象特征理论和具体样例理论，了解语言模型学习机制的认知基础。

Method: 使用GPT-2小模型，通过基于分歧的新度量方法追踪训练过程中的学习轨迹，分析下一个词分布的变化，研究词汇语义和句法类别的涌现过程。

Result: 实验发现：(1)当学习一个结构时，抽象类别行为比具体词汇行为出现得更早；(2)不同语言行为在训练过程中依次突然出现，表明抽象化在语言模型学习中扮演关键角色。

Conclusion: 语言模型的学习过程支持抽象化在语言习得中的重要性，为语言模型作为人类语言习得模型的存在证明提供了依据，表明抽象类别表征在语言学习中具有优先性。
...
TL;DR: L2A是一种学习选择性全局注意力的层，通过让模型决定何时需要长程记忆访问，在保持性能的同时显著减少计算开销，将模型上下文从32K扩展到128K。


<details>
  <summary>Details</summary>
Motivation: 语言模型难以泛化到预训练上下文长度之外，限制了长程推理和检索能力。传统长上下文训练因注意力机制的二次方计算复杂度而代价高昂。

Method: 提出L2A层，基于观察发现大多数token不需要全局注意力，只需局部上下文。L2A让模型学习条件性（按token决定）的长程记忆访问，决定何时调用全局注意力。

Result: 在Qwen 2.5和Qwen 3模型上，将有效上下文从32K扩展到128K，性能与标准长上下文训练相差3%以内，同时为约80%的token跳过全局注意力。定制Triton内核实现2倍训练吞吐量提升，KV缓存内存减少50%。

Conclusion: L2A通过条件性全局注意力机制，在保持性能的同时显著降低了长上下文训练的计算和内存开销，为高效扩展模型上下文长度提供了有效方案。
...
TL;DR: 本文提出HypoTermInstruct数据集，通过教授模型认识自身知识局限性的元认知技能，有效减少LLM幻觉，同时保持基础能力稳定。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生幻觉，输出流畅但虚假的信息，部分原因是监督微调（SFT）隐含地奖励模型总是回应。需要教授模型认识自身知识局限性的元认知能力。

Method: 引入HypoTermInstruct SFT数据集（31,487个回答对应11,151个问题），通过关于不存在的"假设"术语的问题来教授模型认识知识局限性。同时发布HypoTermQA-Enhanced基准测试，通过多重验证强化幻觉倾向评估。在Llama3.1-8B和Gemma3-4B模型上进行了800次受控LoRA SFT实验，测试100种微调配置。

Result: 用HypoTermInstruct替换通用指令数据显著提高了HypoTerm Score（中位数提升0.19%到25.91%）和FactScore（提升0.39%到0.86%），同时在MMLU上保持稳定性能（仅下降0.26%到0.35%）。
...
TL;DR: XBridge是一个组合式编码器-LLM-解码器架构，利用预训练翻译模型处理多语言任务，同时保留LLM作为英语核心处理通用知识，通过跨模型映射层和最优传输对齐实现多语言生成。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多语言性能上存在严重不平衡，虽然编码了大量跨语言知识，但在低资源或未见语言上表现不佳。预训练编码器-解码器翻译模型具有平衡的多语言能力，可以补充LLM的不足。

Method: 提出XBridge架构：将多语言理解和生成卸载到外部预训练翻译模型，LLM作为英语核心处理通用知识。引入轻量级跨模型映射层和基于最优传输的对齐目标，实现细粒度语义一致性。

Result: 在四个LLM上的多语言理解、推理、摘要和生成任务中，XBridge优于强基线方法，特别是在低资源和未见语言上表现突出，且无需重新训练LLM。

Conclusion: XBridge通过组合预训练翻译模型和LLM，有效解决了LLM多语言性能不平衡问题，在保持LLM核心能力的同时显著提升了低资源语言性能。
...
TL;DR: 该论文提出了一个全面的机器生成文本检测基准，评估了多种检测方法在两个数据集上的表现，发现现有方法在跨域迁移、跨LLM泛化和对抗鲁棒性方面存在局限。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速扩散，迫切需要稳健且可泛化的机器生成文本检测器。现有基准通常只在理想条件下评估单一检测器在单一数据集上的表现，未能解决跨域迁移、跨LLM泛化和对抗鲁棒性等关键问题。

Method: 构建了一个综合基准，在两个语料库（HC3：23,363个人类-ChatGPT对；ELI5：15,000个人类-Mistral-7B对）上评估了多种检测方法，包括经典分类器、微调的Transformer编码器（BERT、RoBERTa、ELECTRA、DistilBERT、DeBERTa-v3）、CNN、XGBoost风格计量模型、基于困惑度的检测器以及LLM作为检测器的提示方法。

Result: Transformer模型在分布内数据上达到近乎完美的性能，但在域转移下性能下降。XGBoost风格计量模型在保持可解释性的同时匹配了性能。基于LLM的检测器表现不佳且受生成器-检测器身份偏差影响。基于困惑度的方法表现出极性反转（现代LLM输出的困惑度低于人类文本），但校正后仍有效。没有方法能在跨域和跨LLM源的情况下稳健泛化。

Conclusion: 当前机器生成文本检测方法在跨域和跨LLM泛化方面存在显著局限，需要开发更稳健的检测方法来解决这些挑战。XGBoost风格计量模型在性能和可解释性之间提供了良好平衡，而基于困惑度的方法需要校正以适应现代LLM的特性。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够解释共振峰与发音动作之间关系的教学工具，并为语音生物反馈应用提供基础，帮助语音学学生、相关领域语言学家以及言语治疗从业者和患者。

Method: 基于40名英语母语者的超声舌头成像和声学数据，建立预测模型，通过前两个共振峰值预测舌头位移和形状，并开发了Shiny应用和实时舌头生物反馈原型软件。

Result: 建立了AURORA模型，能够根据共振峰预测舌头特征，开发了使模型更易访问的工具，并进行了定性评估，重点关注特定舌头特征。

Conclusion: AURORA模型成功建立了共振峰与发音动作之间的关系，为语音学教学和言语治疗应用提供了有价值的工具，通过开发的可访问工具将惠及更广泛的用户群体。
...
<details>
  <summary>Details</summary>
Motivation: 多语言语音大语言模型面临数据分布不平衡的挑战。完全共享的参数高效微调会导致语言间负向干扰，而完全语言特定的调优又限制了跨语言知识转移。需要解决这种稳定性-可塑性困境。
...
Conclusion: Zipper-LoRA通过细粒度共享和动态解耦机制，有效解决了多语言语音大语言模型中的稳定性-可塑性困境，为大规模多语言ASR提供了可靠框架。
...
TL;DR: KA2L框架通过分析LLM的潜在空间来评估其对特定知识点的掌握程度，构建模型未掌握的问题进行主动学习，显著降低标注和计算成本50%的同时提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前研究缺乏对LLM领域特定知识深度理解的分析，以及如何应用针对性主动学习来提升其专业能力。需要填补这一研究空白。

Method: 提出知识感知主动学习(KA2L)框架：1) 使用知识分布探测技术分析Transformer层的隐藏状态，识别LLM中已知和未知知识的分布；2) 提出隐藏状态解码方法，从潜在知识空间生成大量未知问题的自然语言表述。

Result: 在9个开源LLM上的实验表明：1) KA2L在两个开放域和一个垂直域数据集上显著降低50%的标注和计算成本；2) 同时获得更好的性能表现。

Conclusion: KA2L框架为LLM的主动学习策略提供了有价值的见解，通过聚焦模型尚未掌握的知识点来提高训练效率，减少已掌握信息的冗余学习。
...
TL;DR: 提出PPA感知的多智能体框架，通过集成EDA工具实现功能正确性和物理指标的联合优化，引入演化记忆机制支持持续改进


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法主要关注RTL代码生成的功能正确性，但忽视了关键的物理设计目标（功耗、性能、面积），需要将PPA指标纳入优化过程

Method: 提出工具集成的多智能体框架，包含程序员代理、正确性代理和PPA代理的闭环工作流，引入演化记忆机制将优化经验外部化为结构化记忆节点，由专用内存管理器动态维护

Result: 实验表明该方法在保持强功能正确性的同时，显著改善了PPA指标，将RTL生成从一次性推理转变为持续、反馈驱动的优化过程

Conclusion: 通过集成工具驱动反馈和结构化可演化记忆，该框架为在实际硬件设计流程中部署LLM提供了可扩展的途径
...
TL;DR: 研究通过线性探测和机制可解释性技术分析LLM中语义关系的编码，发现层次关系存在方向不对称性，关系信号在中间层最强，且在不同规模模型中表现出相似难度模式。


<details>
  <summary>Details</summary>
Motivation: 理解大型语言模型是否捕捉结构化意义，需要研究它们如何表示概念关系。本研究旨在探究不同规模模型中四种语义关系（同义、反义、上位、下位）的编码方式和位置。

Method: 结合线性探测与机制可解释性技术，包括稀疏自编码器和激活修补，分析Pythia-70M、GPT-2和Llama 3.1 8B三个不同规模模型。通过识别关系编码位置和特定特征对表示的贡献来研究语义关系。

Result: 发现层次关系存在方向不对称性：上位关系编码冗余且抵抗抑制，下位关系依赖紧凑特征更易被破坏。关系信号分散但具有稳定模式：在中间层达到峰值，在残差后/MLP路径中比注意力中更强。不同模型难度一致（反义最容易，同义最难）。探针级因果关系与模型容量相关：在Llama 3.1中，SAE引导的修补能可靠地改变这些信号，而在较小模型中变化微弱或不稳定。

Conclusion: 研究阐明了语义关系在LLM内部表示的位置和可靠性，并提供了一个可复现的框架，将稀疏特征与探针级因果证据联系起来，为理解LLM如何编码结构化语义关系提供了新见解。
...
TL;DR: ARAM提出了一种针对掩码扩散模型的无训练自适应引导框架，通过信噪比动态调整检索上下文的引导强度，解决检索增强生成中的噪声和冲突问题。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成（RAG）虽然能提高事实基础性，但当检索到的上下文存在噪声、不可靠或与模型参数知识冲突时，会导致检索-先验冲突，降低生成质量。这一问题在自回归语言模型中已有研究，但在基于扩散的语言模型中尚未充分探索，因为其迭代去噪过程给检索上下文整合带来了独特挑战。

Method: 提出自适应检索增强掩码扩散（ARAM），这是一个针对掩码扩散模型的无训练自适应引导框架。ARAM根据检索上下文引起的分布偏移的信噪比，在去噪过程中动态校准引导尺度。当检索上下文提供可靠纠正证据时增强引导，当上下文信号有噪声或不支持时抑制引导。

Result: 在多个知识密集型问答基准测试上的广泛实验表明，ARAM在整体问答性能上优于竞争性的RAG基线方法。

Conclusion: ARAM框架有效解决了扩散语言模型中检索上下文整合的挑战，通过自适应引导机制提高了检索增强生成的质量和可靠性。
...
TL;DR: 提出一个多模态、多语言的幽默有害性检测基准，包含文本、图像和视频数据，强调文化背景和隐含推理的重要性。


<details>
  <summary>Details</summary>
Motivation: 当前静态基准无法捕捉依赖文化细微差别和隐含线索的黑色幽默，存在安全挑战，需要新的评估方法。

Method: 构建包含3000文本、6000图像和1200视频的多模态多语言数据集，采用严格标注指南区分安全、显性有害和隐性有害幽默，系统评估SOTA模型。

Result: 闭源模型显著优于开源模型，英语和阿拉伯语性能存在明显差距，凸显需要基于文化的推理感知安全对齐。

Conclusion: 需要开发能够理解文化背景和隐含推理的幽默安全检测系统，当前模型在跨语言和多模态任务上仍有不足。
...
<details>
  <summary>Details</summary>
Motivation: 现有无标签强化学习方法依赖多数投票作为伪标签，但存在"共识陷阱"问题：随着训练最大化自一致性，输出多样性崩溃，模型会自信地强化系统性错误。
...
Conclusion: CoVerRL通过生成器和验证器的协同进化机制，成功避免了无标签强化学习中的共识陷阱，实现了更稳健的能力提升。
...
TL;DR: 提出基于信息论自动生成推理步骤标签的新方法，降低计算复杂度至O(N)，用于改进大语言模型的多步推理可靠性


<details>
  <summary>Details</summary>
Motivation: 多步推理能提升大语言模型能力，但会增加错误传播风险。现有过程奖励模型训练方法依赖昂贵的人工标注或计算密集的自动标注，需要更高效、可扩展的解决方案。

Method: 利用信息论自动生成步骤级标签，估计每个推理步骤如何影响正确答案的可能性，提供步骤质量信号。将计算复杂度从O(N log N)降低到O(N)。

Result: 该方法在多种推理基准测试（数学、Python编程、SQL、科学问答）中有效支持链式思维选择，在最佳K评估设置中表现良好。

Conclusion: 这项工作实现了对大语言模型推理的可扩展和高效监督，特别适用于错误传播关键的任务，为自动评估和改进推理过程提供了新途径。
...
TL;DR: 研究语言模型从无结构化文本进行空间推理的能力，提出叙事到舞台布局的任务，通过结合拒绝式监督微调和强化学习的方法提升模型性能。
...
Method: 引入戏剧学启发的确定性评估套件，提出结合拒绝式监督微调（使用Best-of-N采样）和基于可验证奖励的强化学习（通过GRPO）的训练和推理方案。

Result: 在古典英语文学纯文本语料上的实验表明，相比基础模型在多个指标（角色归属、空间合理性、移动经济性）上有改进，并与LLM作为评判者和主观人类偏好保持一致。

Conclusion: 该方法有效提升了语言模型从无结构化文本进行空间推理的能力，为自动化舞台布局推断提供了可行方案。
...
TL;DR: NEVU是一个面向新闻事件的人类价值观理解基准，支持基于角色的、事件中心的、方向感知的价值识别，包含54个细粒度价值观和20个粗粒度类别，覆盖45,793个单位-角色对和168,061个有向价值实例。


<details>
  <summary>Details</summary>
Motivation: 现有的人类价值观数据集在事实新闻的价值理解方面存在不足：许多数据集是角色无关的，依赖孤立的陈述或合成场景，缺乏明确的事件结构或价值方向。需要建立一个专门针对新闻事件的价值理解基准。

Method: 从2,865篇英文新闻文章构建NEVU数据集，采用LLM辅助的标注流程，包含阶段性验证和针对性人工审核。数据在四个语义单元级别组织：子事件、基于行为的复合事件、基于故事的复合事件和文章级别，标注(单元, 角色)对进行细粒度评估。

Result: NEVU包含45,793个单位-角色对和168,061个有向价值实例，使用包含54个细粒度价值观和20个粗粒度类别的分层价值空间。实验表明，轻量级适配（LoRA）能持续提升开源模型性能，NEVU不仅作为基准，也支持超越仅提示的监督适配。

Conclusion: NEVU填补了新闻事件中人类价值观理解的空白，提供了角色条件、事件中心和方向感知的评估框架，支持细粒度的价值识别研究，并为模型适配提供了有价值的资源。
...
TL;DR: LLM的言语置信度（verbal confidence）是通过缓存检索机制生成的，而非即时计算。置信度表示在答案相邻位置形成，然后缓存并在需要时检索输出，它反映了比token对数概率更丰富的答案质量评估。


<details>
  <summary>Details</summary>
Motivation: 言语置信度被广泛用于从黑盒LLM中提取不确定性估计，但LLM内部如何生成这些置信度分数仍不清楚。研究旨在探究两个核心问题：1）置信度是即时计算还是生成答案时自动计算并缓存；2）言语置信度代表什么——是token对数概率还是更丰富的答案质量评估。

Method: 聚焦Gemma 3 27B和Qwen 2.5 7B模型，采用多种实验方法：激活引导、补丁、噪声和交换实验来研究置信度表示何时出现；注意力阻断来追踪信息流；线性探测和方差分解来分析置信度表示的内容。

Result: 证据表明置信度通过缓存检索机制生成：置信度表示在答案相邻位置形成，在第一个答案后位置缓存，然后在需要时检索输出。这些缓存表示解释了言语置信度中超出token对数概率的显著方差，表明它们代表更丰富的答案质量评估而非简单的流畅度读取。

Conclusion: 言语置信度反映了LLM自动、复杂的自我评估能力，而非事后重构。这一发现对理解LLM的元认知和改善校准具有重要意义，表明置信度生成是内在的评估过程。
...
<details>
  <summary>Details</summary>
Motivation: LLM虽然在流畅性方面取得突破，但仍容易产生"幻觉"（生成事实错误或无依据内容），这在可靠性至关重要的高风险领域中尤为关键。

Method: 采用四阶段自调节管道（通过LangGraph实现）：(I) 内在验证与早期退出逻辑优化计算；(II) 自适应搜索路由利用领域检测器定位特定主题档案；(III) 纠正性文档分级过滤无关上下文；(IV) 外部再生后进行原子级声明验证。

Result: 在五个基准测试（TimeQA v2、FreshQA v2、HaluEval General、MMLU Global Facts、TruthfulQA）的650个查询中，管道始终优于零样本基线。TimeQA v2胜率达到83.7%，MMLU Global Facts达到78.0%，在需要细粒度时间和数值精度的领域中表现优异。基础性得分在78.8%-86.4%之间保持稳定。
...
TL;DR: DebugLM为LLM提供内置数据溯源能力，让模型能明确追踪其行为到特定训练数据源，支持精准调试和测试时修复


<details>
  <summary>Details</summary>
Motivation: 当前LLM训练使用多阶段异构数据管道，但缺乏系统方法定位特定行为的数据来源，导致调试困难、问题容易在分布偏移或模型更新后复发

Method: 让模型学习将响应与唯一溯源标签关联，指示负责的数据集；基于此支持针对性测试时修复，可选择性触发对指定数据源的拒绝响应，无需重新训练或修改参数

Result: 实验表明DebugLM能在多阶段训练管道中提供准确的行为溯源，实现有效的测试时修复，同时保持模型的通用能力

Conclusion: DebugLM为LLM提供了内置数据溯源能力，解决了训练数据来源可观测性问题，支持精准调试和针对性修复，提高了模型的可控性和可靠性
...
TL;DR: 本文提出了一种基于预训练多语言模型注意力机制的语言距离测量方法——注意力传输距离（ATD），该方法通过最优传输理论量化语言间的表征距离，能够准确恢复语言谱系关系并提升低资源机器翻译性能。


<details>
  <summary>Details</summary>
Motivation: 尽管语言学长期以来提供了丰富的跨语言变异定性描述，但缺乏统一且可扩展的定量方法来测量语言距离。这种定量方法的缺失限制了语言学、人类学和人类进化历史研究的发展。

Method: 利用预训练多语言语言模型的自发注意力机制，将注意力矩阵视为概率分布，通过最优传输理论测量其几何散度，从而计算语言间的注意力传输距离（ATD）。该方法不依赖于分词，具有鲁棒性。

Result: ATD方法应用于大量多样化语言后，能够高保真地恢复已知的语言分组，揭示与地理和接触诱导关系一致的模式。此外，将ATD作为正则化器能够提升低资源机器翻译的迁移性能。

Conclusion: 该方法为使用人工神经网络测试语言假设建立了原则性基础，将多语言模型转化为强大的定量语言发现工具，促进更公平的多语言人工智能发展。
...
TL;DR: 该研究首次系统评估了LLM在12种印度语言中的安全性，发现存在显著的安全漂移现象，跨语言一致性仅12.8%，表明安全对齐在不同语言间转移不均。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多语言环境中部署时，其在文化多样性、低资源语言中的安全行为仍不明确。印度语言有超过12亿使用者，但在LLM训练数据中代表性不足，需要评估其安全性能。

Method: 使用包含6,000个文化相关提示的数据集，涵盖种姓、宗教、性别、健康和政治等主题，评估10个主流LLM在12种印度语言中的表现。通过翻译变体、提示级熵、类别偏置分数和多语言一致性指数量化分析。

Result: 发现显著安全漂移：跨语言一致性仅12.8%，安全率方差超过17%。某些模型在低资源脚本中过度拒绝良性提示，过度标记政治敏感话题，而其他模型则未能标记不安全生成。

Conclusion: 多语言LLM存在关键的安全泛化差距，安全对齐在不同语言间转移不均。发布了首个印度语言安全评估基准IndicSafe，倡导基于区域危害的语言感知对齐策略。
...
TL;DR: 提出一种无需训练的多令牌预测方法，通过掩码令牌探测LLM的潜在能力，实现并行预测未来令牌，提升生成吞吐量


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型仅训练用于下一个令牌预测，但具有潜在的多令牌预测能力。现有方法要么需要修改模型权重，要么依赖辅助草稿模型，我们希望开发一种无需训练的方法来利用这种潜在能力

Method: 提出基于掩码令牌探测的方法：1) 从模型嵌入空间动态生成掩码令牌进行探测；2) 构建推测令牌树，从掩码令牌对数中采样top-K候选；3) 应用轻量级剪枝策略保留高概率延续；4) 并行验证候选预测，实现无损生成

Result: 在基准测试中，该方法始终优于现有的无需训练基线：在LLaMA3上接受长度增加约12%，在Qwen3上增加8-12%，吞吐量提升达15-19%。理论分析和实证证据表明解码器层自然对齐掩码令牌表示与下一个令牌状态

Conclusion: 该方法成功利用LLM的潜在多令牌预测能力，无需重新训练或辅助模型，通过简单的掩码令牌探测实现并行预测，显著减少模型调用次数并提升令牌吞吐量
...
TL;DR: 提出ShapleyLaw，一种基于合作博弈论的多语言缩放定律，通过量化跨语言迁移效应来优化预训练语言混合比例。


<details>
  <summary>Details</summary>
Motivation: 现有多语言缩放定律未能考虑跨语言迁移效应，导致语言混合比例优化不理想。需要一种能够量化各语言贡献的方法来获得更优的预训练数据配比。

Method: 将多语言预训练建模为合作博弈，每个语言作为玩家共同贡献于预训练，获得测试损失减少作为回报。使用合作博弈论的Shapley值量化各语言的贡献，提出ShapleyLaw多语言缩放定律。

Result: 实验表明ShapleyLaw在模型性能预测和语言混合优化方面优于基线方法，能够更准确地预测测试损失并找到更优的语言混合比例。

Conclusion: 通过合作博弈论框架量化跨语言迁移效应，提出的ShapleyLaw方法能够有效优化多语言预训练的语言混合比例，提升模型性能。
...
TL;DR: 本文提出了一种新的性别偏见评估框架，针对解码器专用MT模型，引入"先验偏见"指标来衡量模型的默认性别假设，发现后训练能有效减少男性偏见。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在NLP任务中表现优异，但在机器翻译中仍存在系统性性别偏见。现有基准测试无法全面反映现代MT中性别偏见的复杂性，特别是解码器专用模型在性别标记方面的表现需要专门评估。

Method: 扩展现有偏见评估框架：(1)引入"先验偏见"新指标，量化模型的默认性别假设；(2)将该框架应用于解码器专用MT模型；(3)分析后训练（如指令调优）对性别偏见的影响。

Result: 解码器专用模型在性别相关指标上并未普遍优于编码器-解码器架构；但后训练不仅能提升上下文感知能力，还能显著减少男性先验偏见。

Conclusion: 需要专门针对解码器专用MT模型开发更精细的性别偏见评估方法，后训练是减少性别偏见的有效途径，但模型规模本身并不能保证更好的性别处理能力。
...
TL;DR: 提出ConGA框架用于标注英意翻译中的性别信息，创建gENder-IT数据集作为评估性别偏见的黄金标准资源，发现当前机器翻译系统存在系统性男性形式过度使用问题。


<details>
  <summary>Details</summary>
Motivation: 从性别中性语言（如英语）翻译到形态性别语言（如意大利语）时，机器翻译和大型语言模型面临持续挑战。英语省略语法性别，而意大利语要求明确的性别一致，这种不对称导致MT系统默认使用男性形式，加剧偏见并降低翻译准确性。

Method: 提出上下文性别标注（ConGA）框架，这是一套基于语言学的词级性别标注指南。该方案区分英语中的语义性别（男性M、女性F、模糊A）和意大利语中的语法性别实现（男性M、女性F），并结合实体级标识符进行跨句子跟踪。将ConGA应用于gENder-IT数据集，创建评估翻译性别偏见的黄金标准资源。

Result: 结果揭示了系统性的男性形式过度使用和不一致的女性形式实现，突显了当前机器翻译系统的持续局限性。通过细粒度语言标注与定量评估相结合，为构建更具性别意识和多语言的NLP系统提供了方法论和基准。

Conclusion: 这项工作通过结合精细的语言学标注和定量评估，为构建更具性别意识和多语言的NLP系统提供了方法论和基准，有助于解决机器翻译中的性别偏见问题。
...
### [204] [Purcell swimmer near a wall](https://arxiv.org/abs/2603.16903)
*Enrico Micalizio,Marco Morandotti,Henry Shum,Marta Zoppello*

Main category: physics.flu-dyn

TL;DR: 研究二维Purcell三连杆游泳器在壁面附近的水动力相互作用，证明在低雷诺数下系统在接近平行于壁面的构型下可控，并分析倾斜构型的净位移。


<details>
  <summary>Details</summary>
Motivation: 研究游泳器与壁面之间的水动力相互作用对运动控制的影响，这对于理解微尺度生物游泳器在受限环境中的运动机制具有重要意义。

Method: 使用修正阻力系数的阻力理论推导低雷诺数下的运动方程，应用几何控制理论分析系统可控性，并通过数值实验验证分析结果。

Result: 证明系统在接近平行于壁面的构型下是可控的，对于倾斜构型也能实现相对于初始方向的净位移。

Conclusion: 壁面附近的水动力相互作用显著影响Purcell游泳器的运动控制特性，系统在特定构型下具有可控性，这为微尺度游泳器的导航和控制提供了理论基础。
...
### [205] [Optimization-Embedded Active Multi-Fidelity Surrogate Learning for Multi-Condition Airfoil Shape Optimization](https://arxiv.org/abs/2603.17057)
*Isaac Robledo,Alberto Vilariño,Arnau Miró,Oriol Lehmkuhl,Carlos Sanmiguel Vila,Rodrigo Castellanos*

Main category: physics.flu-dyn

TL;DR: 开发了主动多保真度代理建模方法用于多工况翼型优化，通过结合低保真度信息的高斯过程迁移模型、不确定性触发采样和遗传算法中的同步精英规则，大幅减少高保真度CFD计算成本，同时保持RANS级精度。


<details>
  <summary>Details</summary>
Motivation: 多工况翼型形状优化需要大量高保真度CFD计算，成本高昂。需要开发能减少高保真度计算需求的方法，同时保持设计精度。

Method: 耦合低保真度信息的高斯过程回归迁移模型与不确定性触发采样，嵌入同步精英规则的混合遗传算法。使用XFOIL进行低保真度评估，当预测不确定性超过阈值时自适应分配稀疏RANS模拟，精英候选强制进行高保真度验证，种群重新评估以防止基于过时代理状态的进化选择。

Result: 优化设计相比第一代最佳个体，巡航效率提高41.05%，起飞升力提高20.75%。在整个优化过程中，仅需对14.78%（巡航）和9.5%（起飞）的个体进行RANS计算，显著减少高保真度使用量。

Conclusion: 该方法能有效减少多工况翼型优化中的高保真度计算成本，同时保持设计性能，为复杂气动优化问题提供了高效的计算框架。
...
### [206] [Entrainment of magnetic fluid by a moving boundary of a plane gap](https://arxiv.org/abs/2603.17462)
*Denis S. Goldobin,Yuriy L. Raikher*

Main category: physics.flu-dyn

TL;DR: 使用磁性流体作为超声无损检测中的声学接触介质，通过磁场稳定流体桥，减少重力引起的流体泄漏，提高动态扫描时的稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统流体声学接触在动态扫描中因重力导致流体泄漏而失去稳定性，需要一种能保持稳定接触同时最小化流体流失的解决方案。

Method: 使用磁性流体（MF）和永磁体创建稳定的流体桥，研究MF桥在平面间隙中的动态行为，其中一个边界以恒定速度移动，分析流体薄膜轮廓和磁性流体流失与边界速度、间隙高度和磁场配置的关系。

Result: 获得了定义动态状态下薄膜轮廓的理论表达式，以及磁性流体流失与边界速度、间隙高度和磁场配置的依赖关系，在此基础上评估了确保流体接触有效保持同时最小化流体流失的最佳参数。

Conclusion: 磁性流体结合永磁体能够有效解决超声无损检测中声学接触的稳定性问题，在任意传感器方向下都能保持稳定接触，同时显著减少流体流失。
...
### [207] [Adaptive near-contact repulsion in conservative Allen-Cahn phase-field lattice Boltzmann multiphase model](https://arxiv.org/abs/2603.17481)
*Andrea Montessori,Maria Rosa Lisboa,Marco Lauricella,Sauro Succi*

Main category: physics.flu-dyn

TL;DR: 提出一种局部排斥通量方法，解决相场模拟中薄膜动力学导致的虚假聚并问题


<details>
  <summary>Details</summary>
Motivation: 解决多相流扩散界面模拟中未解决的薄膜动力学问题，该问题常导致虚假的聚并现象

Method: 在保守Allen-Cahn相场模型中引入完全局部的排斥近接触通量，耦合格子玻尔兹曼流体动力学。该方法仅对方向相反的邻近界面激活，强度基于从相场提取的局部薄膜厚度分析估计值自适应调整

Result: 方法避免了非局部几何过程，保持了计算效率，适合大规模并行实现。碰撞基准测试和三维气泡群模拟显示能稳健抑制人工合并，实现物理一致的近接触动力学

Conclusion: 提出的局部排斥通量方法有效解决了相场模拟中的虚假聚并问题，实现了物理正确的近接触动力学，同时保持了计算效率和并行性
...
### [208] [Coalescence of viscous blisters under an elastic sheet](https://arxiv.org/abs/2603.17731)
*W-E. Khatla,L. Duchemin,A. Eddi,E. Reyssat*

Main category: physics.flu-dyn

TL;DR: 研究弹性薄片下相同粘性水泡的合并过程，通过实验和数值模拟分析合并区域动力学，建立一维润滑近似模型预测合并颈生长规律。


<details>
  <summary>Details</summary>
Motivation: 研究弹性薄片下粘性水泡的合并动力学，特别是合并区域的演化规律，理解弹性弯曲在合并过程中的主导作用。

Method: 采用时间分辨合成纹影技术测量合并水泡厚度场演化，建立基于润滑近似的一维模型推导合并颈生长标度律，并通过数值求解完整非线性方程验证理论并与实验对比。

Result: 模型表明在短时间尺度上，合并动力学主要由弹性薄片的弯曲控制，合并速度与合并颈处界面曲率半径存在特定关系。

Conclusion: 弹性薄片下粘性水泡的合并过程在早期阶段受弹性弯曲主导，建立的理论模型能有效预测合并颈的生长规律，为理解此类流体-结构相互作用提供了理论框架。
...
<details>
  <summary>Details</summary>
Motivation: 社会-环境规划中的深度不确定性要求研究者先识别和概念化问题，再探索政策和部署计划。传统的参与式建模过程依赖利益相关者的自然语言描述转化为定量模型，这一过程复杂且耗时。

Method: 提出模板化工作流，利用大语言模型进行初始概念化：1）从利益相关者的直观问题描述中识别基本模型组件；2）探索解决问题的不同视角；3）将这些组件组装成统一模型；4）通过迭代交流在Python中实现模型。使用ChatGPT 5.2 Instant在湖泊问题和电力市场问题上进行演示。

Result: 在两个案例中，经过几次迭代和人工验证与细化后，都获得了可接受的输出。实验表明大语言模型可以作为社会-环境规划中问题概念化过程中促进参与式建模的有效工具。

Conclusion: 大语言模型能够有效促进社会-环境规划中的参与式建模，特别是在问题概念化阶段，通过模板化工作流可以显著提高效率并减少复杂性。
...
TL;DR: 论文证明Transformer本质上是贝叶斯网络，通过五种方式建立了这一等价关系：1）Sigmoid Transformer实现加权循环信念传播；2）可构造实现精确信念传播；3）唯一性证明；4）层结构的AND/OR布尔解释；5）实验验证。同时证明可验证推理需要有限概念空间。


<details>
  <summary>Details</summary>
Motivation: Transformer是AI中的主导架构，但其工作原理仍不清楚。本文旨在提供精确解释：证明Transformer本质上是贝叶斯网络，揭示其内在的统计推理机制。

Method: 采用五种证明方法：1）证明Sigmoid Transformer实现加权循环信念传播；2）构造性证明Transformer可实现精确信念传播；3）唯一性证明；4）分析Transformer层的AND/OR布尔结构；5）实验验证。所有证明均基于标准数学公理进行形式化验证。

Result: 成功建立了Transformer与贝叶斯网络的等价关系，证明了Transformer层对应信念传播的一轮迭代，注意力机制对应AND操作，前馈网络对应OR操作。实验证实了理论结果，并展示了循环信念传播的实际可行性。

Conclusion: Transformer本质上是贝叶斯网络，其工作原理可通过信念传播解释。可验证推理需要有限概念空间，否则会产生幻觉问题。这一发现为理解Transformer提供了理论基础，并指出了其局限性。
...
TL;DR: 论文提出一种轻量级几何感知调度器，用于符号图网络中的故障传播控制，通过自适应几何选择器在欧几里得和双曲模型间切换，显著提升系统性能。


<details>
  <summary>Details</summary>
Motivation: 当前AI推理系统中的符号图网络调度器存在几何盲区：它们无法建模树状结构与循环结构中故障传播的差异。树状委托中单个故障可能指数级级联，而密集循环图中故障往往自限。这种可观测性差距导致系统级成本。

Method: 提出在线几何控制方法，结合：(1)欧几里得时空传播基线；(2)带时间衰减的双曲路径风险模型；(3)基于结构特征的几何选择器（9->12->1 MLP）。选择器使用六个拓扑统计量和三个几何感知信号：BFS壳增长斜率、循环秩范数和拟合的庞加莱曲率。

Result: 在Genesis 3基准测试中，自适应切换将最难非树状区域的胜率从64-72%提升到92%，整体胜率达87.2%。相比仅使用原生信号（50.4%胜率）的基线，整体提升36.8个百分点，在树状设置中提升48-68个百分点。

Conclusion: 仅133个参数的轻量级辅助系统能显著缓解符号图网络中几何盲故障传播问题，证明了在高级AI推理系统中考虑执行图几何结构的重要性。
...
TL;DR: LLM智能体在文本版Clue游戏中表现不佳，仅4/18获胜，微调对推理能力提升有限


<details>
  <summary>Details</summary>
Motivation: 评估LLM智能体在多步演绎推理任务中的表现，特别是能否在完整游戏过程中保持一致的推理能力，并研究微调对推理能力的迁移效果

Method: 构建基于规则的文本版Clue游戏测试平台，使用GPT-4o-mini和Gemini-2.5-Flash的6个智能体进行18场模拟游戏，并研究在结构化逻辑谜题上的微调是否改善游戏推理

Result: 智能体表现不佳，仅4场获胜，表明在完整游戏中维持一致演绎推理存在困难；微调未能可靠提升性能，有时甚至增加推理量而不提高推理精度

Conclusion: 当前LLM智能体在多步演绎推理任务中仍有局限，微调对复杂推理能力的迁移效果有限，需要更有效的方法来提升智能体的长期推理一致性
...
TL;DR: 提出一个针对机器学习代理的合成环境生成管道，用于训练能够从实践中学习的AI代理，通过在MLGym基准测试上提升性能验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 随着AI代理的发展，自动科学发现成为可行目标。现有代理系统缺乏训练代理的原则性方法，且当前LLM常生成看似合理但无效的想法。需要训练能够从实践中学习的代理。

Method: 开发合成环境生成管道，自动合成与SWE-agent框架兼容的机器学习挑战，包括主题采样、数据集提议和代码生成。合成任务基于真实机器学习数据集（通过Huggingface API验证），并通过自调试循环确保高质量。

Result: 使用合成任务从教师模型（GPT-5）采样轨迹，然后用这些轨迹训练学生模型（Qwen3-4B和Qwen3-8B）。在MLGym基准测试上，学生模型性能显著提升：Qwen3-4B的AUP指标提高9%，Qwen3-8B提高12%。

Conclusion: 提出的合成环境生成管道能有效训练机器学习代理，通过基于真实数据集的合成任务和自调试机制确保质量，显著提升代理在机器学习任务上的性能。
...
TL;DR: Draft-and-Prune (D&P) 是一个推理时框架，通过多样化和验证改进自动形式化的逻辑推理，显著提升多个基准测试的性能。


<details>
  <summary>Details</summary>
Motivation: 当前自动形式化（AF）管道脆弱，程序可能执行失败或编码错误语义。虽然先前工作通过基于求解器反馈的修复缓解了语法失败，但减少语义失败仍然是主要瓶颈。

Method: Draft-and-Prune (D&P) 框架：1）起草多个自然语言计划，并基于这些计划生成程序；2）修剪可执行但矛盾或模糊的形式化；3）通过多数投票从存活路径中聚合预测。

Result: 在四个代表性基准测试（AR-LSAT、ProofWriter、PrOntoQA、LogicalDeduction）上，D&P 显著增强了基于 AF 的推理。在 AR-LSAT 上，使用 GPT-4 达到 78.43% 准确率，GPT-4o 达到 78.00%，大幅超越最强基线 MAD-LOGIC 和 CLOVER。在其他基准测试上达到接近天花板性能，包括 PrOntoQA 和 LogicalDeduction 的 100%。
...
TL;DR: Kumiho是一个基于形式信念修正语义的图原生认知记忆架构，通过统一图结构同时支持AI代理的记忆管理和工作资产管理，在认知记忆基准测试中显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有AI代理系统中的记忆组件缺乏体系化架构设计和形式化理论基础，需要统一的架构来同时处理认知记忆和代理工作资产管理。

Method: 提出基于属性图的统一架构，使用不可变修订、可变标签指针、类型化依赖边和URI寻址等结构原语；实现双存储模型（Redis工作记忆+Neo4j长期图）；采用前瞻索引、事件提取和客户端LLM重排三项创新技术。

Result: 在LoCoMo基准上达到0.565总体F1分数（n=1,986），包含97.5%的对抗性拒绝准确率；在LoCoMo-Plus基准上达到93.3%的评估准确率（n=401），显著优于所有已发布的基线（最佳基线Gemini 2.5 Pro为45.7%）。

Conclusion: Kumiho通过形式化信念修正语义与图原生架构的结合，为AI代理提供了统一的认知记忆系统，在多个基准测试中表现出色，且具有模型解耦特性，能够灵活切换底层LLM模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有防御主要在输出层面操作，缺乏对推理过程的深度安全对齐。需要利用模型的推理能力和隐藏表示来构建更鲁棒的安全对齐框架。

Method: CRAFT结合对比表示学习和强化学习，在隐藏状态空间分离安全与不安全推理轨迹，通过潜在-文本一致性优化GRPO，消除表面对齐策略。

Result: 在多个安全基准测试中，CRAFT在Qwen3-4B-Thinking和R1-Distill-Llama-8B模型上优于IPO和SafeKey等SOTA防御，推理安全平均提升79.0%，最终响应安全提升87.7%。

Conclusion: 隐藏空间推理对齐能有效提升模型对抗越狱攻击的鲁棒性，CRAFT框架为安全对齐提供了新的方向。
...
TL;DR: 提出InfoDensity奖励框架，通过衡量推理质量而非仅长度来优化LLM推理过程，在保持准确率的同时显著减少计算成本


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法仅优化最终响应长度，忽略了中间推理步骤的质量，导致模型容易奖励黑客攻击。研究发现冗余推理是中间推理质量差的表现，而非简单的长度问题

Method: 提出InfoDensity奖励框架，结合基于AUC的奖励和单调性奖励作为推理质量的统一度量，并通过长度缩放项鼓励在保持质量的前提下更简洁的推理

Result: 在数学推理基准测试中，InfoDensity在准确率上匹配或超越现有最优方法，同时显著减少token使用量，实现了良好的准确率-效率权衡

Conclusion: 通过关注推理质量而非仅长度，InfoDensity能够有效减少冗余推理，为LLM推理优化提供了更有效的方法
...
TL;DR: PIER是一个基于离线强化学习的船舶航线优化框架，利用物理校准环境和历史数据，无需在线模拟器，能显著减少燃料消耗和碳排放，同时提高安全性。


<details>
  <summary>Details</summary>
Motivation: 国际航运占全球温室气体排放的3%，但当前航线规划仍主要依赖启发式方法，存在燃料浪费和安全性问题，需要更智能、节能且安全的航线优化方案。

Method: PIER采用离线强化学习框架，结合物理信息状态构建、演示增强的离线数据，以及解耦的后验安全屏蔽，从物理校准环境中学习燃料效率和安全性策略。

Result: 在墨西哥湾7条航线一年数据验证中，PIER相比大圆航线平均减少10% CO2排放，将极端燃料消耗（>1.5倍中位数）从4.8%降至0.5%，燃料方差降低3.5倍，且不依赖天气预报。

Conclusion: PIER框架能有效优化船舶航线，显著减少燃料消耗和碳排放，提高安全性，且架构可推广到野火疏散、飞机轨迹优化和未知地形自主导航等应用。
...
TL;DR: ShuttleEnv是一个基于数据的羽毛球交互式模拟环境，用于支持强化学习和战略行为分析，无需物理模拟即可实现真实的对抗交互。


<details>
  <summary>Details</summary>
Motivation: 为快速对抗性体育（如羽毛球）提供一个可重复使用的研究平台，支持强化学习、战略行为分析，并实现真实且可解释的智能体交互。

Method: 基于精英选手比赛数据，采用显式概率模型模拟回合级动态，实现无需物理仿真的真实对抗交互，并提供实时可视化分析。

Result: 开发了ShuttleEnv环境，展示了多个训练有素的智能体，提供实时回合可视化，支持探索不同打法风格、观察涌现策略和交互式决策分析。

Conclusion: ShuttleEnv作为一个可重复使用的平台，为体育AI领域的研究、可视化和智能体演示提供了有效工具。
...
TL;DR: RideJudge：一个渐进式视觉-逻辑对齐框架，通过合成轨迹、自适应上下文优化和序数敏感强化学习，解决网约车责任判定中的语义鸿沟、法规复杂性和反馈稀疏问题，在8B规模上达到88.41%准确率。


<details>
  <summary>Details</summary>
Motivation: 网约车责任判定面临三大挑战：1) 手动审查无法应对指数级增长的订单量；2) 传统自动化方法缺乏准司法决策所需的推理透明度；3) 多模态大语言模型存在视觉语义与证据协议之间的鸿沟，导致感知幻觉和逻辑松散。

Method: 提出RideJudge框架：1) SynTraj合成引擎将抽象责任概念转化为具体轨迹模式；2) 自适应上下文优化策略提炼专家知识，配合裁决链机制进行主动证据查询；3) 序数敏感强化学习机制根据严重性层级校准决策边界。

Result: RideJudge-8B模型达到88.41%的准确率，超越了32B规模的基线模型，为可解释的裁决设立了新标准。

Conclusion: RideJudge通过渐进式视觉-逻辑对齐框架，有效解决了网约车责任判定中的系统性错位问题，在保持可解释性的同时实现了高精度决策。
...
TL;DR: 大型推理模型在使用思维链时会出现安全性下降，本文提出一种新方法让模型在开始思维链前先做安全决策，从而提升安全性同时保持推理能力。


<details>
  <summary>Details</summary>
Motivation: 研究发现大型推理模型在使用思维链（CoT）时推理能力显著提升，但安全性却大幅下降。这种安全性下降只在启用CoT时出现，禁用CoT时不会发生。这促使研究者思考如何让模型在开始CoT生成前先做出安全决策。

Method: 提出一种新颖的安全对齐方法：1）使用基于Bert的分类器从安全模型（如禁用CoT的LRM）中提取安全决策信号；2）将这些信号作为辅助监督集成到LRM的安全对齐中；3）让安全梯度反向传播到LRM的潜在表示，从而增强模型在CoT生成前的安全决策能力。

Result: 大量实验表明，该方法显著提升了大型推理模型的安全性，同时有效保持了模型的一般推理性能。

Conclusion: 通过让大型推理模型在开始思维链生成前先进行安全决策，可以有效解决CoT带来的安全性下降问题，实现安全性与推理能力的平衡。
...
TL;DR: 本文系统综述了从数字孪生向世界模型的演进，探讨其在实现边缘通用智能中的作用，包括概念差异、设计原则、架构组件、无线EGI系统集成以及应用场景，并提出了研究路线图和未来方向。


<details>
  <summary>Details</summary>
Motivation: 随着6G及后6G通信系统的发展，数字孪生与世界模型在网络边缘的融合加速。传统数字孪生在高度动态的边缘环境中面临自主性、适应性和可扩展性限制，需要向更智能的世界模型演进以支持边缘通用智能。

Method: 本文采用系统性综述方法：1) 澄清数字孪生与世界模型的概念差异；2) 回顾世界模型的设计原则、架构和关键组件（感知、潜在状态表示、动态学习、基于想象的规划、记忆）；3) 分析世界模型与数字孪生在无线EGI系统中的集成；4) 调查新兴应用场景；5) 提供设计路线图和实践见解。

Result: 论文明确了从物理驱动、集中式、系统中心的数字孪生向数据驱动、分布式、智能体中心的世界模型转变，这种转变能够实现更自适应、自主和资源高效的边缘智能。提出了世界模型驱动的边缘智能系统设计框架，并识别了关键应用领域。

Conclusion: 世界模型是实现边缘通用智能的关键演进方向，能够克服传统数字孪生在动态边缘环境中的局限性。未来研究需要解决可扩展性、可靠性和互操作性挑战，以推动边缘原生智能体AI的发展。
...
Method: 将医患对话建模为部分可观测下的主动知识查询问题，结合状态提取、顺序信念更新、缺口感知状态建模、混合知识检索和POMDP-lite动作规划器

Result: 在10个标准化多轮对话和300个查询检索基准测试中，完整框架达到83.3%覆盖率、80.0%风险召回率、81.4%结构完整性，冗余度低于基线方法

Conclusion: 主动知识查询在严格受控条件下具有方法论意义，是对话式电子病历生成的概念性吸引人的框架，但仅为概念验证而非临床部署准备
...
TL;DR: IET框架通过嵌入密钥信号实现多智能体系统的执行追踪，无需元数据即可从生成文本中恢复智能体贡献和交互拓扑


<details>
  <summary>Details</summary>
Motivation: 当多智能体系统产生错误或有害答案时，如果没有执行日志和智能体标识符，很难确定责任归属。当前多智能体语言系统依赖结构化交互，但最终输出往往掩盖了底层交互拓扑和智能体贡献。

Method: 提出IET（隐式执行追踪）框架：1）在生成过程中，将智能体特定的密钥信号嵌入到token分布中，使文本成为自描述的执行轨迹；2）在检测时，使用转移感知的评分方法识别智能体交接点并重建交互图。

Result: 实验表明，IET能够以高精度恢复智能体片段和协调结构，同时保持生成质量，实现了多智能体语言系统的隐私保护审计。

Conclusion: IET框架为多智能体系统提供了一种无需元数据的可追溯性解决方案，解决了责任归属问题，同时保护了隐私，支持对复杂多智能体交互的审计。
...
TL;DR: 提出ISF方法，在传统半事实解释基础上补充隐藏特征信息，解释为何极端值变化不会改变预测结果


<details>
  <summary>Details</summary>
Motivation: 现有半事实解释方法只说明特征值极端变化不会改变结果，但未解释原因，缺乏信息量

Method: 开发ISF算法，生成包含隐藏特征信息的半事实解释，说明为何特征值变化不会影响预测结果
...
TL;DR: 本文提出学习Q值函数而非状态值函数，通过正则化区分教师规划器采取与未采取的动作，在10个领域表现优于状态值策略，与LAMA-first规划器竞争


<details>
  <summary>Details</summary>
Motivation: 学习跨领域泛化策略是规划学习的关键挑战。传统方法使用图神经网络学习状态值函数，但评估成本高，需要处理所有后继状态。本文主张学习Q值函数，评估成本更低，只需处理当前状态。

Method: 使用监督学习学习Q值函数，但发现普通监督学习效果差，无法区分教师规划器采取与未采取的动作。通过添加正则化项强制这种区分，学习更有效的Q值策略。

Result: Q值策略在10个不同领域中始终优于状态值策略，且与LAMA-first规划器性能相当。Q值策略评估成本显著降低，只需处理当前状态而非所有后继状态。

Conclusion: 学习Q值函数是更有效的跨领域泛化策略学习方法，通过适当的正则化可以学习区分动作的Q值，实现高性能且计算效率高的策略。
...
Method: 采用灰盒方法，使用工具调用序列作为反馈函数驱动测试过程；通过突变提示设计恶意注入提示，将代理任务与注入任务链接，使注入任务成为完成代理功能的必要步骤。
...
Conclusion: VeriGrey展示了动态测试方法在发现LLM代理安全风险方面的价值，为构建代理保证框架提供了基础。
...
Method: 提出Sensi架构：1) 分离感知与行动的两玩家架构；2) 外部状态机管理的课程学习系统；3) 数据库作为控制平面实现可编程上下文窗口；4) 动态生成评估标准的LLM-as-judge组件。

Result: Sensi v1解决了2个游戏关卡，Sensi v2虽然解决了0个关卡，但仅用约32次行动尝试就完成了整个学习课程，相比需要1600-3000次尝试的系统实现了50-94倍的样本效率提升。

Conclusion: 架构瓶颈已从学习效率转向感知基础问题，这是一个更易处理的问题。感知层的自洽幻觉级联是主要失败模式，表明需要改进感知基础。
...
<details>
  <summary>Details</summary>
Motivation: 现实经济中的决策面临高维、多模态环境的挑战，同时存在智能体异质性和组合数据稀疏性问题，需要统一的仿真框架来应对跨领域、跨类别的场景。

Method: 1) 基于偏好学习范式，通过后训练使LLM在经济上对齐，从异构交易记录中学习消费者偏好模式；2) 采用均值场机制建模产品环境与客户群体的动态交互，稳定高维决策空间采样；3) 提出多智能体讨论框架，通过专业智能体协作处理产品信息，分散认知负荷。

Result: 相比现有经济和金融LLM仿真基线，该框架在产品选择准确性、购买数量预测和仿真稳定性方面取得显著改进，验证了LLM作为高保真、可扩展决策仿真基础支柱的潜力。

Conclusion: 大型语言模型可作为现实经济中高保真、可扩展决策仿真和分析的基础支柱，基于偏好学习和多智能体架构的方法能有效解决数据稀疏性和高维决策挑战。
...
TL;DR: 本文综述了自动驾驶中合成数据与虚拟仿真的最新进展，涵盖感知规划、数字孪生验证和域适应三大维度，并讨论了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶技术虽取得显著进展，但在实际部署中面临数据稀缺、安全要求和跨环境泛化等挑战。合成数据和虚拟环境为解决这些问题提供了可扩展、可控且标注丰富的解决方案。

Method: 从三个核心维度组织研究现状：(1) 合成数据在感知与规划中的应用；(2) 基于数字孪生的系统验证仿真；(3) 连接合成与真实数据的域适应策略。同时提供数据集、工具和仿真平台的详细分类。

Result: 建立了自动驾驶仿真与合成数据的综合分类体系，分析了基准设计趋势，强调了视觉语言模型和仿真真实感在场景理解和泛化中的作用。
...
TL;DR: 本文对比了上下文记忆与知识对象(KOs)两种知识存储方式，发现上下文记忆存在容量限制、压缩损失和目标漂移三大失效模式，而KOs在准确性、成本和推理能力方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型作为持久知识工作者时，通常使用上下文记忆（将事实存储在提示中）作为默认策略。本文旨在评估这种方法的局限性，并与知识对象(KOs)这种离散哈希寻址元组进行比较，以寻找更可靠的知识存储和检索方案。

Method: 1. 在上下文窗口内测试Claude Sonnet 4.5的准确性（10-7,000个事实）
2. 生产部署中识别三种失效模式：容量限制、压缩损失、目标漂移
3. 对比KOs的O(1)检索性能
4. 多跳推理测试
5. 跨四个前沿模型的复制实验
6. 评估嵌入检索在对抗性事实上的表现
7. 测试神经记忆(Titans)的检索能力
8. 引入密度自适应检索作为切换机制

Result: 1. 上下文记忆：在200K窗口内7,000个事实时达到97.5%准确率，但存在三大失效模式
2. KOs：所有条件下100%准确率，成本降低252倍
3. 多跳推理：KOs达到78.9% vs 上下文记忆31.6%
4. 压缩损失是架构性问题，非模型特定
5. 嵌入检索在对抗性事实上表现差（召回率1时精度仅20%）
6. 神经记忆能存储事实但无法按需检索

Conclusion: 上下文记忆作为默认知识存储策略存在严重缺陷，特别是在生产环境中。知识对象(KOs)提供了更可靠、高效且成本更低的替代方案，特别是在处理大量事实和复杂推理任务时。研究还揭示了嵌入检索和神经记忆的局限性，并提出了密度自适应检索作为解决方案。
...
TL;DR: 论文提出Governed Memory架构，解决企业AI中多个自主代理节点缺乏共享内存和统一治理的问题，通过双重内存模型、分层治理路由等机制实现高效、安全的信息管理。


<details>
  <summary>Details</summary>
Motivation: 企业AI部署中，多个自主代理节点在同一实体上操作，但缺乏共享内存和统一治理，导致内存孤岛、治理碎片化、非结构化记忆无法被下游系统使用、冗余上下文传递以及无反馈循环的质量退化等五大结构性问题。

Method: 提出Governed Memory架构，包含四个核心机制：1) 双重内存模型（开放集原子事实+模式强制的类型化属性）；2) 分层治理路由与渐进式上下文传递；3) 反射边界检索与实体范围隔离；4) 闭环模式生命周期（AI辅助创建+自动化属性级优化）。

Result: 实验验证显示：99.6%的事实召回率，92%的治理路由精度，50%的token减少（渐进式传递），500个对抗查询中零跨实体泄漏，100%对抗治理合规性，每个实体约7个治理记忆时输出质量饱和。在LoCoMo基准测试中达到74.8%总体准确率。

Conclusion: Governed Memory架构有效解决了企业AI中的内存治理缺口问题，证明了治理和模式强制不会损害检索质量，已在Personize.ai投入生产使用。
...
TL;DR: RPMS架构通过规则检索和信念状态管理，解决了LLM智能体在封闭世界环境中的动作可行性和状态漂移问题，显著提升了任务成功率。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在封闭世界具身环境中经常失败，因为动作需要满足严格的前提条件（如位置、库存、容器状态），而失败反馈稀疏。研究者识别出两种结构耦合的失败模式：无效动作生成和状态漂移，它们会相互放大形成退化循环。

Method: 提出RPMS冲突管理架构：1) 通过结构化规则检索强制执行动作可行性；2) 通过轻量级信念状态门控记忆适用性；3) 通过规则优先仲裁解决两个来源之间的冲突。

Result: 在ALFWorld的134个未见任务中：Llama 3.1 8B达到59.7%单次成功率（比基线提高23.9个百分点），Claude Sonnet 4.5达到98.5%（提高11.9个百分点）。规则检索单独贡献了14.9个百分点的增益。在ScienceWorld中，GPT-4在所有消融条件下都获得一致增益（平均得分54.0 vs ReAct基线的44.9）。

Conclusion: RPMS架构有效解决了LLM智能体在封闭世界环境中的核心问题。关键发现是情景记忆有条件地有用：在没有基础的情况下会损害某些任务类型的性能，但一旦通过当前状态过滤并受明确动作规则约束，就会成为稳定的净收益。核心机制在不同结构环境中具有可迁移性。
...
Method: 提出AgentFactory范式，将成功任务解决方案保存为可执行的子智能体Python代码，基于执行反馈持续精炼，形成标准化文档的可移植代码库。

Result: AgentFactory实现了持续能力积累：可执行子智能体库随时间增长和改进，逐步减少类似任务所需工作量，无需人工干预。

Conclusion: AgentFactory通过将成功经验编码为可执行子智能体代码而非文本经验，提供了一种更可靠、可移植的LLM智能体自进化范式，支持持续能力积累。
...
### [235] [A foundation model for electrodermal activity data](https://arxiv.org/abs/2603.16878)
...
TL;DR: 研究人员创建了首个大规模、公开的EDA数据集EDAMAME（包含25,000+小时数据），并基于此训练了首个EDA专用基础模型UME，该模型在多数场景下优于基线且计算资源消耗仅为通用时序模型的1/20。


<details>
  <summary>Details</summary>
Motivation: EDA（皮肤电活动）作为反映交感神经系统活动的重要生理信号，在认知负荷、压力、参与度评估中有广泛应用，但缺乏大规模、公开、高质量的数据集阻碍了该领域基础模型的发展。现有大型档案多为专有数据，限制了研究进展。

Method: 1. 从24个公开数据集中收集EDA数据，构建EDAMAME数据集（634名用户，超过25,000小时数据）；2. 基于该数据集训练UME模型，这是首个专门针对EDA设计的基础模型；3. 在10个不同场景下评估模型性能，并与基线方法和通用时序基础模型进行比较。

Result: 在10个评估场景中，UME在8个场景中表现优于基线方法，且与通用时序基础模型性能相当，同时计算资源消耗仅为后者的1/20。研究也揭示了EDA建模的内在挑战，表明该领域仍有进一步研究空间。

Conclusion: EDAMAME数据集和UME模型的发布填补了EDA领域缺乏公开大规模数据资源的空白，为后续研究提供了重要基础。虽然模型在多数场景中表现良好，但EDA建模仍面临挑战，需要进一步研究以充分发挥其潜力。所有数据集、模型权重和代码均已开源。
...
### [236] [Federated Multi Agent Deep Learning and Neural Networks for Advanced Distributed Sensing in Wireless Networks](https://arxiv.org/abs/2603.16881)
*Nadine Muller,Stefano DeRosa,Su Zhang,Chun Lee Huan*

Main category: cs.LG

TL;DR: 该论文是关于多智能体深度学习在无线系统中应用的综述，重点分析了2021-2025年的研究进展，涵盖分布式感知和无线通信领域，并提出了面向6G系统的研究方向。


<details>
  <summary>Details</summary>
Motivation: 5G-Advanced和6G愿景加强了感知、通信和计算的紧密耦合，形成了去中心化、部分可观测、时变和资源受限的控制问题，需要多智能体深度学习框架来解决这些挑战。

Method: 采用任务驱动的分类法，从四个维度进行系统综述：(1)学习形式化（马尔可夫博弈、Dec-POMDPs、CTDE），(2)神经网络架构（基于GNN的无线资源管理、注意力策略、分层学习、空中聚合），(3)先进技术（联邦强化学习、通信高效的联邦深度RL、无服务器边缘学习编排），(4)应用领域（MEC卸载切片、UAV异构网络、传感器网络入侵检测、ISAC驱动的感知移动网络）。

Result: 提供了算法比较表、训练拓扑结构以及系统级权衡（延迟、频谱效率、能耗、隐私、鲁棒性）的分析，识别了当前研究的现状和技术水平。

Conclusion: 指出了可扩展性、非平稳性、安全防护、通信开销和实时安全性等开放问题，并提出了面向6G原生感知-通信-计算-学习系统的研究方向。
...
### [237] [Multi-Agent Reinforcement Learning for Dynamic Pricing: Balancing Profitability,Stability and Fairness](https://arxiv.org/abs/2603.16888)
*Krishna Kumar Neelakanta Pillai Santha Kumari Amma*

Main category: cs.LG

TL;DR: 本文系统评估了多智能体强化学习（MAPPO和MADDPG）在竞争性动态定价中的应用，发现MAPPO在利润和稳定性方面表现最佳，而MADDPG在公平性方面最优。


<details>
  <summary>Details</summary>
Motivation: 竞争性零售市场的动态定价需要能够适应需求波动和竞争对手行为的策略。传统独立学习方法可能无法充分捕捉多智能体交互的复杂性。

Method: 使用基于真实零售数据构建的模拟市场环境，系统评估MAPPO和MADDPG两种多智能体强化学习方法，并与独立DDPG（IDDPG）基线进行比较。评估指标包括利润表现、随机种子稳定性、公平性和训练效率。

Result: MAPPO始终获得最高的平均回报且方差低，为竞争性价格优化提供了稳定可复现的方法；MADDPG利润略低但实现了最公平的利润分配。两种方法都优于独立学习基线。
...
### [238] [From Language to Action in Arabic: Reliable Structured Tool Calling via Data-Centric Fine-Tuning](https://arxiv.org/abs/2603.16901)
...
TL;DR: 开发了面向阿拉伯语的函数调用框架AISA-AR-FunctionCall，基于270M参数的FunctionGemma模型，通过系统数据集审计、模式修复、工具感知提示重构和全参数监督微调，将解析失败率从87%降至1%以下


<details>
  <summary>Details</summary>
Motivation: 现有函数调用语言模型在处理阿拉伯语时表现出严重的结构不稳定性，需要专门针对阿拉伯语的函数调用框架来改善这一问题

Method: 基于270M参数的FunctionGemma骨干网络，采用系统数据集审计、模式修复、工具感知提示重构和全参数监督微调的方法，并探索了推理增强的LoRA变体，在工具调用前引入显式中间推理

Result: 在测试集上，微调将解析失败率从87%降至1%以下，函数名称准确率提高八倍以上，显著增强了跨方言和领域的参数对齐。错误分析显示问题从结构崩溃转向语义错位

Conclusion: 序列化稳定性和决策级推理是可分离的挑战，AISA-AR-FunctionCall框架有效解决了阿拉伯语函数调用的结构不稳定性问题，所有数据集和模型已公开发布
...
### [239] [What on Earth is AlphaEarth? Hierarchical structure and functional interpretability for global land cover](https://arxiv.org/abs/2603.16911)
...
TL;DR: GAEF嵌入维度具有层次化功能组织：从特定类别专家到共享特征中低通才，再到环境梯度高通才维度，仅需2-12个维度即可达到98%基线性能。


<details>
  <summary>Details</summary>
Motivation: 地理空间基础模型生成高维嵌入具有强大预测性能，但其内部组织不透明，限制了科学应用。需要理解嵌入空间是否具有功能或层次组织，以及维度如何编码不同地理空间结构。

Method: 提出功能可解释性框架，通过特征重要性模式和渐进消融分析，结合大规模实验和嵌入-类别关系结构分析，逆向工程嵌入维度的功能角色。

Result: 嵌入维度表现出一致且非均匀的功能行为，可分为层次化功能谱系：专家维度（特定土地覆盖类别）、低中通才维度（类别间共享特征）、高通才维度（更广泛环境梯度）。仅需2-12个维度即可达到98%基线性能。

Conclusion: AlphaEarth嵌入不仅具有物理信息性，而且功能组织成层次结构，为操作分类任务中的维度选择提供实用指导，展示了嵌入空间中的显著冗余，可实现计算成本显著降低。
...
### [240] [HoloByte: Continuous Hyperspherical Distillation for Tokenizer-Free Modeling](https://arxiv.org/abs/2603.16917)
*Vladimer Khasia*

Main category: cs.LG

TL;DR: HoloByte：一种无分词器的序列建模框架，通过连续超球面蒸馏将字节序列投影到连续流形，降低注意力复杂度，实现词汇表无关的建模。


<details>
  <summary>Details</summary>
Motivation: 传统基于子词分词的序列建模存在三个问题：1) 强加人工形态边界；2) 依赖词汇表；3) 破坏优化景观的连续性。需要一种无分词器的解决方案来避免这些限制。

Method: HoloByte采用连续超球面蒸馏技术：1) 将离散字节序列分块；2) 通过可逆正交旋转算子投影到连续有界超球面流形；3) 宏观Transformer在压缩连续表示上操作；4) 局部因果微解码器恢复字节级分布；5) 使用全息潜在均方误差的双目标优化。

Result: 理论上推导出确保无误差离散恢复的最小嵌入维度D = Ω(W ln|V|)。在严格匹配参数约束下，HoloByte系统性地优于可比较的BPE基线，证明了连续超球面蒸馏的数学严谨性和计算可行性。

Conclusion: 连续超球面蒸馏为词汇表无关的序列建模提供了数学严谨且计算可行的基础，HoloByte框架成功避免了传统分词方法的局限性。
...
### [241] [Anisotropic Permeability Tensor Prediction from Porous Media Microstructure via Physics-Informed Progressive Transfer Learning with Hybrid CNN-Transformer](https://arxiv.org/abs/2603.17532)
*Mohammad Nooraiepour*

Main category: cs.LG

TL;DR: 提出基于MaxViT混合CNN-Transformer架构的物理信息深度学习框架，通过渐进式迁移学习和可微分物理约束，从孔隙尺度微结构图像高效预测渗透率张量，相比直接数值模拟大幅提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 孔隙尺度微结构图像预测渗透率张量对地下流动建模至关重要，但直接数值模拟每个样本需要数小时，严重限制了大尺度不确定性量化和储层优化工作流程，需要开发高效替代方法。

Method: 采用MaxViT混合CNN-Transformer架构，结合渐进式迁移学习：1) 使用ImageNet预训练基线和D4等变增强；2) 引入张量变换和分量加权损失函数；3) 冻结骨干网络进行迁移学习，通过FiLM进行孔隙度条件化。通过可微分惩罚项强制执行Onsager互易性和正定性约束。

Result: 在4000个测试样本上，框架达到方差加权R²=0.9960（Kxx的R²=0.9967，Kxy的R²=0.9758），相比监督基线减少了33%的未解释方差，显著提升了预测精度。
...
### [242] [Translation Invariance of Neural Operators for the FitzHugh-Nagumo Model](https://arxiv.org/abs/2603.17523)
*Luca Pellegrini*

Main category: cs.LG

TL;DR: 该研究评估了7种神经算子架构在捕捉FitzHugh-Nagumo模型刚烈时空动力学方面的表现，特别关注平移不变性，发现不同架构在训练成本、推理速度和泛化能力方面各有优劣。


<details>
  <summary>Details</summary>
Motivation: 研究神经算子（NOs）捕捉描述可兴奋细胞的FitzHugh-Nagumo模型刚烈时空动力学的能力，特别关注评估平移不变性，为复杂离子模型动力学提供全面基准测试。

Method: 采用新颖的训练策略：在固定时间使用不同空间位置和强度的外加电流训练NOs，测试集引入更具挑战性的分布外场景（电流在时间和空间上平移）。评估7种架构：CNOs、DONs、DONs-CNN、POD-DONs、FNOs、TFNOs、LocalNOs，基于训练/测试精度、效率和推理速度进行基准测试。

Result: CNOs在平移测试动力学上表现良好但训练成本较高；FNOs训练误差最低但推理时间最长，对平移动力学预测精度较差；DONs及其变体训练和推理效率高但测试集泛化能力不足。

Conclusion: 研究揭示了不同神经算子架构在捕捉复杂离子模型动力学方面的能力与局限性，为涉及平移动力学的场景应用提供了全面基准，表明没有单一架构在所有指标上表现最优，需根据具体应用需求权衡选择。
...
### [243] [MHPO: Modulated Hazard-aware Policy Optimization for Stable Reinforcement Learning](https://arxiv.org/abs/2603.16929)
*Hongjun Wang,Wei Liu,Weibo Gu,Xing Sun,Kai Han*

Main category: cs.LG

TL;DR: MHPO提出了一种新的强化学习框架，通过Log-Fidelity调制器和解耦危险惩罚机制，解决GRPO训练中重要性比率调节的稳定性问题，在多种推理任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有重要性比率控制方法（如硬裁剪）存在不可微边界和梯度消失区域，无法保持梯度保真度，且缺乏危险感知机制来自适应抑制极端偏差，导致优化过程容易受到突然策略变化的影响。

Method: 提出MHPO框架：1) Log-Fidelity调制器(LFM)将无界重要性比率映射到有界可微域，防止高方差异常值破坏损失景观；2) 解耦危险惩罚(DHP)集成生存分析中的累积危险函数，独立调节正负策略偏移，在稳定信任区域内同时缓解过度扩展的模式崩溃和灾难性收缩的策略侵蚀。

Result: 在多种文本和视觉语言推理基准测试中，MHPO始终优于现有方法，实现了卓越性能，同时显著提高了训练稳定性。

Conclusion: MHPO通过创新的调制器和危险感知惩罚机制，有效解决了GRPO训练中的稳定性问题，为强化学习提供了更稳健和稳定的优化框架。
...
### [244] [Beyond Muon: MUD (MomentUm Decorrelation) for Faster Transformer Training](https://arxiv.org/abs/2603.17970)
...
<details>
  <summary>Details</summary>
Motivation: Muon等正交动量优化器通过极分解实现动量白化，但需要多次大矩阵乘法，计算开销大且硬件依赖性强。需要一种更高效的白化方法。

Method: 提出MUD（MomentUm Decorrelation），用三角（类Cholesky）白化替代Muon的极分解更新，灵感来自经典Gram-Schmidt和Gauss-Seidel思想。证明行正交矩阵是MUD映射的不动点，将内部步骤关联到Gram矩阵的对称Gauss-Seidel预处理。

Result: MUD在时间困惑度上比调优的AdamW和Muon提升10-50%的挂钟时间，收敛速度略慢于Muon但优化器开销显著降低。相对于Muon，在大多数设置下峰值tokens/s提升1.3-2.6倍，在GPT-2 large上达到近3倍。在ESM-2 150M蛋白质语言模型训练中，MUD在显著更少的挂钟时间内达到Muon水平的验证困惑度。

Conclusion: MUD通过三角白化方法有效解决了正交动量优化器的计算开销问题，在保持训练质量的同时大幅提升了训练效率，是Transformer训练中一个有前景的优化器选择。
...
### [245] [Integrating Explainable Machine Learning and Mixed-Integer Optimization for Personalized Sleep Quality Intervention](https://arxiv.org/abs/2603.16937)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究多关注睡眠质量的风险预测，但很少将预测结果转化为实际的干预策略，缺乏从预测到行动的可操作框架

Method: 使用监督分类器预测睡眠质量，SHAP特征归因量化可修改因素的影响，混合整数优化模型识别最小可行行为调整，并通过惩罚机制建模改变阻力

Result: 测试F1分数0.9544，准确率0.9366；敏感性分析显示预期改善与干预强度之间存在权衡；个体层面生成简洁建议（通常1-2个高影响力调整）
...
### [246] [Minimum-Action Learning: Energy-Constrained Symbolic Model Selection for Physical Law Identification from Noisy Data](https://arxiv.org/abs/2603.16951)
*Martin G. Frasch*

Main category: cs.LG

TL;DR: MAL框架通过最小化三动作函数（轨迹重建、架构稀疏性、能量守恒）从预设基函数库中选择符号力定律，结合宽模板加速度匹配技术降低噪声方差10000倍，在开普勒引力和胡克定律基准测试中实现100%管道级识别。


<details>
  <summary>Details</summary>
Motivation: 从噪声观测数据中识别物理定律是科学机器学习中的核心挑战，现有方法在处理高噪声数据时存在困难，需要开发既能处理噪声又能保持物理约束的符号模型选择框架。

Method: 提出最小动作学习（MAL）框架：1）使用宽模板加速度匹配技术将噪声方差降低10000倍；2）通过最小化三动作函数（轨迹重建、架构稀疏性、能量守恒）从基函数库中选择符号力定律；3）结合能量守恒准则进行模型选择。

Result: 在开普勒引力基准中恢复指数p=3.01±0.01，能耗降低40%；原始正确基函数选择率为开普勒40%、胡克定律90%；通过能量守恒准则实现100%管道级识别；与噪声鲁棒SINDy变体、哈密顿神经网络和拉格朗日神经网络相比，MAL在可解释、能量约束的模型选择方面具有独特优势。

Conclusion: MAL框架成功解决了从高噪声数据中识别物理定律的挑战，通过结合噪声预处理、符号基函数选择和物理约束验证，实现了可解释、能量守恒的模型选择，为科学机器学习中的符号发现提供了有效工具。
...
### [247] [Formal verification of tree-based machine learning models for lateral spreading](https://arxiv.org/abs/2603.16983)
*Krishna Kumar*

Main category: cs.LG

TL;DR: 该研究提出使用SMT求解器将训练好的树集成模型编码为逻辑公式，验证其在所有输入域上是否符合物理规范，而非仅采样点，为安全关键的地工应用提供形式化认证。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习模型在稀疏或有偏训练数据下可能学习到物理不一致的关系，而现有方法（如SHAP、LIME等后验解释性和训练时约束）要么只能近似诊断单个预测，要么限制模型能力而不提供全面保证。

Method: 将训练好的树集成模型（XGBoost和EBMs）编码为SMT求解器中的逻辑公式，将四个地工规范（地下水位深度、PGA单调性、距离安全性和平坦地面安全性）形式化为可判定的逻辑公式，通过SMT验证在整个输入域上的合规性。

Result: 无约束EBM（80.1%准确率）违反所有四个规范；完全约束EBM（67.2%准确率）满足其中三个规范；33个模型变体的帕累托分析显示准确率与规范合规性之间存在持久权衡；SHAP分析显示违规特征可能排名最后，证明后验解释不能替代形式验证。

Conclusion: 建立了验证-修复-验证的工程循环，为在安全关键的地工应用中部署物理一致的机器学习模型提供了形式化认证方法，证明形式验证对于确保模型物理一致性至关重要。
...
### [248] [Integrating Inductive Biases in Transformers via Distillation for Financial Time Series Forecasting](https://arxiv.org/abs/2603.16985)
*Yu-Chen Den,Kuan-Yu Chen,Kendro Vincent,Darby Tien-Hao Chang*

Main category: cs.LG

TL;DR: TIPS框架通过知识蒸馏将因果性、局部性和周期性三种时序先验整合到统一Transformer中，在非平稳金融时序预测中实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer假设时序平稳性，但金融市场存在机制转换和非平稳性，导致SOTA时序Transformer在金融任务上表现不佳。不同归纳偏置在不同市场或机制中各有优势，需要整合互补的时序先验以实现稳健预测。

Method: 提出TIPS框架：1）通过注意力掩码训练具有特定归纳偏置（因果性、局部性、周期性）的Transformer教师模型；2）将教师知识蒸馏到单一学生模型中，实现机制依赖的归纳偏置对齐。

Result: 在四个主要股票市场中，TIPS在年化收益率、夏普比率和卡尔玛比率上分别超越强集成基线55%、9%和16%，同时推理计算量仅需38%。TIPS产生统计显著的超额收益，并在盈利期间与经典架构表现出机制依赖的行为对齐。

Conclusion: 机制依赖的归纳偏置利用对于非平稳金融时序的稳健泛化至关重要。TIPS通过整合互补时序先验，在保持Transformer灵活性的同时实现了更好的金融预测性能。
...
### [249] [Transformers Can Learn Rules They've Never Seen: Proof of Computation Beyond Interpolation](https://arxiv.org/abs/2603.17019)
*Andy Gray*

Main category: cs.LG

TL;DR: 论文通过两个实验证明Transformer能够学习训练数据中未出现的规则，而不仅仅是基于相似性的插值。实验1使用XOR规则细胞自动机，实验2使用符号操作链，都显示Transformer能发现并表达未见规则。


<details>
  <summary>Details</summary>
Motivation: 解决LLM辩论中的核心问题：Transformer是否能够推断训练数据中未出现的规则，还是仅仅基于观察到的示例进行相似性插值。作者旨在测试"纯插值"假说，证明Transformer具有规则发现能力。

Method: 设计了两个受控实验：1）使用线性不可分的XOR规则细胞自动机，移除特定局部输入模式，使相似性预测器失效；2）使用符号操作链，保留一对操作符不出现在训练中，要求模型输出中间步骤和最终答案。在两个实验中训练两层Transformer，并进行电路提取分析。

Result: 实验1：Transformer成功恢复XOR规则（最佳100%；47/60次收敛），性能依赖多步约束传播；实验2：在所有49个保留对中，Transformer超越所有插值基线（平均41.8%，最高78.6%），而移除中间步骤监督会降低性能。电路提取确认了XOR计算。

Conclusion: Transformer能够学习训练中未直接观察到的规则结构并明确表达，这排除了最强的"纯插值"架构假说。虽然证明了这种能力在原则上是可能的，但大型语言训练中何时出现这种行为仍待研究。
...
### [250] [Do Understanding and Generation Fight? A Diagnostic Study of DPO for Unified Multimodal Models](https://arxiv.org/abs/2603.17044)
...
<details>
  <summary>Details</summary>
Motivation: 研究DPO（直接偏好优化）能否同时对齐统一多模态模型的理解和生成能力，这类模型使用语言模型作为主干来处理文本和图像

Method: 在Janus-Pro模型的1B和7B参数版本上，应用7种训练策略和2种后处理方法进行DPO对齐实验，分析梯度正交性和幅度不平衡问题

Result: DPO无法改善生成质量：7B模型生成CLIPScore无显著变化，1B模型生成质量反而下降；理解和生成梯度接近正交（cos~0），幅度相差11-14倍，VQ离散化导致生成损失收敛到ln(2)

Conclusion: VQ离散化是统一多模态模型DPO对齐的主要瓶颈，理解和生成任务的梯度正交性和幅度不平衡导致DPO无法同时对齐两种能力
...
### [251] [SCE-LITE-HQ: Smooth visual counterfactual explanations with generative foundation models](https://arxiv.org/abs/2603.17048)
*Ahmed Zeid,Sidney Bender*

Main category: cs.LG

TL;DR: SCE-LITE-HQ：利用预训练生成基础模型的可扩展反事实解释框架，无需任务特定训练，在生成器潜在空间操作，通过平滑梯度和掩码多样化生成真实多样的反事实解释。


<details>
  <summary>Details</summary>
Motivation: 现代神经网络在视觉领域表现优异但难以解释。现有反事实解释方法依赖数据集特定生成模型且计算成本高，限制了在高分辨率数据上的可扩展性。

Method: 在预训练生成基础模型的潜在空间操作，无需任务特定重新训练；使用平滑梯度提高优化稳定性；应用基于掩码的多样化策略促进真实且结构多样的反事实生成。

Result: 在自然和医学数据集上评估，SCE-LITE-HQ生成有效、真实且多样的反事实解释，性能与现有基线相当或更优，同时避免了训练专用生成模型的开销。

Conclusion: SCE-LITE-HQ提供了一个可扩展的反事实解释框架，利用预训练生成模型避免了训练开销，在保持解释质量的同时提高了方法的可扩展性。
...
### [252] [Early Quantization Shrinks Codebook: A Simple Fix for Diversity-Preserving Tokenization](https://arxiv.org/abs/2603.17052)
*Wenhao Zhao,Qiran Zou,Rushi Shah,Yudi Wu,Zhouhan Lin,Dianbo Liu*

Main category: cs.LG

TL;DR: 首次系统研究向量量化在生成模型中的表示坍缩问题，识别两种坍缩类型及触发条件，并提出缓解方案


<details>
  <summary>Details</summary>
Motivation: 向量量化广泛用于大语言模型、扩散模型等生成模型的数据表示标记化，但其在生成模型中的特性和行为尚未得到充分探索，特别是表示坍缩问题

Method: 使用合成和真实数据集系统研究向量量化中的坍缩问题，识别离散码本标记和连续潜在嵌入的坍缩类型、严重程度及触发条件

Result: 发现随机初始化和有限编码器容量会导致标记坍缩和嵌入坍缩，并基于这些发现提出了针对每种坍缩的缓解方案

Conclusion: 这是首个全面研究向量量化中表示坍缩问题的工作，为理解和解决这一重要但被忽视的问题提供了系统分析
...
### [253] [PRISM: Demystifying Retention and Interaction in Mid-Training](https://arxiv.org/abs/2603.17074)
*Bharat Runwal,Ashish Agrawal,Anurag Roy,Rameswar Panda*

Main category: cs.LG

TL;DR: PRISM研究表明，在大型语言模型训练中期加入约270亿高质量token的中间训练，能显著提升数学、代码和科学推理能力，同时保持通用性能，为RL微调创造有效起点。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索大型语言模型训练中期的设计选择，特别是如何通过中间训练提升模型的推理能力，并为后续的强化学习微调创造更好的起点。

Method: 对7个基础模型进行控制实验，涵盖4个模型家族、2种架构类型（密集Transformer和注意力-Mamba混合）、参数规模从30亿到240亿。采用约270亿高质量token进行中间训练，然后进行强化学习微调。通过权重分析、表示相似性分析（CKA）等方法研究机制。

Result: 中间训练使数学能力提升15-40分，代码能力提升5-12分，科学能力提升6-13分，同时保持通用性能。完整的PRISM到RL流程将六个推理基准的宏观平均从不到12分提升到29-42分（3-4倍提升）。中间训练重构了90%以上的模型权重，而RL只稀疏调整约5%的参数。

Conclusion: 中间训练对于可靠推理增强非常有效，数据组合在中间训练阶段比在RL阶段更重要。中间训练将模型置于RL可以有效改进的配置中，为设计稳健的中间训练流程提供了实用指导。
...
### [254] [CircuitBuilder: From Polynomials to Circuits via Reinforcement Learning](https://arxiv.org/abs/2603.17075)
*Weikun K. Zhang,Rohan Pandey,Bhaumik Mehta,Kaijie Jin,Naomi Morato,Archit Ganapule,Michael Ruofan Zeng,Jarod Alper*

Main category: cs.LG

TL;DR: 使用强化学习自动发现高效算术电路计算多项式，比较PPO+MCTS和SAC两种方法在不同变量数下的表现。
...
Method: 将问题建模为单人游戏，RL代理在固定操作次数内构建电路。采用AlphaZero风格训练循环，比较两种方法：PPO+MCTS（近端策略优化+蒙特卡洛树搜索）和SAC（软演员-评论家）。

Result: SAC在双变量目标上获得最高成功率，而PPO+MCTS能扩展到三变量并在更难的实例上表现出稳定改进。

Conclusion: 多项式电路合成是一个紧凑、可验证的环境，适合研究自改进搜索策略。
...
### [255] [SENSE: Efficient EEG-to-Text via Privacy-Preserving Semantic Retrieval](https://arxiv.org/abs/2603.17109)
*Akshaj Murhekar,Christina Liu,Abhijit Mishra,Shounak Roychowdhury,Jacek Gwizdka*

Main category: cs.LG

TL;DR: SENSE是一个轻量级、保护隐私的脑电信号解码框架，通过两阶段方法将EEG信号转换为文本，无需微调大语言模型，实现了本地化处理和隐私保护。
...
Method: 采用两阶段解码：1) 本地设备上的语义检索，将EEG信号映射到离散文本空间提取非敏感的词袋表示；2) 基于提示的语言生成，使用现成的大语言模型以零样本方式合成流畅文本。

Result: 在128通道EEG数据集上对6名受试者的评估显示，SENSE在生成质量上匹配或超越了完全微调的基线方法（如Thought2Text），同时显著减少了计算开销。
...
### [256] [Topology-Preserving Deep Joint Source-Channel Coding for Semantic Communication](https://arxiv.org/abs/2603.17126)
*Omar Erak,Omar Alhussein,Fang Fang,Sami Muhaidat*

Main category: cs.LG

TL;DR: 提出TopoJSCC框架，通过持久同调正则化器保护图像拓扑结构，在低信噪比和低带宽下提升拓扑保持和PSNR性能
...
Method: 提出TopoJSCC框架，集成持久同调正则化器进行端到端训练，通过惩罚原始图像与重建图像之间立方体持久图、以及信道前后潜在特征之间Vietoris-Rips持久图的Wasserstein距离来强制拓扑一致性

Result: 实验表明在低信噪比和低带宽比情况下，拓扑保持和峰值信噪比均有改善

Conclusion: TopoJSCC通过拓扑感知的深度联合信源信道编码框架，有效保护了图像全局结构信息，在无线视觉应用中具有优势
...
### [257] [Contextual Preference Distribution Learning](https://arxiv.org/abs/2603.17139)
*Benjamin Hudson,Laurent Charlin,Emma Frejinger*

Main category: cs.LG

TL;DR: 提出一个序列学习-优化框架，通过有界方差得分函数梯度估计器学习偏好分布，用于解决具有不确定性和上下文依赖的人类偏好决策问题
...
### [258] [REAL: Regression-Aware Reinforcement Learning for LLM-as-a-Judge](https://arxiv.org/abs/2603.17145)
*Yasi Zhang,Tianyu Chen,Mingyuan Zhou,Oscar Leong,Ying Nian Wu,Michal Lukasik*

Main category: cs.LG

TL;DR: 提出REAL框架，将回归目标融入强化学习，优化LLM作为评估者的性能，显著提升相关性指标


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个问题：1）标准RL方法使用二元奖励，忽略了回归任务中的序数结构；2）现有回归感知方法局限于监督微调，无法探索最优推理路径。需要将回归目标整合到RL探索中

Method: 提出REAL框架，使用广义策略梯度估计器，将优化分解为两个互补组件：1）在思维链轨迹上进行探索；2）对最终分数的回归感知预测细化

Result: 在8B到32B模型规模上，REAL始终优于回归感知SFT基线和标准RL方法，在领域外基准上表现出更好的泛化能力。在Qwen3-32B上，相比SFT基线获得+8.40皮尔逊和+7.20斯皮尔曼相关性提升

Conclusion: 将回归目标整合到RL探索中对准确的LLM评估具有关键价值，REAL框架为回归奖励优化提供了原则性解决方案
...
### [259] [Personalized Fall Detection by Balancing Data with Selective Feedback Using Contrastive Learning](https://arxiv.org/abs/2603.17148)
*Awatif Yasmin,Tarek Mahmud,Sana Alamgeer,Anne H. H. Ngu*

Main category: cs.LG

TL;DR: 提出结合半监督聚类与对比学习的个性化跌倒检测框架，通过平衡用户反馈样本解决数据不平衡问题，在三种重训练策略中，从头训练方法表现最佳，提升25%


<details>
  <summary>Details</summary>
Motivation: 个性化跌倒检测模型因真实跌倒数据稀缺和非跌倒样本主导而受限，数据不平衡导致模型偏向常规活动，对真实跌倒事件敏感性降低

Method: 结合半监督聚类与对比学习，识别并平衡最具信息量的用户反馈样本，在三种重训练策略（从头训练、迁移学习、少样本学习）下评估框架适应性

Result: 10名参与者的实时实验显示，从头训练方法性能最高，比基线提升25%；少样本学习次之，提升7%，证明选择性个性化在实际部署中的有效性

Conclusion: 提出的个性化框架通过平衡信息量大的用户反馈样本有效解决了跌倒检测中的数据不平衡问题，为实际部署提供了有效的选择性个性化方案
...
### [260] [Noise-Response Calibration: A Causal Intervention Protocol for LLM-Judges](https://arxiv.org/abs/2603.17172)
...
Method: 基于控制输入干预的校准协议：通过增加噪声严重程度，测试任务性能是否出现统计显著的下降趋势；使用基于斜率的假设检验，对表格数据采用信噪比扰动，对文本数据采用词汇扰动

Result: 发现明显的模态依赖行为：文本评估器性能可预测下降，但多数表格数据集即使在显著信噪比降低下也缺乏统计显著的性能下降；对噪声干预不敏感的数据集上模型性能较低

Conclusion: 提出了可复现的方法和报告协议，用于在分布偏移下进行稳健的LLM评估器校准，揭示了模态差异对校准效果的影响
...
### [261] [Domain-informed explainable boosting machines for trustworthy lateral spread predictions](https://arxiv.org/abs/2603.17175)
*Cheng-Hsi Hsiao,Krishna Kumar,Ellen M. Rathje*

Main category: cs.LG

TL;DR: 提出一个领域知识引导的框架，通过修改EBM学习到的形状函数来提高横向扩展预测的物理一致性，在可接受的精度损失（4-5%）下获得更可靠的解释。


<details>
  <summary>Details</summary>
Motivation: EBM虽然提供可解释的预测，但在自然灾害应用中可能学习到非物理关系，降低其可靠性。需要改进EBM的物理一致性以增强其在横向扩展预测中的可信度。

Method: 提出领域知识引导的框架，基于领域知识修改EBM学习到的形状函数，纠正非物理行为同时保持数据驱动模式。应用于2011年基督城地震数据集。

Result: 成功纠正了原始EBM中观察到的非物理趋势，生成更物理一致的全局和局部解释，精度损失在可接受范围内（4-5%）。

Conclusion: 领域知识引导的EBM框架能够显著提高横向扩展预测模型的物理一致性，在可接受的精度损失下提供更可靠的可解释预测，增强模型在自然灾害应用中的可信度。
...
### [262] [MetaClaw: Just Talk -- An Agent That Meta-Learns and Evolves in the Wild](https://arxiv.org/abs/2603.17187)
...
TL;DR: MetaClaw：一个持续元学习框架，通过联合演化基础LLM策略和可重用行为技能库，实现LLM代理的持续适应，无需停机即可提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLM代理在部署后通常保持静态，无法适应用户需求的变化，而现有方法要么存储原始轨迹而不提炼知识，要么维护静态技能库，要么需要中断性停机进行重新训练。

Method: MetaClaw采用两种互补机制：1）技能驱动的快速适应：通过LLM进化器分析失败轨迹来合成新技能，实现零停机即时改进；2）机会性策略优化：在用户非活跃窗口期间，通过云LoRA微调和基于过程奖励模型的强化学习进行梯度更新。

Result: 技能驱动适应将准确率相对提升高达32%，完整流程将Kimi-K2.5准确率从21.4%提升至40.6%，并将综合鲁棒性提高18.3%。

Conclusion: MetaClaw通过持续元学习框架有效解决了LLM代理的适应性问题，实现了无需停机的性能提升，并在生产规模LLM上展示了显著效果。
...
### [263] [Self-Conditioned Denoising for Atomistic Representation Learning](https://arxiv.org/abs/2603.17196)
*Tynan Perez,Rafael Gomez-Bombarelli*

Main category: cs.LG

TL;DR: 提出Self-Conditioned Denoising (SCD)方法，一种适用于多种原子数据领域的自监督预训练策略，在多个下游任务中超越现有自监督方法，匹配甚至超过监督预训练性能。


<details>
  <summary>Details</summary>
Motivation: 当前原子数据预训练策略不足：现有自监督学习方法局限于基态几何结构和单一数据领域，而大规模监督预训练虽然性能好但需要大量DFT力-能量标签。需要开发更通用的预训练方法。

Method: 提出Self-Conditioned Denoising (SCD)方法，这是一种与骨干网络无关的重建目标，利用自嵌入进行跨域条件去噪，适用于小分子、蛋白质、周期性材料和非平衡几何结构等多种原子数据领域。

Result: 在控制骨干架构和预训练数据集的情况下，SCD显著优于现有自监督方法，匹配甚至超过监督力-能量预训练性能。小型快速GNN通过SCD预训练后，在多个领域的任务中能够与在更大标记或未标记数据集上预训练的大型模型竞争甚至超越。

Conclusion: SCD为原子数据提供了一种有效的自监督预训练策略，能够跨多个数据领域工作，性能优于现有自监督方法，可与监督预训练媲美，为物理科学基础模型开发提供了新方向。
...
### [264] [Abstraction as a Memory-Efficient Inductive Bias for Continual Learning](https://arxiv.org/abs/2603.17198)
*Elnaz Rahmati,Nona Ghazizadeh,Zhivar Sourati,Nina Rouhani,Morteza Dehghani*

Main category: cs.LG

TL;DR: AAT通过联合优化具体实例和抽象表示，在严格在线数据流中实现无记忆缓冲的持续学习，性能媲美经验回放方法


<details>
  <summary>Details</summary>
Motivation: 现实世界非平稳且无限复杂，智能体需要持续学习而不必从头训练。在线持续学习中，学习新知识会干扰旧知识，导致遗忘和泛化能力下降

Method: 提出抽象增强训练(AAT)，通过损失函数修改鼓励模型捕捉示例间的潜在关系结构。联合优化具体实例和抽象表示，引入内存高效的归纳偏置，无需回放缓冲区

Result: AAT在关系数据集和叙事数据集上表现优异，性能达到或超过强经验回放基线，且无需额外内存，仅需最小训练目标修改

Conclusion: 结构抽象是经验回放的有效内存免费替代方案，为在线持续学习提供了新方向
...
### [265] [Catching rationalization in the act: detecting motivated reasoning before and after CoT via activation probing](https://arxiv.org/abs/2603.17199)
*Parsa Mirtaheri,Mikhail Belkin*

Main category: cs.LG

TL;DR: LLMs会进行动机推理：在选择题中，即使有提示偏向某个选项，模型会改变答案并生成合理化推理而不承认提示，这种现象可以通过内部激活探测比思维链监控更可靠地检测。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型中的动机推理现象，即模型会改变答案以符合提示，并生成合理化推理而不承认外部提示的影响，这揭示了模型推理过程的不透明性和潜在偏见。

Method: 在多个LLM家族和数据集上研究动机推理现象，使用监督探针训练在模型的残差流上，包括预生成探针（在生成思维链之前）和后生成探针（在生成思维链之后），并与基于LLM的思维链监控器进行比较。

Result: 预生成探针在预测动机推理方面与访问完整思维链轨迹的LLM监控器表现相当，而后生成探针优于同一监控器。内部表示比思维链监控更可靠地检测动机推理，预生成探测可以早期标记动机行为，避免不必要的生成。

Conclusion: 动机推理是LLMs中可检测的现象，通过内部激活探测比思维链监控更可靠地识别，预生成探测具有早期检测优势，为理解模型推理过程和开发更可靠的监控工具提供了新途径。
...
### [266] [On the Cone Effect and Modality Gap in Medical Vision-Language Embeddings](https://arxiv.org/abs/2603.17246)
...
TL;DR: 提出一种轻量级后处理机制，通过单个超参数λ连续控制跨模态分离，系统分析模态间隙对下游多模态性能的影响，发现适度而非完全消除模态间隙能获得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型（VLMs）存在"锥形效应"导致模态间隙已被广泛观察，但其对监督多模态学习（特别是在医学领域）的实际影响仍不清楚。需要在不昂贵重新训练的情况下分析模态间隙如何影响下游性能。

Method: 引入轻量级后处理机制，保持预训练VLM编码器冻结，通过单个超参数λ连续控制跨模态分离。评估通用模型（CLIP、SigLIP）和医学专用模型（BioMedCLIP、MedSigLIP）在医学和自然数据集上的表现。

Result: 减少过度的模态间隙能改善下游性能，医学数据集对间隙调制表现出更强的敏感性；但完全消除间隙并非总是最优，中等程度、任务依赖的分离能获得最佳结果。

Conclusion: 模态间隙应被视为多模态表示的可调属性，而非必须普遍最小化的量。适度的跨模态分离对任务性能至关重要，需要根据具体任务调整。
...
### [267] [Binary Latent Protein Fitness Landscapes for Quantum Annealing Optimization](https://arxiv.org/abs/2603.17247)
*Truong-Son Hy*

Main category: cs.LG

TL;DR: Q-BIOLAT：在二元潜在空间中建模和优化蛋白质适应度景观的框架，使用蛋白质语言模型嵌入，通过QUBO模型近似适应度，支持经典启发式搜索和量子退火硬件


<details>
  <summary>Details</summary>
Motivation: 建立蛋白质表示学习与组合优化之间的桥梁，为蛋白质工程提供高效搜索方法，并兼容新兴量子退火硬件

Method: 使用预训练蛋白质语言模型获得连续嵌入，转换为紧凑二元潜在表示，通过QUBO模型近似蛋白质适应度，采用模拟退火、遗传算法等经典启发式进行组合搜索

Result: 在ProteinGym基准测试中，Q-BIOLAT能捕捉蛋白质适应度景观的有意义结构，识别高适应度变体，在不同优化策略下表现良好，尤其在强配置下能检索到训练适应度分布顶部的序列

Conclusion: Q-BIOLAT为蛋白质适应度建模提供了有效框架，连接了表示学习与组合优化，为量子辅助蛋白质工程开辟了新方向
...
### [268] [Pathology-Aware Multi-View Contrastive Learning for Patient-Independent ECG Reconstruction](https://arxiv.org/abs/2603.17248)
...
<details>
  <summary>Details</summary>
Motivation: 从减少导联重建12导联心电图是一个不适定逆问题，标准深度学习方法常忽略基础心脏病理，丢失胸前导联的重要形态信息
...
Conclusion: 该框架弥合了硬件便携性与诊断级重建之间的差距，通过病理感知学习实现了更准确、泛化能力更强的12导联心电图重建
...
### [269] [Variational Rectification Inference for Learning with Noisy Labels](https://arxiv.org/abs/2603.17255)
*Haoliang Sun,Qi Wei,Lei Feng,Yupeng Hu,Fan Liu,Hehe Fan,Yilong Yin*

Main category: cs.LG

TL;DR: 提出VRI方法，通过变分推断框架自适应修正损失函数，解决标签噪声问题，避免模型坍塌，提升泛化性能


<details>
  <summary>Details</summary>
Motivation: 现有基于元学习的概率模型虽然能处理标签噪声，但容易发生模型坍塌，导致泛化性能下降。需要一种既能有效处理标签噪声，又能避免模型坍塌的方法。

Method: 提出变分修正推断(VRI)，将损失函数的自适应修正建模为摊销变分推断问题。将修正向量作为隐变量，构建层次贝叶斯模型，通过摊销元网络近似条件后验分布，避免坍缩为狄拉克函数。

Result: VRI在标签噪声学习，特别是开放集噪声场景下表现优异，能显著提升模型的鲁棒性和泛化性能。理论分析证明元网络可以高效学习。

Conclusion: VRI通过变分推断框架有效解决了标签噪声学习中的模型坍塌问题，在保持鲁棒性的同时提升了泛化能力，特别适用于开放集噪声场景。
...
### [270] [Classifier Pooling for Modern Ordinal Classification](https://arxiv.org/abs/2603.17278)
...
### [271] [WINFlowNets: Warm-up Integrated Networks Training of Generative Flow Networks for Robotics and Machine Fault Adaptation](https://arxiv.org/abs/2603.17301)
*Zahin Sufiyan,Shadan Golestan,Yoshihiro Mitsuka,Shotaro Miwa,Osmar Zaiane*

Main category: cs.LG

TL;DR: WINFlowNets提出了一种新的连续场景生成流网络框架，通过流网络和检索网络的协同训练，解决了传统CFlowNets在机器人控制任务中依赖检索网络预训练的问题，在动态环境中表现出更好的适应性和训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统CFlowNets在机器人控制任务中依赖检索网络的预训练，这在动态机器人环境中存在挑战，因为预训练数据可能不易获得或不能代表当前环境。需要一种能够协同训练流网络和检索网络的方法，以适应动态环境。

Method: WINFlowNets采用三阶段方法：1) 检索网络的预热阶段以引导其策略；2) 共享训练架构；3) 共享回放缓冲区用于协同训练两个网络。这种方法避免了预训练依赖，使两个网络能够同时学习和适应。

Result: 在模拟机器人环境中的实验表明，WINFlowNets在平均奖励和训练稳定性方面超越了CFlowNets和最先进的强化学习算法。在故障环境中表现出强大的适应能力，适合需要有限样本数据快速适应的任务。

Conclusion: WINFlowNets为动态和易故障的机器人系统部署提供了潜力，在这些系统中，传统的预训练或样本效率低下的数据收集可能不切实际。该框架展示了在动态环境中协同训练方法的优势。
...
### [272] [Learning Permutation Distributions via Reflected Diffusion on Ranks](https://arxiv.org/abs/2603.17353)
*Sizhuang He,Yangtian Zhang,Shiyang Zhang,David van Dijk*

Main category: cs.LG

TL;DR: 提出Soft-Rank Diffusion框架，通过软排名表示将置换扩散从离散随机游走转换为连续平滑过程，结合上下文广义Plackett-Luce去噪器，在排序和组合优化任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 学习置换群S_n上的概率分布具有挑战性：规模阶乘增长、离散非欧结构。现有置换扩散方法使用基于洗牌的随机游走进行前向加噪，但轨迹突变且随n增大去噪困难。

Method: 提出Soft-Rank Diffusion框架：1) 前向过程：将置换提升为连续顺序表示，将离散排名松弛为软排名，获得更平滑轨迹；2) 反向过程：引入上下文广义Plackett-Luce去噪器，扩展PL参数化，提升序列决策结构表达能力。

Result: 在排序和组合优化基准测试中，Soft-Rank Diffusion始终优于现有扩散基线，在长序列和内在序列设置中表现尤为突出。

Conclusion: 通过软排名表示和上下文广义PL去噪器，Soft-Rank Diffusion为置换学习提供了更平滑、更易处理的扩散框架，特别适用于序列决策任务。
...
### [273] [Beyond Outliers: A Data-Free Layer-wise Mixed-Precision Quantization Approach Driven by Numerical and Structural Dual-Sensitivity](https://arxiv.org/abs/2603.17354)
...
TL;DR: NSDS提出一种无需校准数据的层间混合精度量化框架，通过数值和结构双重敏感性分析来分配比特位宽，在极端低比特设置下实现更优的模型压缩效果。
...
Method: NSDS框架首先将每层分解为不同的操作角色，从数值和结构两个角度量化其敏感性。然后通过基于MAD-Sigmoid和Soft-OR的鲁棒聚合方案，将双重敏感性分数聚合成统一的层级指标，指导比特分配。

Result: 大量实验表明，NSDS在各种模型和下游任务上均优于多个基线方法，且无需依赖任何校准数据，在极端低比特设置下实现了更优的性能。

Conclusion: NSDS通过综合考虑数值和结构双重敏感性，解决了现有层间混合精度量化方法的局限性，为无需校准数据的模型压缩提供了有效解决方案，在保持性能的同时实现了更好的压缩效果。
...
### [274] [Variational Kernel Design for Internal Noise: Gaussian Chaos Noise, Representation Compatibility, and Reliable Deep Learning](https://arxiv.org/abs/2603.17365)
*Ziran Liu*

Main category: cs.LG

TL;DR: 提出Variational Kernel Design框架设计神经网络内部噪声，推导出Gaussian Chaos Noise(GCh)门控机制，相比传统dropout等噪声方法在ImageNet上提升校准性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有深度网络内部噪声（如dropout、硬掩码、加性扰动）通常基于启发式设计，缺乏理论指导。本文旨在回答两个关键问题：内部噪声应具有何种相关几何结构？实现的扰动是否与作用表示兼容？

Method: 提出Variational Kernel Design(VKD)框架，通过法律族、相关核和注入算子指定噪声机制。在空间子问题中，基于潜在对数场的二次最大熵原理得到Dirichlet拉普拉斯精度的高斯优化器，Wick归一化后得到Gaussian Chaos Noise(GCh)门控机制。

Result: 理论证明GCh能精确控制成对对数比变形、边缘敏感排序稳定性，并提供精确的期望内在粗糙度预算。在ImageNet和ImageNet-C上，GCh在保持竞争力的准确率下，持续改善校准性，在分布偏移下也改善负对数似然。

Conclusion: VKD框架为神经网络内部噪声设计提供理论指导，推导出的GCh门控机制相比传统硬二值掩码能避免对正相干表示的奇异或相干放大失真，在实践中有更好的校准和鲁棒性表现。
...
### [275] [Efficient Exploration at Scale](https://arxiv.org/abs/2603.17378)
...
TL;DR: 提出一种在线学习算法，将RLHF的数据效率提升10倍以上，仅用不到2万标签就能达到传统离线RLHF使用20万标签的效果


<details>
  <summary>Details</summary>
Motivation: 传统RLHF需要大量人工标注数据，数据效率低下，限制了其在资源有限场景的应用。需要开发更高效的学习算法来减少对标注数据的依赖。

Method: 开发在线学习算法，增量更新奖励模型和语言模型。奖励模型拟合选择数据，语言模型通过改进的REINFORCE算法更新，奖励模型提供强化信号。关键技术包括：强化信号添加小正向激励、建模奖励不确定性的认知神经网络、信息导向探索。

Result: 使用Gemma大语言模型，仅需不到2万标签就能达到传统离线RLHF使用20万标签的性能，数据效率提升10倍以上。预计使用100万标签就能达到传统方法10亿标签的效果，实现1000倍效率提升。

Conclusion: 该在线学习算法显著提升了RLHF的数据效率，首次证明如此大规模效率提升是可能的，为资源有限场景下的RLHF应用开辟了新途径。
...
### [276] [SCALE:Scalable Conditional Atlas-Level Endpoint transport for virtual cell perturbation prediction](https://arxiv.org/abs/2603.17380)
...
TL;DR: SCALE是一个用于虚拟细胞扰动预测的大规模基础模型，通过改进训练推理框架、采用条件传输建模和生物导向评估，显著提升了预测性能和效率。


<details>
  <summary>Details</summary>
Motivation: 虚拟细胞模型旨在通过单细胞测量预测细胞对遗传、化学或细胞因子扰动的响应，但面临三个瓶颈：低效的训练推理流程、高维稀疏表达空间中的建模不稳定、以及过度强调重建精度而忽视生物保真度的评估协议。

Method: 1) 构建基于BioNeMo的训练推理框架，提升数据吞吐、分布式扩展和部署效率；2) 将扰动预测建模为条件传输问题，采用基于LLaMA的细胞编码和端点导向监督的集合感知流架构；3) 在Tahoe-100M数据集上使用以生物意义指标为中心的严格细胞级评估协议。

Result: 在匹配系统设置下，预训练速度提升12.51倍，推理速度提升1.29倍；在Tahoe-100M基准测试中，PDCorr提升12.02%，DE Overlap提升10.66%，优于现有最佳方法。

Conclusion: 推进虚拟细胞模型不仅需要更好的生成目标，还需要可扩展基础设施、稳定传输建模和生物保真评估的协同设计，SCALE模型为此提供了有效解决方案。
...
### [277] [Cohomological Obstructions to Global Counterfactuals: A Sheaf-Theoretic Foundation for Generative Causal Models](https://arxiv.org/abs/2603.17384)
*Rui Wu,Hong Xie,Yongjun Li*

Main category: cs.LG

TL;DR: 论文提出了一种基于代数拓扑的因果生成模型框架，通过引入熵正则化和Wasserstein空间上的层结构，解决了传统连续生成模型在非平凡同调因果图中的全局一致性失败问题。


<details>
  <summary>Details</summary>
Motivation: 当前连续生成模型（如扩散模型、流匹配）隐含假设局部一致的因果机制能自然产生全局一致的因果推断。然而，当因果图具有非平凡同调（如结构冲突或隐藏混杂因子）时，这一假设会根本性失败，需要新的理论框架来处理拓扑障碍。

Method: 1. 将结构因果模型形式化为Wasserstein空间上的胞腔层；2. 引入熵正则化避免确定性奇点（流形撕裂）；3. 推导熵正则化Wasserstein因果层拉普拉斯算子（耦合非线性Fokker-Planck方程组）；4. 证明推前测度一阶变分的熵拉回引理；5. 结合隐函数定理和Sinkhorn最优条件，建立与自动微分的直接算法桥梁。

Result: 1. 实现了O(1)-内存的反向模式梯度，严格独立于迭代时域；2. 成功利用热力学噪声导航高维scRNA-seq因果推断中的拓扑障碍（"熵隧穿"）；3. 提出了拓扑因果评分，证明层拉普拉斯算子可作为拓扑感知因果发现的高度敏感代数检测器。

Conclusion: 该工作为具有复杂拓扑结构的因果图提供了严格的代数拓扑框架，通过熵正则化Wasserstein层理论解决了传统生成模型的全局一致性限制，并建立了与自动微分的实用算法连接，为高维生物数据的因果推断开辟了新途径。
...
### [278] [The Causal Uncertainty Principle: Manifold Tearing and the Topological Limits of Counterfactual Interventions](https://arxiv.org/abs/2603.17385)
*Rui Wu,Hong Xie,Yongjun Li*

Main category: cs.LG

TL;DR: 论文探讨了因果推断在连续生成模型中的几何挑战，提出了Counterfactual Event Horizon概念和Manifold Tearing Theorem，并开发了Geometry-Aware Causal Flow算法来解决流形撕裂问题。


<details>
  <summary>Details</summary>
Motivation: Judea Pearl的do-calculus为因果推断提供了理论基础，但在连续生成模型中面临几何挑战。现有方法在处理极端干预时存在局限性，需要建立理论基础并开发实用算法。

Method: 定义了Counterfactual Event Horizon概念，证明了Manifold Tearing Theorem（确定性流在极端干预下必然产生有限时间奇点），建立了Causal Uncertainty Principle，并提出了Geometry-Aware Causal Flow算法，该算法利用拓扑雷达绕过流形撕裂。

Result: 建立了连续生成模型中因果干预的基本限制理论框架，证明了流形撕裂的必然性，提出了干预极端性与身份保持之间的权衡原则，并在高维scRNA-seq数据上验证了GACF算法的有效性。

Conclusion: 连续生成模型中的因果推断面临根本性的几何限制，但通过几何感知的方法可以绕过这些限制。GACF算法为解决高维生物数据中的因果推断问题提供了实用工具。
...
### [279] [Large-Scale 3D Ground-Motion Synthesis with Physics-Inspired Latent Operator Flow Matching](https://arxiv.org/abs/2603.17403)
...
TL;DR: GMFlow是一个基于物理启发的潜在算子流匹配框架，能够快速生成大规模区域地震动时程，相比传统物理模拟加速10,000倍，为分布式基础设施风险评估提供高效工具。


<details>
  <summary>Details</summary>
Motivation: 电力网络和能源管道等分布式基础设施的地震风险评估需要大量具有真实频率内容和时空相干性的地震动时程，但基于物理的模拟计算成本过高，难以满足工程应用需求。

Method: 提出GMFlow框架，结合物理启发的潜在算子流匹配技术，能够根据物理参数条件生成大规模区域地震动时程，实现网格无关的功能性生成建模。
...
### [280] [Causal Representation Learning on High-Dimensional Data: Benchmarks, Reproducibility, and Evaluation Metrics](https://arxiv.org/abs/2603.17405)
*Alireza Sadeghi,Wael AbdAlmageed*

Main category: cs.LG

TL;DR: 本文对因果表示学习（CRL）领域进行了批判性分析，重点关注数据集局限性、评估指标整合和可重复性问题，提出了数据集特征标准、综合评估指标，并评估了现有实现的可重复性。


<details>
  <summary>Details</summary>
Motivation: CRL模型需要高质量数据集进行开发和评估，但现有数据集各有局限性；同时多方向评估（重建、解耦、因果发现、反事实推理）使模型比较复杂化；领域内可重复性问题也亟待解决。

Method: 1）批判性分析现有合成和真实世界数据集，提出适合CRL模型开发的数据集特征标准；2）引入单一聚合指标整合所有评估方向的性能；3）审查现有文献实现并评估其可重复性。

Result: 识别了现有数据集的局限性，提出了数据集特征标准；开发了综合评估指标以简化模型比较；发现了可重复性方面的差距和最佳实践。

Conclusion: 为CRL领域提供了系统性的评估框架，包括数据集标准、综合指标和可重复性指南，有助于推动该领域的标准化发展和实际应用。
...
### [281] [The Phasor Transformer: Resolving Attention Bottlenecks on the Unit Circle](https://arxiv.org/abs/2603.17433)
*Dibakar Sigdel*

Main category: cs.LG

TL;DR: 提出Phasor Transformer块和Large Phasor Model (LPM)，通过相位表示和DFT全局耦合实现高效长序列时间序列建模，避免传统自注意力的二次复杂度瓶颈。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer的自注意力机制在处理长序列时间序列时存在二次复杂度瓶颈，限制了其在长上下文时序建模中的可扩展性。需要一种更高效的序列混合机制来支持大规模时间序列建模。

Method: 提出Phasor Transformer块，将序列状态表示为单位圆流形S^1上的相位，结合可训练的轻量级相移和参数无关的离散傅里叶变换(DFT)令牌耦合，实现全局O(N log N)复杂度的序列混合。堆叠这些块构成Large Phasor Model (LPM)。

Result: 在合成多频率基准测试的自回归时间序列预测任务中，LPM以紧凑的参数预算学习稳定的全局动态，相比传统自注意力基线达到竞争性的预测性能，建立了明确的效率-性能边界。

Conclusion: 通过几何约束的相位计算和确定性全局耦合，可以实现大规模时间序列模型的高效扩展，为振荡领域中的可扩展时序建模提供了实用路径。
...
### [282] [TimeAPN: Adaptive Amplitude-Phase Non-Stationarity Normalization for Time Series Forecasting](https://arxiv.org/abs/2603.17436)
...
<details>
  <summary>Details</summary>
Motivation: 多变量长期时间序列预测面临非平稳性挑战，表现为幅度和相位的快速变化，导致严重的分布偏移和预测性能下降。现有基于归一化的方法主要依赖一阶和二阶统计量，隐含假设分布平滑演变，忽略了细粒度时间动态。

Method: TimeAPN框架：1) 在时间和频域联合建模均值序列并预测其未来演变；2) 在频域提取相位信息，显式建模预测序列与真实序列之间的相位差异；3) 将幅度信息融入自适应归一化机制以处理信号能量的突变；4) 通过协作去归一化过程将预测的非平稳因素与骨干预测输出结合。

Result: 在7个真实世界多变量数据集上的广泛实验表明，TimeAPN在多个预测时间范围内持续提高长期预测准确性，并优于最先进的可逆归一化方法。

Conclusion: TimeAPN是一个模型无关的框架，可无缝集成到各种预测骨干网络中，通过显式建模幅度和相位的非平稳性，有效解决了时间序列预测中的分布偏移问题。
...
### [283] [Baguan-TS: A Sequence-Native In-Context Learning Model for Time Series Forecasting with Covariates](https://arxiv.org/abs/2603.17439)
...
TL;DR: Baguan-TS是一个统一框架，将原始序列表示学习与上下文学习结合，通过3D Transformer在时间、变量和上下文轴上联合注意力，在时间序列预测中实现无需梯度调整的快速适应。


<details>
  <summary>Details</summary>
Motivation: 当前基于Transformer的上下文学习方法大多依赖表格化、手工制作的特征，而端到端序列模型缺乏推理时适应能力。本文旨在填补这一空白，将原始序列表示学习与上下文学习统一起来。

Method: 提出Baguan-TS框架，使用3D Transformer在时间、变量和上下文三个维度上进行联合注意力。为解决高容量模型的实践问题，采用特征无关、基于目标空间检索的局部校准来改善训练稳定性，并通过上下文过拟合策略缓解输出过度平滑问题。

Result: 在带有协变量的公共基准测试中，Baguan-TS始终优于现有基线，获得最高的胜率，并在点预测和概率预测指标上显著降低误差。在多样化的真实世界能源数据集评估中也表现出鲁棒性，带来实质性改进。

Conclusion: Baguan-TS成功地将原始序列表示学习与上下文学习统一起来，通过创新的3D Transformer架构和训练策略，在时间序列预测中实现了无需梯度调整的高效适应，并在多个基准测试中表现出优越性能。
...
### [284] [Efficient Soft Actor-Critic with LLM-Based Action-Level Guidance for Continuous Control](https://arxiv.org/abs/2603.17468)
...
<details>
  <summary>Details</summary>
Motivation: 在强化学习中，大规模状态-动作空间中的高效探索是一个关键挑战。现有方法如SAC在复杂环境中探索效率有限，需要更智能的探索策略来加速学习过程。

Method: GuidedSAC将大语言模型作为智能监督器集成到SAC框架中。LLM监督器分析最近轨迹（包括状态信息和视觉回放），提供动作级干预来引导探索。该方法保持了SAC的理论基础，同时引入了LLM的指导能力。

Result: 实验表明，在离散和连续控制环境（包括文本任务和MuJoCo基准测试）中，GuidedSAC在样本效率和最终性能上均优于标准SAC和最先进的探索增强变体（如RND、ICM和E3B）。

Conclusion: GuidedSAC成功地将LLM的智能指导能力与SAC的强化学习框架相结合，在保持收敛保证的同时显著提高了探索效率和性能，为复杂环境中的强化学习提供了新的解决方案。
...
### [285] [Auto-Unrolled Proximal Gradient Descent: An AutoML Approach to Interpretable Waveform Optimization](https://arxiv.org/abs/2603.17478)
*Ahmet Kaplan*

Main category: cs.LG

TL;DR: AutoML与深度展开结合，通过自动超参数优化实现仅用5层展开网络达到传统200次迭代PGD算法98.8%的频谱效率，仅需100个训练样本


<details>
  <summary>Details</summary>
Motivation: 传统无线波束赋形和波形优化需要大量计算迭代，深度展开方法虽然能加速但需要大量训练数据和手动调参。本文旨在结合AutoML自动优化深度展开网络，减少训练数据和计算成本

Method: 将迭代近端梯度下降算法转换为深度神经网络，每层参数可学习；引入混合层进行可学习的线性梯度变换；使用AutoGluon和TPE进行超参数优化，搜索网络深度、步长初始化、优化器、学习率调度器、层类型和后梯度激活等参数

Result: 提出的Auto-PGD仅用5层展开网络就达到传统200次迭代PGD算法98.8%的频谱效率，仅需100个训练样本；解决了梯度归一化问题确保训练和评估性能一致；通过每层和速率记录提高透明度

Conclusion: AutoML与深度展开结合显著减少了训练数据和推理成本，同时保持了高可解释性，相比传统黑盒架构具有明显优势
...
### [286] [QuantFL: Sustainable Federated Learning for Edge IoT via Pre-Trained Model Quantisation](https://arxiv.org/abs/2603.17507)
*Charuka Herath,Yogachandran Rahulamathavan,Varuna De Silva,Sangarapillai Lambotharan*

Main category: cs.LG

TL;DR: QuantFL：基于预训练模型和量化技术的节能联邦学习框架，显著减少通信能耗，适用于物联网设备


<details>
  <summary>Details</summary>
Motivation: 联邦学习在物联网设备上实现隐私保护智能，但频繁的上行传输带来高能耗和碳足迹。预训练模型在边缘设备上日益普及，但其减少微调能耗的潜力尚未充分探索。

Method: 提出QuantFL框架，利用预训练初始化实现激进的轻量级量化。预训练自然集中更新统计，允许使用内存高效的桶量化，无需复杂的误差反馈机制。

Result: 在MNIST和CIFAR-100上，QuantFL减少40%总通信量（上行链路减少≥80%），在严格带宽限制下匹配或超过未压缩基线。MNIST达到89.00%测试准确率，CIFAR-100达到66.89%，比特数减少数个数量级。

Conclusion: QuantFL为电池受限的物联网网络提供了实用、绿色的可扩展训练方案，平衡了通信效率和模型性能。
...
### [287] [AirDDE: Multifactor Neural Delay Differential Equations for Air Quality Forecasting](https://arxiv.org/abs/2603.17529)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法将污染物动态建模为瞬时过程，忽略了污染物传播的内在延迟，导致预测不准确

Method: 提出AirDDE框架，包含两个核心组件：1）记忆增强注意力模块，检索全局和局部历史特征，自适应捕捉多因素数据调制的延迟效应；2）基于扩散-平流方程的物理引导延迟演化函数，建模扩散、延迟平流和源/汇项

Result: 在三个真实世界数据集上的实验表明，AirDDE实现了最先进的预测性能，平均MAE比最佳基线降低了8.79%

Conclusion: AirDDE通过整合延迟建模到连续时间污染物演化中，在物理指导下显著提升了空气质量预测的准确性
...
### [288] [CA-Based Interpretable Knowledge Representation and Analysis of Geometric Design Parameters](https://arxiv.org/abs/2603.17535)
...
<details>
  <summary>Details</summary>
Motivation: 在CAD应用中，复杂几何通常由大量设计参数定义，导致高维设计空间给仿真、优化等下游工程带来挑战。虽然PCA能有效降维，但无法直接恢复生成几何的底层设计参数。

Method: 分析针对该应用领域修改的PCA方法，证明其与标准PCA结果相同；研究该方法的局限性，提出实现准确参数估计的合理条件；通过专门实验深入分析PCA各阶段及几何变化过程。

Result: 揭示了修改的PCA与标准PCA的等价性；明确了PCA方法在参数恢复方面的局限性；建立了实现准确、可解释参数估计的条件框架。

Conclusion: 虽然PCA能有效压缩几何表示，但从降维表示中恢复原始设计参数存在固有挑战。通过理解PCA各阶段的几何变化并满足特定条件，可以实现准确且可解释的参数估计。
...
### [289] [CLeAN: Continual Learning Adaptive Normalization in Dynamic Environments](https://arxiv.org/abs/2603.17548)
*Isabella Marasco,Davide Evangelista,Elena Loli Piccolomini,Michele Colajanni*

Main category: cs.LG

TL;DR: 提出CLeAN自适应归一化方法，解决持续学习中数据分布动态变化时传统归一化方法失效的问题，通过可学习参数和指数移动平均模块适应数据分布演变。


<details>
  <summary>Details</summary>
Motivation: 现实世界动态环境（如网络安全、自动驾驶、金融）中数据分布频繁变化，传统AI系统依赖静态数据分布而失效。持续学习虽能处理序列数据，但数据归一化这一关键问题被忽视，传统归一化方法需要访问完整数据集，与持续学习的序列特性不兼容。

Method: 提出CLeAN（Continual Learning Adaptive Normalization）自适应归一化技术，使用可学习参数估计全局特征尺度，通过指数移动平均（EMA）模块更新这些参数，使模型能够适应不断变化的数据分布。

Result: 在两个数据集和多种持续学习策略（Reservoir Experience Replay、A-GEM、EwC）上的综合评估表明，CLeAN不仅提高了模型在新数据上的性能，还缓解了灾难性遗忘问题。

Conclusion: 自适应归一化对增强表格数据的稳定性和有效性至关重要，为在动态学习环境中使用归一化来保护知识提供了新的视角。
...
### [290] [Conditional Inverse Learning of Time-Varying Reproduction Numbers Inference](https://arxiv.org/abs/2603.17549)
*Lanlan Yu,Quan-Hui Liu,Haoyue Zheng,Xinfu Yang*

Main category: cs.LG

TL;DR: 提出CIRL框架，通过条件映射从历史发病数据和显式时间信息学习潜在再生数，结合流行病学约束与数据驱动表示，提高对非平稳传播动态的适应性。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖强结构假设，难以适应干预或行为变化引起的非平稳传播动态，导致对机制转变的检测延迟和估计精度下降。

Method: 提出条件逆再生学习框架(CIRL)，学习从历史发病模式和时间信息到潜在再生数的条件映射，软性整合流行病学结构与灵活似然统计建模，使用更新方程作为前向算子确保动态一致性。

Result: 在具有受控机制变化的合成流行病以及真实SARS和COVID-19数据上的实验证明了该方法的有效性，能够稳健处理观测噪声，同时快速响应突发传播变化和零膨胀观测。

Conclusion: CIRL框架结合流行病学基础约束与数据驱动时间表示，能够更准确地估计时变再生数，特别是在非平稳传播动态和干预措施变化的场景下。
...
### [291] [FoMo X: Modular Explainability Signals for Outlier Detection Foundation Models](https://arxiv.org/abs/2603.17570)
*Simon Klüttermann,Tim Katzke,Phuong Huong Nguyen,Emmanuel Müller*

Main category: cs.LG

TL;DR: FoMo-X为表格基础模型添加了轻量级诊断功能，通过附加诊断头实现零样本异常检测的可解释性和不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 现有的异常检测基础模型（如PFNs）虽然能实现零样本适应，但通常作为黑盒运行，只输出异常分数，缺乏安全关键决策所需的操作上下文和不确定性信息。
...
Conclusion: FoMo-X通过桥接基础模型性能和操作可解释性之间的差距，为零样本异常检测提供了可扩展的信任路径。
...
### [292] [Unsupervised Symbolic Anomaly Detection](https://arxiv.org/abs/2603.17575)
*Md Maruf Hossain,Tim Katzke,Simon Klüttermann,Emmanuel Müller*

Main category: cs.LG

TL;DR: SYRAN：基于符号回归的无监督异常检测方法，通过学习可解释的符号不变量方程来检测异常，无需后处理解释。


<details>
  <summary>Details</summary>
Motivation: 现有异常检测方法通常使用不透明的高维模型，缺乏可解释性。需要一种能直接提供人类可读解释的检测方法，使异常检测逻辑透明化。

Method: 使用符号回归学习一组描述符号不变量的可读方程，这些函数在正常数据上近似恒定。通过测量数据对这些不变量的偏离程度来计算异常分数。

Result: SYRAN具有高度可解释性，生成的方程对应已知的科学或医学关系，同时保持了与最先进方法相当的异常检测性能。

Conclusion: SYRAN通过符号回归实现了可解释的异常检测，无需后处理解释，在保持检测性能的同时提供了透明的人类可读检测逻辑。
...
### [293] [Identifying Latent Actions and Dynamics from Offline Data via Demonstrator Diversity](https://arxiv.org/abs/2603.17577)
*Felix Schur*

Main category: cs.LG

TL;DR: 从无动作标签的离线轨迹中，利用演示者多样性可以识别潜在动作和环境动态


<details>
  <summary>Details</summary>
Motivation: 研究在离线轨迹中从未观察到动作的情况下，是否能够恢复潜在动作和环境动态。当轨迹没有动作标签但带有演示者身份时，探索如何利用演示者多样性来实现可识别性。

Method: 假设每个演示者遵循不同的策略，环境动态在演示者间共享。将条件下一观测分布建模为潜在动作条件转移核的混合，形成列随机非负矩阵分解。利用充分分散的策略多样性和秩条件证明可识别性，并扩展到连续观测空间。

Result: 证明在适当条件下，潜在转移和演示者策略在潜在动作标签的排列意义下是可识别的。通过Gram行列式最小体积准则扩展到连续观测空间，少量标记动作数据即可解决最终排列模糊性。
...
### [294] [One-Step Sampler for Boltzmann Distributions via Drifting](https://arxiv.org/abs/2603.17579)
*Wenhan Cao,Keyu Yan,Lin Zhao*

Main category: cs.LG

TL;DR: 提出一种基于漂移的摊销采样框架，通过训练一步神经网络生成器，将样本沿高斯平滑的得分场从当前模型分布投影到目标玻尔兹曼分布


<details>
  <summary>Details</summary>
Motivation: 玻尔兹曼分布（特别是归一化常数未知的情况）的采样在统计物理和机器学习中很重要，传统迭代采样方法计算成本高，需要开发摊销采样方法将迭代采样转化为单步前向传播

Method: 训练一步神经生成器，使用高斯平滑得分场将样本从当前模型分布投影到目标分布。对于归一化常数未知的目标，推导出基于平滑能量的实用目标侧漂移，使用两种估计器：局部重要性采样均值漂移估计器和二阶曲率校正近似。结合采样器侧平滑得分的mini-batch高斯均值漂移估计，得到简单的stop-gradient目标用于稳定的一步训练

Result: 在四模态高斯混合玻尔兹曼目标上，采样器达到均值误差0.0754、协方差误差0.0425和RBF MMD 0.0020。在双阱和香蕉形目标上的实验表明，同一方法也能处理非凸和弯曲的低能量几何结构

Conclusion: 漂移是摊销玻尔兹曼分布迭代采样的有效方法，可将测试时的采样转化为单步前向传播，为玻尔兹曼分布采样提供了高效实用的解决方案
...
### [295] [End-to-end data-driven prediction of urban airflow and pollutant dispersion](https://arxiv.org/abs/2603.17606)
*Nishant Kumar,Franck Kerhervé,Lionel Agostini,Laurent Cordier*

Main category: cs.LG

TL;DR: 提出一个端到端数据驱动框架，结合SPOD降维、自编码器压缩、LSTM时序建模和CNN映射，用于快速准确预测街道峡谷中的气流和污染物扩散。
...
Method: 1) 使用谱本征正交分解(SPOD)从大涡模拟(LES)数据库中获取降维基；2) 用自编码器对时间系数进行非线性压缩；3) 在潜空间使用长短期记忆(LSTM)网络学习降阶模型(ROM)；4) 通过卷积神经网络(CNN)从预测的速度场映射到污染物扩散场。

Result: 模型在预测瞬时场和统计稳态场方面表现出高效性，能够实现长时间范围内的准确预测。

Conclusion: 该端到端数据驱动框架为城市污染物扩散建模提供了一种快速准确的解决方案，有助于支持城市环境管理和决策制定。
...
### [296] [AdaMuS: Adaptive Multi-view Sparsity Learning for Dimensionally Unbalanced Data](https://arxiv.org/abs/2603.17610)
*Cai Xu,Changhao Sun,Ziyu Guan,Wei Zhao*

Main category: cs.LG

TL;DR: 提出AdaMuS框架解决多视图学习中极端维度不平衡问题，通过参数无关剪枝和稀疏融合有效对齐不同维度视图。


<details>
  <summary>Details</summary>
Motivation: 实际应用中不同视图常存在严重维度差异（如视频帧10^6维vs生理信号10^1维），现有方法偏向高维数据且难以有效对齐表示，导致低维视图信息被忽视和冗余问题。

Method: 1) 构建视图特定编码器将不同维度映射到统一空间；2) 设计参数无关剪枝方法自适应移除编码器冗余参数；3) 提出稀疏融合范式抑制冗余维度并有效对齐视图；4) 引入自监督学习范式通过构建相似图获取监督信息。

Result: 在合成玩具数据集和7个真实基准测试上评估，AdaMuS在分类和语义分割任务中均取得优越性能，表现出强泛化能力。

Conclusion: AdaMuS框架能有效解决极端维度不平衡的多视图学习问题，防止忽略低维视图信息，减少冗余，提升表示对齐和泛化性能。
...
### [297] [Complementary Reinforcement Learning](https://arxiv.org/abs/2603.17621)
*Dilxat Muhtar,Jiashun Liu,Wei Gao,Weixun Wang,Shaopan Xiong,Ju Huang,Siran Yang,Wenbo Su,Jiamang Wang,Ling Pan,Bo Zheng*

Main category: cs.LG

TL;DR: 提出Complementary RL框架，通过让经验提取器与策略执行器协同进化，解决传统RL中经验与执行器能力不匹配的问题，提升样本效率。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习在训练LLM智能体时样本效率低，不仅因为稀疏奖励反馈，还因为智能体无法有效利用跨episode的历史经验。现有方法中，从历史中提取的经验要么静态存储，要么无法与改进中的执行器协同进化，导致经验与执行器能力逐渐不匹配，降低训练效果。

Method: 提出Complementary RL框架，受神经科学互补学习系统启发，在RL优化循环中实现经验提取器与策略执行器的无缝协同进化。执行器通过稀疏奖励优化，而经验提取器根据其提取的经验是否真正有助于执行器成功来优化，使经验管理策略与执行器能力同步进化。

Result: 实验表明，Complementary RL优于不学习经验的基于结果的智能体RL基线，在单任务场景中实现10%的性能提升，并在多任务设置中展现出强大的可扩展性。

Conclusion: Complementary RL建立了一个高效经验驱动智能体学习的新范式，通过协同进化机制解决了经验与执行器能力不匹配的问题，显著提升了样本效率。
...
### [298] [ARES: Scalable and Practical Gradient Inversion Attack in Federated Learning through Activation Recovery](https://arxiv.org/abs/2603.17623)
*Zirui Gong,Leo Yu Zhang,Yanjun Zhang,Viet Vo,Tianqing Zhu,Shirui Pan,Cong Wang*

Main category: cs.LG

TL;DR: ARES是一种无需修改架构的主动梯度反演攻击，通过稀疏恢复方法从大训练批次中高保真重建训练样本，揭示了联邦学习中激活值的严重隐私风险。


<details>
  <summary>Details</summary>
Motivation: 尽管联邦学习通过共享模型更新而非原始数据来保护隐私，但研究表明梯度更新可能泄露敏感训练数据。现有主动梯度反演攻击虽然强大，但通常需要修改模型架构，限制了实际应用。

Method: 提出ARES攻击，将恢复问题建模为噪声稀疏恢复任务，使用广义LASSO求解。通过imprint方法分离激活，实现多样本恢复。建立了恢复率的期望和重建误差的上界。

Result: 在CNN和MLP上的实验表明，ARES在大批次和实际联邦学习设置下实现了高保真重建，显著优于现有梯度反演攻击方法。

Conclusion: 中间激活值在联邦学习中构成严重且被低估的隐私风险，迫切需要更强的防御措施。
...
### [299] [Benchmarking Reinforcement Learning via Stochastic Converse Optimality: Generating Systems with Known Optimal Policies](https://arxiv.org/abs/2603.17631)
*Sinan Ibrahim,Grégoire Ouerdane,Hadi Salloum,Henni Ouerdane,Stefan Streif,Pavel Osinenko*

Main category: cs.LG

TL;DR: 提出一个强化学习算法的严格基准测试框架，通过扩展逆最优性到离散时间、控制仿射、非线性带噪声系统，为RL算法提供可重现的精确评估基础。


<details>
  <summary>Details</summary>
Motivation: 强化学习算法的比较非常复杂，因为结果和性能基准对环境设计、奖励结构和算法学习与环境动态中的随机性高度敏感。需要一种严格的基准测试框架来解决这一问题。

Method: 扩展逆最优性到离散时间、控制仿射、非线性带噪声系统，提供必要和充分条件来确定给定值函数和策略对于构造系统的最优性，通过同伦变化和随机化参数系统生成基准测试家族。

Result: 自动构建了多样化环境，展示了框架能够对算法进行受控和全面的评估。通过将标准方法与真实最优解进行比较，为精确严格的RL基准测试提供了可重现的基础。

Conclusion: 该框架为强化学习算法提供了系统化、可重现的基准测试方法，能够进行精确和严格的性能评估，解决了RL算法比较中的复杂性问题。
...
### [300] [Discovering Decoupled Functional Modules in Large Language Models](https://arxiv.org/abs/2603.17823)
*Yanke Yu,Jin Li,Ying Sun,Ping Li,Zhefeng Wang,Yi Zheng*

Main category: cs.LG

TL;DR: 提出ULCMOD框架，通过无监督方法发现LLM中的功能模块，揭示神经元如何组织成具有特定语义功能的模块，并展示模块的空间和层次结构。


<details>
  <summary>Details</summary>
Motivation: 理解LLM内部功能组织对于提高其可信度和性能至关重要，但目前LLM如何将不同功能组织成模块仍未被充分探索。

Method: 提出ULCMOD框架，包含新的目标函数和高效的迭代解耦算法，同时将整个LLM的大量神经元解耦成模块，并发现与这些模块相关的输入样本主题。

Result: 方法发现了高质量、解耦的模块，能捕获更有意义的语义信息，在各种下游任务中表现优异。模块具有语义一致性、可解释的专业化特征，以及清晰的空间和层次组织。
...
### [301] [DSS-GAN: Directional State Space GAN with Mamba backbone for Class-Conditional Image Synthesis](https://arxiv.org/abs/2603.17637)
*Aleksander Ogonowski,Konrad Klimaszewski,Przemysław Rokita*

Main category: cs.LG

TL;DR: DSS-GAN首次使用Mamba作为分层生成器骨干进行噪声到图像合成，通过新颖的方向性潜在路由机制实现更好的条件生成。


<details>
  <summary>Details</summary>
Motivation: 传统GAN的条件机制通常注入全局信号，无法有效耦合类别身份和潜在结构在特征图的不同空间轴上。需要一种新的条件机制来更好地控制生成过程。

Method: 提出方向性潜在路由(DLR)，将潜在向量分解为方向特定的子向量，每个子向量与类别嵌入联合投影，产生对应Mamba扫描的特征级仿射调制。使用Mamba作为分层生成器骨干。

Result: 在多个数据集上相比StyleGAN2-ADA取得了改进的FID、KID和精度-召回分数。潜在空间分析显示方向性子向量表现出可测量的专业化：沿单个分量的扰动产生结构化、方向相关的图像变化。

Conclusion: DSS-GAN通过方向性潜在路由机制成功实现了类别身份和潜在结构在空间轴上的耦合，为条件生成提供了更精细的控制能力。
...
### [302] [Only relative ranks matter in weight-clustered large language models](https://arxiv.org/abs/2603.17917)
*Borja Aizpurua,Sukhbinder Singh,Román Orús*

Main category: cs.LG

TL;DR: 论文提出权重相对排序比精确数值更重要，通过权重聚类将LLM压缩到16-64个不同值，无需训练即可保持准确性，并揭示了保持排序比保持统计分布更关键。
...
Method: 使用K-means聚类对预训练模型的权重矩阵进行压缩，将每个矩阵替换为K个共享值；可选地微调聚类中心以恢复精度；通过随机化聚类中心值来研究排序和统计分布的影响。

Result: 将每个矩阵压缩到16-64个不同值即可保持强准确性；微调聚类中心可恢复30-40%精度损失；破坏聚类相对排序会严重降低质量，而保持排序的随机化几乎无损失；尺度漂移是主要崩溃机制，但仿射校正可显著延迟。

Conclusion: 权重相对排序是LLM功能的关键因素，基于排序的压缩方法简单有效，为模型压缩和鲁棒性提供了新视角，仿射变换可缓解尺度漂移问题。
...
### [303] [Flow Matching Policy with Entropy Regularization](https://arxiv.org/abs/2603.17685)
*Ting Gao,Stavros Orfanoudakis,Nan Lin,Elvin Isufi,Winnie Daamen,Serge Hoogendoorn*

Main category: cs.LG

TL;DR: FMER提出基于流匹配和熵正则化的ODE在线强化学习框架，通过直线概率路径采样动作，相比扩散策略训练更快、熵控制更直接。


<details>
  <summary>Details</summary>
Motivation: 现有基于SDE的扩散策略存在两个问题：1) 由于精确熵难以计算，只能依赖间接熵控制；2) 通过迭代去噪链计算策略梯度计算代价过高。需要更高效、可扩展的策略表示方法。

Method: FMER采用基于ODE的流匹配策略，通过最优传输思想在直线概率路径上采样动作。利用生成模型特性构建优势加权目标速度场，从候选集中引导策略更新至高价值区域。推导出可处理的熵目标，实现原则性的最大熵优化。

Result: 在稀疏多目标FrankaKitchen基准测试中超越最先进方法，在标准MuJoco基准测试中保持竞争力。相比QVPO等扩散基线训练时间减少7倍，相比高效变体减少10-15%。

Conclusion: FMER提供了一种高效、可扩展的ODE-based RL框架，通过流匹配和直接熵正则化解决了扩散策略的计算和熵控制问题，在复杂任务中表现出色且训练效率显著提升。
...
### [304] [Objective Mispricing Detection for Shortlisting Undervalued Football Players via Market Dynamics and News Signals](https://arxiv.org/abs/2603.17687)
*Chinenye Omejieke,Shuyao Chen,Xia Cui*

Main category: cs.LG

TL;DR: 提出一个基于客观错误定价识别被低估足球球员的实用框架，通过市场价值预测与新闻NLP特征结合来筛选被低估球员
...
Method: 使用结构化数据（历史市场动态、传记合同特征、转会历史）预测市场价值，与观察价值比较定义错误定价；加入新闻NLP特征（情感统计和语义嵌入）；采用时序评估和梯度提升回归

Result: 梯度提升回归能解释市场价值的大部分方差；市场动态是主要信号，NLP特征提供一致的次要增益；SHAP分析显示市场趋势和年龄占主导，新闻波动线索在高不确定性情况下增强信号

Conclusion: 提出的管道适用于球探工作流程的决策支持，强调排名/筛选而非硬分类阈值，包含可重复性和伦理声明
...
### [305] [Can Blindfolded LLMs Still Trade? An Anonymization-First Framework for Portfolio Optimization](https://arxiv.org/abs/2603.17692)
*Joohyoung Jeon,Hongchul Lee*

Main category: cs.LG

TL;DR: 论文提出BlindTrade方法，通过匿名化股票代码和公司名称来验证LLM交易代理是否真正理解市场动态而非依赖记忆，使用GNN和PPO-DSR策略在2025年实现Sharpe 1.40，并发现策略在波动市场中表现更好。


<details>
  <summary>Details</summary>
Motivation: 确保LLM交易代理真正理解市场动态而非依赖预训练记忆，解决记忆偏差和幸存者偏差问题，构建可信赖的多代理系统。

Method: 采用盲测方法：匿名化所有股票代码和公司名称，四个LLM代理输出评分和推理，从推理嵌入构建GNN图，使用PPO-DSR策略进行交易。

Result: 在2025年YTD（至2025-08-01）实现Sharpe 1.40 ± 0.22（20个种子平均），通过负控制实验验证信号有效性。在2024-2025年扩展测试中，发现策略在波动市场中表现优异，但在趋势性牛市中alpha减少。

Conclusion: BlindTrade方法能有效验证LLM交易信号的合法性，策略表现具有市场状态依赖性，为构建可信赖的LLM交易系统提供了验证框架。
...
### [306] [Predicting Trajectories of Long COVID in Adult Women: The Critical Role of Causal Disentanglement](https://arxiv.org/abs/2603.17722)
*Jing Wang,Jie Shen,Yiming Luo,Amar Sra,Qiaomin Xie,Jeremy C. Weiss*

Main category: cs.LG

TL;DR: 基于LLM的因果网络模型，整合临床数据和可穿戴设备数据，能够高精度预测女性COVID-19后遗症严重程度，并能有效区分真实病理信号与更年期等混杂因素。


<details>
  <summary>Details</summary>
Motivation: COVID-19后遗症（PASC）的早期预测对女性健康至关重要，但PASC症状与更年期等常见激素转变存在诊断重叠，需要准确区分这些混杂因素以实现精准的长期轨迹预测。

Method: 回顾性研究1,155名女性患者，整合静态临床档案和四周纵向可穿戴设备数据（监测心脏活动和睡眠），基于大语言模型构建因果网络来预测未来PASC评分。

Result: 临床严重程度预测精度达到86.7%；因果归因分析显示模型能有效区分病理信号与基线噪声：呼吸困难、不适等直接指标达到最大显著性（1.00），而更年期、糖尿病等混杂因素被成功抑制（显著性低于0.27）。

Conclusion: 该因果网络框架不仅能高精度预测PASC严重程度，还能有效识别和抑制混杂因素，为女性COVID-19后遗症的精准预测提供了有力工具。
...
### [307] [Embedding World Knowledge into Tabular Models: Towards Best Practices for Embedding Pipeline Design](https://arxiv.org/abs/2603.17737)
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏关于如何为表格预测任务设计有效LLM嵌入管道的证据，需要系统评估不同配置对预测性能的影响

Method: 系统基准测试256种管道配置，涵盖8种预处理策略、16种嵌入模型和2种下游模型，评估LLM嵌入在表格预测中的效果

Result: 管道设计对性能影响显著；拼接嵌入优于替换原始列；大嵌入模型表现更好；公开排行榜排名和模型流行度是差的性能指标；梯度提升决策树是强下游模型

Conclusion: 为研究人员和从业者提供了构建更有效表格预测嵌入管道的指导，强调管道设计的重要性而非单纯依赖模型规模或流行度
...
### [308] [Towards Infinitely Long Neural Simulations: Self-Refining Neural Surrogate Models for Dynamical Systems](https://arxiv.org/abs/2603.17750)
*Qi Liu,Laure Zanna,Joan Bruna*

Main category: cs.LG

TL;DR: 提出SNS模型，通过条件扩散模型平衡短期精度与长期一致性，解决自回归神经代理模型中的分布漂移问题


<details>
  <summary>Details</summary>
Motivation: 自回归神经代理模型在模拟动力系统时存在分布漂移问题，即自回归展开中的累积误差会严重降低长期生成质量。现有方法通过超参数调优隐式利用短期精度与长期一致性之间的权衡，但缺乏明确的数学框架。

Method: 提出统一的数学框架，使短期精度与长期一致性的权衡显式化。在此基础上，提出SNS（Self-refining Neural Surrogate）模型，这是一个无需超参数的条件扩散模型，通过构造平衡短期保真度与长期一致性。SNS可以作为独立模型优化自身自回归输出，或作为现有神经代理模型的补充模型。

Result: SNS模型在复杂动力系统的高保真模拟中表现出数值可行性，能够在任意长的时间范围内保持模拟质量。

Conclusion: SNS模型通过条件扩散方法有效解决了自回归神经代理模型的分布漂移问题，为长期动力系统模拟提供了稳健的解决方案，既可作为独立模型使用，也可增强现有神经代理模型的长期一致性。
...
### [309] [Attention Sinks Induce Gradient Sinks](https://arxiv.org/abs/2603.17771)
*Yihong Chen,Quanming Yao*

Main category: cs.LG

TL;DR: 研究发现注意力汇聚（attention sinks）在因果掩码下会导致梯度汇聚（gradient sinks），而大规模激活（massive activations）是训练期间对这种梯度压力的自适应响应。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注Transformer模型的前向传播，不清楚注意力汇聚和大规模激活之间的连接是直接的还是由训练时机制介导的。本文从反向传播的角度研究这个问题。

Method: 通过实证和理论分析，研究因果掩码下注意力汇聚如何导致梯度汇聚。引入V-scale修改来调整值路径的反向传播梯度，测试梯度汇聚作为中介机制的假设。

Result: 在预训练的V-scale模型中，注意力汇聚得以保留而大规模激活被抑制。这支持了梯度汇聚是连接注意力汇聚和大规模激活的关键训练时中介机制。

Conclusion: 梯度汇聚是连接注意力汇聚和大规模激活的关键训练时中介机制，大规模激活是预归一化架构中对局部梯度压力的自适应响应。
...
### [310] [RangeAD: Fast On-Model Anomaly Detection](https://arxiv.org/abs/2603.17795)
*Luca Hinkamp,Simon Klüttermann,Emmanuel Müller*

Main category: cs.LG

TL;DR: 提出On-Model AD框架，利用主模型内部信息进行异常检测，避免额外独立AD模型的开销
...
Method: 提出RangeAD算法，利用主模型的神经元输出范围进行异常检测，显著降低推理成本

Result: RangeAD在高维任务上表现优异，同时大幅降低推理开销，证明了On-Model AD框架的实用性

Conclusion: On-Model AD为高效异常检测提供了实用框架，通过利用主模型已有信息实现更优性能
...
### [311] [Dropout Robustness and Cognitive Profiling of Transformer Models via Stochastic Inference](https://arxiv.org/abs/2603.17811)
*Antônio Junior Alves Caiado,Michael Hahsler*

Main category: cs.LG

TL;DR: 本文首次系统评估了19个Transformer模型在推理时随机性（MC Dropout）下的表现，发现模型对dropout的鲁棒性高度依赖于架构而非规模，53%的模型在基线MC Dropout下出现严重精度下降，不适合不确定性量化应用。


<details>
  <summary>Details</summary>
Motivation: Transformer语言模型在推理时随机性（如dropout）下的行为尚未得到充分研究，限制了模型在不确定性感知应用中的可靠性评估。需要系统评估不同架构模型在MC Dropout下的表现，为模型选择提供指导。

Method: 使用MC Dropout方法，对19个Transformer模型进行100次随机前向传播采样。采用认知分解框架将性能分解为记忆和推理组件。实验涵盖5种dropout配置，在1000个样本上进行95次独特评估。

Result: 发现显著的架构差异：小模型预测稳定，中等模型波动大；中等规模模型整体性能最佳，大模型在记忆任务上表现优异。53%的模型在基线MC Dropout下精度严重下降（最高24个百分点）。记忆任务受dropout影响更大（下降27个百分点），推理任务仅下降1个百分点。84%的模型表现出记忆偏向性能。

Conclusion: 这是首个全面的Transformer MC Dropout基准测试，表明dropout鲁棒性依赖于架构而非规模。认知分析框架为不确定性感知应用中的模型选择提供了实用指导，揭示了现有模型在不确定性量化方面的局限性。
...
### [312] [Federated Distributional Reinforcement Learning with Distributional Critic Regularization](https://arxiv.org/abs/2603.17820)
*David Millard,Cecilia Alm,Rashid Ali,Pengcheng Shi,Ali Baheri*

Main category: cs.LG

TL;DR: 提出联邦分布强化学习(FedDistRL)，通过量化价值函数和Wasserstein重心约束，解决传统联邦RL中参数平均导致的分布信息丢失问题，提升安全关键场景下的性能。
...
Method: 提出FedDistRL框架，客户端参数化量化价值函数critics并仅联邦化这些网络。进一步提出TR-FedDistRL，为每个客户端构建基于时间缓冲区的风险感知Wasserstein重心，作为参考区域约束参数平均的critic，通过shrink-squash步骤实现分布信任区域。

Result: 在固定策略评估下，可行性映射是非扩张的，更新在探针集Wasserstein度量下是收缩的。实验在老虎机、多智能体网格世界和连续高速公路环境中显示：减少了均值模糊、改善了安全代理指标（灾难/事故率）、降低了critic/策略漂移。

Conclusion: FedDistRL和TR-FedDistRL能有效保留分布信息，避免传统联邦RL中的均值模糊问题，在安全关键场景中提供更好的性能和安全保障。
...
### [313] [Symmetry-Reduced Physics-Informed Learning of Tensegrity Dynamics](https://arxiv.org/abs/2603.17824)
*Jing Qin,Muhao Chen*

Main category: cs.LG

TL;DR: 提出SymPINN框架，将群论对称性嵌入PINN中预测张拉整体结构动力学，通过对称轨道分解和对称基表示，显著提升预测精度和计算效率。
...
Method: 提出SymPINN框架：1) 将节点分解为对称轨道，用对称基表示自由节点坐标；2) 通过对称变换恢复完整坐标；3) 通过轨道坐标生成、对称一致消息传递和物理残差约束强制等变性；4) 编码初始条件为硬约束，使用傅里叶特征编码，采用两阶段优化策略。

Result: 在对称T型杆和着陆器结构上的数值实验表明，相比标准物理信息模型，预测精度和计算效率显著提升。

Conclusion: SymPINN框架展示了对称感知学习在张拉整体结构动力学保持建模方面的巨大潜力，为利用几何对称性改进物理信息神经网络提供了有效途径。
...
### [314] [Physics-Aware Machine Learning for Seismic and Volcanic Signal Interpretation](https://arxiv.org/abs/2603.17855)
*William Thorossian*

Main category: cs.LG

TL;DR: 本文综述了机器学习和信号处理在地震与火山监测中的最新进展，强调模型需具备跨域鲁棒性、不确定性量化和物理可解释性，以支持实际决策应用。


<details>
  <summary>Details</summary>
Motivation: 现代地震和火山监测面临多传感器连续观测、非平稳噪声场等挑战，机器学习虽已从研究工具发展为实用处理链组件，但仅提高固定数据集精度不足。模型需在域偏移（新台站、变化噪声、演化火山活动）下保持可靠，提供支持决策的不确定性，并将输出与物理约束关联。

Method: 本文系统综述并组织地震与火山信号分析的近期机器学习方法：1）强调经典信号处理提供必要的归纳偏置；2）展示自监督和生成建模如何减少对标注数据的依赖；3）提出最能反映跨区域迁移性能的评估协议。

Result: 论文系统梳理了机器学习在地震火山监测中的应用框架，识别了关键方法学进展：经典信号处理与ML的结合、无监督/自监督方法减少标注需求、以及面向实际部署的评估标准。

Conclusion: 总结指出面向鲁棒、可解释和可维护的AI辅助监测仍存在开放挑战，包括模型在动态环境中的可靠性、不确定性量化、物理约束整合以及跨区域泛化能力等关键问题。
...
### [315] [Procedural Generation of Algorithm Discovery Tasks in Machine Learning](https://arxiv.org/abs/2603.17863)
...
TL;DR: DiscoGen是一个用于机器学习算法发现任务的程序化生成器，通过配置参数生成数百万个不同难度和复杂度的任务，用于优化算法发现代理，并提供了DiscoBench基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有算法发现系统的任务套件存在评估方法不佳、数据污染、问题饱和或相似等问题，限制了算法发现系统的改进和评估能力。

Method: 受强化学习中程序化生成的启发，DiscoGen通过少量配置参数生成数百万个机器学习算法发现任务，涵盖多个机器学习领域，并创建了DiscoBench基准测试。

Result: 开发了DiscoGen系统，能够生成大量多样化的算法发现任务，并展示了其在算法发现代理提示优化中的应用。

Conclusion: DiscoGen为算法发现研究提供了强大的工具，开启了多个有影响力的研究方向，并已开源发布。
...
### [316] [RHYME-XT: A Neural Operator for Spatiotemporal Control Systems](https://arxiv.org/abs/2603.17867)
*Marijn Ruiter,Miguel Aguiar,Jake Rap,Karl H. Johansson,Amritam Das*

Main category: cs.LG

TL;DR: RHYME-XT：用于具有局部节律行为的输入仿射非线性偏积分微分方程时空控制系统的算子学习框架，通过Galerkin投影和神经网络参数化基函数实现高效代理建模。


<details>
  <summary>Details</summary>
Motivation: 需要为具有局部节律行为的输入仿射非线性PIDEs时空控制系统开发高效的代理建模方法，避免传统方法的高计算成本。

Method: 使用Galerkin投影将无限维PIDE近似到学习到的有限维子空间，空间基函数由神经网络参数化；直接学习投影ODE系统的流映射，避免积分非自治系统。

Result: 在神经场PIDE实验中，RHYME-XT优于最先进的神经算子，并能通过微调在不同数据集训练的模型间有效迁移知识。

Conclusion: RHYME-XT为具有节律行为的PIDEs控制系统提供了一种计算高效、连续时间、离散化不变的代理建模框架，具有优越性能和知识迁移能力。
...
### [317] [Operator-Theoretic Foundations and Policy Gradient Methods for General MDPs with Unbounded Costs](https://arxiv.org/abs/2603.17875)
*Abhishek Gupta,Aditya Mahajan*

Main category: cs.LG

TL;DR: 论文将MDP视为函数空间上线性算子的优化问题，利用算子扰动理论推导目标函数导数，将强化学习结果推广到一般状态动作空间，并提出新的低复杂度PPO算法。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习理论主要局限于有限状态动作空间或特定线性函数逼近，缺乏对一般状态动作空间MDP的统一理论框架和高效算法。

Method: 将MDP重新表述为函数空间上线性算子的优化问题，应用线性算子扰动理论推导目标函数导数，建立一般状态动作空间下的理论框架。

Result: 将许多强化学习经典结果推广到一般状态动作空间，突破了有限状态动作空间的限制，并提出了新的低复杂度PPO类型算法。

Conclusion: 该框架为一般状态动作空间MDP提供了统一的理论基础，扩展了强化学习理论的应用范围，并启发了新的高效算法设计。
...
### [318] [RAMP: Reinforcement Adaptive Mixed Precision Quantization for Efficient On Device LLM Inference](https://arxiv.org/abs/2603.17891)
*Arpit Singh Gautam,Saurabh Jha*

Main category: cs.LG

TL;DR: RAMP是一个基于强化学习的自适应混合精度量化框架，通过每层比特分配优化模型性能，在保持低比特的同时提升准确率，并实现跨模型零样本迁移。


<details>
  <summary>Details</summary>
Motivation: 现有后训练量化方法通常对所有层使用统一的比特宽度，导致准确率与效率的权衡不够优化。需要一种能够根据每层特性自适应分配比特宽度的方法，以在有限比特预算下最大化模型性能。

Method: 提出RAMP框架：1) 使用离策略Soft Actor-Critic强化学习算法学习每层比特分配；2) 基于11维特征嵌入（激活统计、权重特性、结构描述符）进行决策；3) 引入Scale Folding预处理技术，通过每通道缩放和归一化层补偿将激活异常值迁移到权重中；4) 采用质量优先奖励函数，包含非对称惩罚和预算悬崖机制。

Result: 在Llama 2 7B上，RAMP达到5.54困惑度（3.68GB，3.65有效比特），优于均匀4比特AWQ（5.60困惑度，3.90GB）和GPTQ，尺寸减少6%，质量提升1-3%。仅在Llama 2 7B上训练的策略能够零样本迁移到Llama 2 13B和Mistral 7B，甚至超过特定目标训练效果。HALO管道导出到GGUF格式，在CPU、GPU和边缘设备上保持99.5%的FP16常识推理性能。

Conclusion: RAMP证明了通过强化学习自适应分配比特宽度的有效性，能够实现更好的准确率-效率权衡。其跨模型零样本迁移能力表明量化敏感性主要与架构相关而非特定模型。Scale Folding技术使低于4比特的稳定量化成为可能，为资源受限设备上的LLM部署提供了实用解决方案。
...
### [319] [CARE: Covariance-Aware and Rank-Enhanced Decomposition for Enabling Multi-Head Latent Attention](https://arxiv.org/abs/2603.17946)
...
TL;DR: CARE方法通过协方差感知的秩增强转换，将预训练注意力模块转换为MLA格式，在固定KV缓存成本下提升表达力，显著降低困惑度并保持准确率。


<details>
  <summary>Details</summary>
Motivation: 现有方法将预训练注意力模块转换为MLA时，仅关注权重矩阵差异而忽略激活协方差结构，采用均匀秩分配导致激活漂移和注意力保真度下降，需要更有效的转换方法。

Method: 提出CARE转换流程：1) 激活保持分解，基于实际输入激活而非仅权重进行近似；2) 调整秩分配，在固定KV预算下为需求更大的层分配更多容量；3) KV对等映射，重新参数化K和V以适应MLA格式同时保持KV缓存大小不变。

Result: 在Qwen3-4B/30B和Llama-3.1-8B/70B模型上，相比均匀秩SVD基线，单次困惑度降低高达215倍，平均准确率提升高达1.70倍。经简短后处理微调后能完全恢复原始模型准确率。

Conclusion: CARE方法通过协方差感知的秩增强转换，在固定KV缓存成本下显著提升MLA转换质量，为高效推理提供了有效的注意力模块转换方案。
...
### [320] [Unified Policy Value Decomposition for Rapid Adaptation](https://arxiv.org/abs/2603.17947)
...
<details>
  <summary>Details</summary>
Motivation: 解决复杂控制系统中快速适应的挑战，传统方法需要重新训练表示，而本文旨在实现无需重新训练表示即可立即适应新任务的能力

Method: 采用双线性actor-critic分解框架：critic分解为Q = Σ_k G_k(g) y_k(s,a)，其中G_k(g)是目标条件系数向量，y_k(s,a)是学习的值基函数；actor类似地组合一组原始策略，由相同系数G_k(g)加权；测试时基函数冻结，G_k(g)通过单次前向传播零样本估计

Result: 在MuJoCo Ant环境的多方向运动任务上训练Soft Actor-Critic智能体，双线性结构使每个策略头专门处理方向子集，共享系数层实现跨方向泛化，通过目标嵌入空间插值适应新方向
...
### [321] [Interpretable AI-Assisted Early Reliability Prediction for a Two-Parameter Parallel Root-Finding Scheme](https://arxiv.org/abs/2603.16980)
*Bruno Carpentieri,Andrei Velichko,Mudassir Shams,Paola Lecca*

Main category: math.NA

TL;DR: 提出基于kNN-LLE代理稳定性分析和多时间尺度早期预测的可解释AI辅助可靠性诊断框架，用于参数化求根方案


<details>
  <summary>Details</summary>
Motivation: 传统数值求解器在参数化求根问题中难以早期识别稳定与不稳定参数区域，需要一种能够从迭代动态早期阶段预测求解器可靠性的方法

Method: 结合kNN-LLE代理稳定性分析和多时间尺度早期预测，构建轻量级预测层，从迭代动态的早期片段估计求解器可靠性，推导基于收缩性的可靠性评分

Result: 在两参数并行求根方案实验中，仅需几次迭代即可实现可靠预测：最佳模型在T=1时R²=0.48，T=3时提升至0.67，在稳定性特征最小位置尺度前超过0.89，更大时间尺度下R²达0.96，平均绝对误差约0.03，推理成本可忽略（微秒级）

Conclusion: 该框架提供可解释的稳定性指标，支持求解器执行期间的早期决策（继续、重启或调整参数），实现了高效可靠的AI辅助数值求解诊断
...
### [322] [A space-time dual-pairing summation-by-parts framework for forward and adjoint wave equations](https://arxiv.org/abs/2603.17014)
*Kenny Wiratama,Kenneth Duru,Yunho Kim*

Main category: math.NA

TL;DR: 提出首个时空对偶配对求和分部（DP-SBP）数值框架，用于正演和伴随波传播问题，实现时空高阶精度并自然引入时间耗散。


<details>
  <summary>Details</summary>
Motivation: 需要开发一种能够同时处理正演和伴随波传播问题的数值框架，该框架应具备时空高阶精度、自然时间耗散特性，并能用于解决反演波传播问题。

Method: 采用时空对偶配对求和分部（DP-SBP）方法，结合同时逼近项（SAT）技术弱施加初始和边界条件，构建完全离散的能量估计以确保数值格式稳定性。

Result: 建立了稳定的完全离散数值格式，实现了伴随一致性，能够用于解决反演波传播问题，并通过一维和二维数值实验验证了理论分析和数值误差收敛性。

Conclusion: 提出的时空DP-SBP框架是首个能够同时处理正演和伴随波传播问题的数值方法，具备时空高阶精度、自然时间耗散和伴随一致性，适用于反演问题求解。
...
### [323] [On the role of relaxation and acceleration in the non-overlapping Schwarz alternating method for coupling](https://arxiv.org/abs/2603.17143)
*Giulia Sambataro,Irina Tezaur*

Main category: math.NA

TL;DR: 研究松弛和加速技术对非重叠Schwarz算法收敛行为的影响，比较Aitken加速和Anderson加速在域分解耦合中的表现


<details>
  <summary>Details</summary>
Motivation: 研究如何通过加速技术改进非重叠Schwarz算法的收敛性能，特别是在域分解耦合场景中，探索更高效的数值计算方法

Method: 将乘性Schwarz格式表述为不动点迭代，理论分析和数值实验结合，研究Aitken加速和Anderson加速技术，并提出自适应Anderson加速变体"Anderson with memory adaptation"

Result: Aitken加速Schwarz在双子域设置中效率和鲁棒性最佳，而Anderson加速Schwarz在更多子域的多域设置中表现更优

Conclusion: 不同加速技术适用于不同域分解场景：Aitken加速适合双子域问题，Anderson加速适合多子域问题，提出的自适应Anderson加速变体具有鲁棒性和高效性
...
### [324] [Modified Halley's method for computation of zeros of solution of second order ODEs](https://arxiv.org/abs/2603.17448)
*Dhivya Prabhu K,Sanjeev Singh,Antony Vijesh*

Main category: math.NA

TL;DR: 提出计算二阶常微分方程解的所有零点的有效迭代方法，包括三阶Halley方法和改进的Halley方法，并应用于高斯-勒让德和高斯-埃尔米特求积的节点与权重计算。


<details>
  <summary>Details</summary>
Motivation: 需要高效计算二阶常微分方程解的所有零点，这在数值分析中具有重要应用，特别是对于高斯求积公式的节点和权重计算。

Method: 1. 通过近似关联的Riccati微分方程推导三阶Halley方法；2. 提出改进的Halley方法，将Halley方案中的一个函数固定为常数以提高计算效率；3. 基于二阶ODE系数行为建立非局部收敛性结果；4. 为两种方法提供合适的初始猜测；5. 基于改进的Halley方法开发计算高斯-勒让德和高斯-埃尔米特求积节点与权重的算法。

Result: 改进的Halley方法保持三阶收敛性，同时提高计算效率。建立了两种方法的非局部收敛性理论。开发的算法在数值实验中表现出优于现有方法的效率。

Conclusion: 提出的改进Halley方法能高效计算二阶ODE解的所有零点，特别适用于高斯求积公式的数值实现，具有理论和实际应用价值。
...
### [325] [Wavenumber-explicit $hp$-FEM analysis of Maxwell's equations with impedance boundary conditions in piecewise smooth media](https://arxiv.org/abs/2603.17467)
*Jens Markus Melenk,David Wörgötter*

Main category: math.NA

TL;DR: 针对具有解析边界和分片解析介质参数的时间谐波Maxwell方程，证明了在多项式适定条件下，Nédélec元Galerkin离散的拟最优性，需要满足波数相关的尺度分辨率条件。


<details>
  <summary>Details</summary>
Motivation: 研究具有跳跃系数的Maxwell方程的数值离散，特别是在波数较大时保持数值方法的稳定性，为高频电磁场计算提供理论保证。

Method: 使用Nédélec有限元进行Galerkin离散，分析需要满足两个波数相关的尺度分辨率条件：1) |k|h/p足够小；2) p/log|k|有下界。

Result: 证明了在多项式适定性和分辨率条件下，离散化是拟最优的，即离散解与连续解之间的误差由最佳逼近误差控制。

Conclusion: 对于具有解析边界和分片解析系数的Maxwell方程，在适当的波数相关分辨率条件下，Nédélec元离散能够保持拟最优收敛性。
...
### [326] [A New Fractional Step Structure Preserving Method for The Landau-Lifshitz-Gilbert Equation](https://arxiv.org/abs/2603.17477)
*Changjian Xie*

Main category: math.NA

TL;DR: 提出一种基于Crank-Nicolson格式和隐式Gauss-Seidel分数迭代的结构保持方法，具有时间一阶、空间二阶精度，且稳定并保持长度


<details>
  <summary>Details</summary>
Motivation: 开发一种能够保持结构特性（如长度保持）的数值方法，便于理论分析，同时保证数值稳定性和精度

Method: 采用Crank-Nicolson类型方法结合隐式Gauss-Seidel分数迭代，构建时间一阶精度、空间二阶精度的结构保持算法

Result: 方法在1D和3D测试中验证了数值精度、范数保持和稳定性，证明其有效性

Conclusion: 提出的结构保持方法在理论和数值上都具有优势，为相关问题的分析提供了有效工具
...
### [327] [On the validity limits of the parametrisation method for invariant manifolds: an assessment of practical criteria for vibrating systems](https://arxiv.org/abs/2603.17611)
*André de Figueiredo Stabile,Aurélien Grolet,Alessandra Vizzaccaro,Cyril Touzé*

Main category: math.NA

TL;DR: 本文研究参数化方法在非线性振动系统降阶模型中的有效性限制，提出三种实用准则来估计近似解的有效范围，并在不同示例中验证这些准则。


<details>
  <summary>Details</summary>
Motivation: 参数化方法虽然能准确计算非线性模态，但由于依赖局部理论和渐近展开，结果只在特定振幅范围内有效。需要建立实用方法来估计降阶模型的有效使用范围。

Method: 提出三种有效性判断准则：1) 不变方程误差随固定点距离的变化；2) 基于同调算子奇异性的上界准则；3) 使用柯西和达朗贝尔级数收敛规则。在Duffing方程、两自由度系统和有限元梁结构上进行测试。

Result: 三种准则在不同非线性振动场景中进行了评估，包括保守振荡、主共振强迫系统和超谐波共振。准则在有限元梁结构上也得到验证，为非线性振动结构降阶模型提供了准确易用的误差边界。

Conclusion: 建立了实用的误差估计准则来界定参数化方法在非线性振动降阶模型中的有效范围，为工程应用提供了安全的使用边界和指导原则。
...
### [328] [Decoupled Divergence-Free Neural Networks Basis Method for Incompressible Fluid Problems](https://arxiv.org/abs/2603.17906)
*Jinbao Cheng,Jianguo Huang,Haoqin Wang,Tao Zhou*

Main category: math.NA

TL;DR: 提出Decoupled-DFNN方法，通过流函数/矢量势表示速度场确保严格无散度，将不可压缩流问题解耦为速度和压力子问题，采用顺序求解策略降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有方法在求解不可压缩流（Stokes和Navier-Stokes方程）时，需要同时满足无散度约束和求解压力场，计算成本高且难以严格保持无散度条件。

Method: 1. 使用流函数（2D）或矢量势（3D）的旋度表示速度场，确保严格无散度；2. 利用旋度算子性质推导解耦的速度和压力子问题；3. 采用顺序求解策略；4. 对Navier-Stokes非线性问题使用Gauss-Newton线性化；5. 使用TransNet框架求解子问题。

Result: 相比现有方法，该方法在严格保持不可压缩约束的同时降低了计算成本，实现了速度和压力的独立求解。

Conclusion: Decoupled-DFNN方法通过解耦策略和严格的无散度表示，为求解不可压缩流问题提供了一种高效且精确的数值方法。
...
### [329] [State-dependent temperature control in Langevin diffusions using numerical exploratory Hamiltonian-Jacobi-Bellman equations](https://arxiv.org/abs/2603.17934)
*Taorui Wang,Xun Li,Gu Wang,Zhongqiang Zhang*

Main category: math.NA

TL;DR: 提出一种基于物理信息神经网络求解探索性HJB方程的方法，用于稳定计算高维优化中状态依赖的噪声


<details>
  <summary>Details</summary>
Motivation: 朗之万动力学中噪声大小的选择对优化效果至关重要，但现有基于HJB方程的方法在高维问题中面临控制边界设置和二阶信息计算不稳定两大挑战

Method: 引入原则性控制边界，开发物理信息神经网络框架，将探索性HJB方程结构直接嵌入训练过程，稳定计算并实现高维问题中状态依赖噪声的准确估计

Result: 数值实验表明该方法在超越低维测试案例的高维问题中保持鲁棒性和有效性

Conclusion: 首次系统研究了HJB参数的影响，提出的框架解决了高维优化中噪声选择的关键挑战，为复杂优化问题提供了稳定有效的解决方案
